...
def testManifestHeader(root):
    ...
    try:
        ...
        print("Warning: android:installLocation is not 'preferExternal'")
...
def testXperiaOnly(root):
    ...
    for x in libs:
        if (
            x.getAttribute(AName) == "xperiaplaycertified"
            and x.getAttribute("android:required") == "true"
        ):
            return True
    return False
...
def testPlaystationOnly(root):
    ...
    for x in libs:
        if (
            x.getAttribute(AName) == "playstationcertified"
            and x.getAttribute("android:required") == "true"
        ):
            return True
    return False
...
def testNameAndIcon(root):
    ...
    for x in metas:
        if x.hasAttribute(AName):
            if x.getAttribute(AName) == "game_display_name":
                hasName = True
            if x.getAttribute(AName) == "game_icon":
                hasIcon = True
...
def testDebuggable(root):
    elem = root.getElementsByTagName("application")[0]
    if not elem.hasAttribute("android:debuggable"):
        return True
...
def testMinSdkVersion(Version):
    def _minSdkVersion(root):
        ...
        if not elem.hasAttribute("android:minSdkVersion"):
            return False
...
if __name__ == "__main__":
    ...
    allStores = {
        ...
        Google: [marketPerms],
        Verizon: [verizonPerms, testRealLicenseCheck],
        Exclusive: [testXperiaOnly, testPlaystationOnly],
    }

    try:
        fn = sys.argv[1]
    except:
        fn = r"C:\dev\subversion\mojang\minecraftcpp\trunk\handheld\project\android\AndroidManifest.xml"

    try:
        stores = sys.argv[2].lower().split(",")
    except:
        stores = allStores.keys()
    if General not in stores:
        stores.append(General)
