...
def PackageToGuards(x):
    try:
        ...
        package = "_".join(pkg)
        x = x[:b] + "//" + x[b:]
        clazz = re.search("(class|interface) (%s)" % h.identifier(), x, re.M).groups()[
            1
        ]
...
def convert(infilename, outfilename, params):
    ...
    for rexp in params.regexps:
        type, src, dst = rexp[:3]
        try:
            dstFunc = F.get(rexp[3], None)(dst)
        except:
            dstFunc = None
        if type == SUB:
            outs = re.sub(src, dstFunc or dst, outs)

    file(outfilename, "w").write(outs)


def compileRegexpRules(ruleList):
    return ruleList
...
JavaToCppParams = Params(
    JavaToCppRules, (FUNC_PER_ROW, ImportToInclude), (FUNC_PER_FILE, PackageToGuards)
)
