...
public:
  ...
  CompoundTag(const std::string &name) : super(name) {}
  ...
  void getAllTags(std::vector<Tag *> &tags_) const {
    // std::transform(tags.begin(), tags.end(), std::back_inserter(tags_),
    // PairValueFunctor());
