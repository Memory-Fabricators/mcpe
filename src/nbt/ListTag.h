...
public:
  ...
  ListTag(const std::string &name) : super(name) {}

  void write(IDataOutput *dos) /*throws IOException*/ {
    if (list.size() > 0)
      type = list.front()->getId();
    else
      type = TAG_Byte;
...
public:
  ListTagFloatAdder() : tag(NULL) {}
  ListTagFloatAdder(float f) : tag(NULL) { operator()(f); }
  ListTagFloatAdder(ListTag *tag) : tag(tag) {}
  ...
  ListTagFloatAdder &operator()(float f) { return operator()("", f); }
