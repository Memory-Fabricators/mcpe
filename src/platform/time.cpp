...
#include <ctime>
int getEpochTimeS() { return (int)std::time(NULL); }

StopwatchNLast::StopwatchNLast(int numMeasurements)
    : n(numMeasurements), k(0), sum(0) {
  t = new float[n];
  memset(t, 0, n * sizeof(float));
}

StopwatchNLast::~StopwatchNLast() { delete[] t; }
...
// Stopwatch
//
Stopwatch::Stopwatch() : _count(0), _printcounter(0) { reset(); }

void Stopwatch::start() { _st = getTimeS(); }
...
float Stopwatch::getLast() { return _last; }

float Stopwatch::getTotal() { return _tt; }

float Stopwatch::getMax() { return _max; }

int Stopwatch::getCount() { return _count; }
