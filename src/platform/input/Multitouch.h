...
public:
  ...
  static bool wasFirstMovement(int pointerId) {
    return g(pointerId).wasFirstMovement();
  }
  ...
  static void rewind() { _index = -1; }

  static MouseAction &getEvent() { return _inputs[_index]; }
  ...
  static int getMaxPointers() { return MAX_POINTERS; }
