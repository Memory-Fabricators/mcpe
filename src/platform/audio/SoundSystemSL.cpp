...
SoundSystemSL::SoundSystemSL()
    : available(true), listener(NULL), numBuffersPlaying(0) {
  init();
}
...
void SoundSystemSL::init() {
  ...
  res =
      slCreateEngine(&SoundSystemSL::objEngine, 1, EngineOption, 0, NULL, NULL);
  checkErr(res);

  /* Realizing the SL Engine in synchronous mode. */
  res = (*SoundSystemSL::objEngine)
            ->Realize(SoundSystemSL::objEngine, SL_BOOLEAN_FALSE);
  if (checkErr(res)) {
    available = false;
    return;
  }
  ...
  required */
  res = (*engEngine)
            ->CreateOutputMix(engEngine, &objOutput, 0, iidArray, required);
  checkErr(res);
...
void SoundSystemSL::setListenerAngle(float deg) {
  ...
  SLresult res =
      (*listener)->SetOrientationAngles(listener, deg * 1000.0f, 0, 0);
  checkErr(res);
}
