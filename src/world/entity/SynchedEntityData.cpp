...
SynchedEntityData::SynchedEntityData() : _isDirty(false) {}
...
void SynchedEntityData::assignValues(DataList *items) {
  for (DataList::const_iterator it = items->begin(); it != items->end(); ++it) {
    ...
    if (jt != itemsById.end()) {
      switch (item->getType()) {
      case TYPE_BYTE:
        set(jt->second, ((DataItem2<TypeChar> *)item)->data);
        break;
      case TYPE_SHORT:
        set(jt->second, ((DataItem2<TypeShort> *)item)->data);
        break;
      case TYPE_INT:
        set(jt->second, ((DataItem2<TypeInt> *)item)->data);
        break;
      case TYPE_FLOAT:
        set(jt->second, ((DataItem2<TypeFloat> *)item)->data);
        break;
      case TYPE_STRING:
        set(jt->second, ((DataItem2<std::string> *)item)->data);
        break;
      case TYPE_ITEMINSTANCE:
        set(jt->second, ((DataItem2<ItemInstance> *)item)->data);
        break;
      case TYPE_POS:
        set(jt->second, ((DataItem2<Pos> *)item)->data);
        break;
      default:
