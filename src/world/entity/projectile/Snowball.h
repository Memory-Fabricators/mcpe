...
public:
  Snowball(Level *level) : super(level) {
    entityRendererId = ER_SNOWBALL_RENDERER;
  }

  Snowball(Level *level, Mob *mob) : super(level, mob) {
    entityRendererId = ER_SNOWBALL_RENDERER;
  }

  Snowball(Level *level, float x, float y, float z) : super(level, x, y, z) {
    entityRendererId = ER_SNOWBALL_RENDERER;
  }

  virtual int getEntityTypeId() const { return EntityTypes::IdSnowball; }
