...
public:
  ThrownEgg(Level *level) : super(level) {
    entityRendererId = ER_THROWNEGG_RENDERER;
  }

  ThrownEgg(Level *level, Mob *mob) : super(level, mob) {
    entityRendererId = ER_THROWNEGG_RENDERER;
  }

  ThrownEgg(Level *level, float x, float y, float z) : super(level, x, y, z) {
    entityRendererId = ER_THROWNEGG_RENDERER;
  }

  virtual int getEntityTypeId() const { return EntityTypes::IdThrownEgg; }
