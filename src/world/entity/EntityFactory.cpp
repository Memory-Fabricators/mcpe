...
Entity *EntityFactory::loadEntity(CompoundTag *tag, Level *level) {
  ...
  if (e) {
    ...
    if (e->isItemEntity()) {
      ...
      if (item.isNull() || item.id < 0 || item.id >= Item::MAX_ITEMS ||
          !Item::items[item.id]) {
        delete e;
        e = NULL;
      }
