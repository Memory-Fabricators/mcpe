...
class DataItem {
public:
  DataItem(int type, int id) : type(type), id(id), dirty(true) {}
  virtual ~DataItem() {}
...
public:
  DataItem2(int type, int id, const T &data) : super(type, id), data(data) {}
  void setFlag(int flag) { data = data | (1 << flag); }
  void clearFlag(int flag) { data = data & ~(1 << flag); }
  bool getFlag(int flag) const { return (data & (1 << flag)) != 0; }
  T data;
};
...
#define DataItemClassFactory(TypeName)	DataItemClassFactory2(TypeName,
TypeName)
...
public:
  ...
  template <class T> void define(int idd, const T &value) {
  ...
  template <class T> void set(int id, const T &value) {
    set(itemsById[id], value);
  }
  template <class T> void set(DataItem *dataItem, const T &value) {
    if (!dataItem) {
  ...
  template <class T> void setFlag(int id, int flag) {
  ...
  template <class T> void clearFlag(int id, int flag) {
    DataItem2<T> *item = (DataItem2<T> *)itemsById[id];
    bool oldFlag = item->getFlag(flag);
    item->clearFlag(flag);
    if (item->getFlag(flag) != oldFlag) {
  ...
  template <class T> bool getFlag(int id, int flag) {
