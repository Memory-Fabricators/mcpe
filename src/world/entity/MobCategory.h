...
public:
  ...
  int getBaseClassId() const { return _baseClassId; }

  int getMaxInstancesPerChunk() const { return _max; }

  int getMaxInstancesPerLevel() const { return _maxPerLevel; }

  const Material *getSpawnPositionMaterial() const {
    return _spawnPositionMaterial;
  }

  bool isFriendly() const { return _isFriendly; }
...
private:
  ...
  MobCategory(int baseClassId, int max, int maxPerLevel, bool isFriendly)
      : _baseClassId(baseClassId), _max(max), _maxPerLevel(maxPerLevel),
        _spawnPositionMaterial(NULL), _isFriendly(isFriendly) {
    // LOGI("Creating a Mobcategory: %d, %d, %p, %d    ", _baseClassId, _max,
    // _spawnPositionMaterial, _isFriendly);
  }
