...
public:
  HurtByTargetGoal(Monster *mob, bool alertSameType)
      : super(mob, 16, false), alertSameType(alertSameType) {
    setRequiredControlFlags(TargetGoal::TargetFlag);
  }

  bool canUse() { return canAttack(mob->getLastHurtByMob(), true); }
  ...
  void start() {
    ...
    //         if (alertSameType) {
    //             List<Entity> nearby =
    //             mob.level->getEntitiesOfClass(mob.getClass(),
    //             AABB.newTemp(mob.x, mob.y, mob.z, mob.x + 1, mob.y + 1, mob.z
    //             + 1).grow(within, 4, within)); for (Entity* ent : nearby) {
    //                 Mob other = (Mob) ent;
