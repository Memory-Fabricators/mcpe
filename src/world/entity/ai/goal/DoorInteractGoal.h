...
public:
  DoorInteractGoal(Monster *mob) : mob(mob) {}
  ...
  bool canUse() {
    ...
    for (int i = 0; i < Mth::Min(path->getIndex() + 2, path->getSize()); ++i) {
      ...
      doorY = n->y + 1;
      doorZ = n->z;
      if (mob->distanceToSqr((float)doorX, mob->y, (float)doorZ) > 1.5f * 1.5f)
        continue;
  ...
  bool canContinueToUse() { return !passed; }
