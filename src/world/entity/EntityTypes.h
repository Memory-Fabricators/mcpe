...
public:
        static bool isEntityType(int type, int entityType) {
                return getBaseType(type) == BaseTypes::Entity? getSubType(1,
type) == entityType : false;
        }
        ...
        static bool isSameTypeUpTo(int a, int b, int hierarchy) {
                if (hierarchy < 0) hierarchy = Mth::Min(getHierarchyLevel(a),
getHierarchyLevel(b)); for (int i = 0; i <= hierarchy; ++i) if (getSubType(i, a)
!= getSubType(i, b)) return false; return true;
        }
