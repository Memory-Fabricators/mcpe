...
public:
  ChunkCache(Level *level_, ChunkStorage *storage_, ChunkSource *source_)
      : xLast(-999999999), zLast(-999999999), last(NULL), level(level_),
        storage(storage_), source(source_) {
    isChunkCache = true;
  ...
  bool hasChunk(int x, int z) {
    // with a fixed world size, chunks outside the fitting area are always
    // available (emptyChunks)
    if (!fits(x, z))
      return true;
  ...
  LevelChunk *getChunk(int x, int z) {
    ...
    if (!hasChunk(x, z)) {
      ...
      if (updateLights) {
        ...
        // level->updateLight(LightLayer::Sky, x * 16, 0, z * 16, x * 16 + 15,
        // 128, z * 16 + 15); level->updateLight(LightLayer::Block, x * 16, 0, z
        // * 16, x * 16 + 15, 128, z * 16 + 15);
      }
      ...
      if (!chunks[slot]->terrainPopulated && hasChunk(x + 1, z + 1) &&
  ...
  std::string gatherStats() { return "ChunkCache: 1024"; }
