...
public:
  LadderTile(int id, int tex) : super(id, tex, Material::decoration) {}
  ...
  AABB *getAABB(Level *level, int x, int y, int z) {
    ...
    if (dir == 2)
      this->setShape(0, 0, 1 - r, 1, 1, 1);
    if (dir == 3)
      this->setShape(0, 0, 0, 1, 1, r);
    if (dir == 4)
      this->setShape(1 - r, 0, 0, 1, 1, 1);
    if (dir == 5)
      this->setShape(0, 0, 0, r, 1, 1);
  ...
  int getRenderShape() { return Tile::SHAPE_LADDER; }
  int getRenderLayer() { return Tile::RENDERLAYER_ALPHATEST; }
