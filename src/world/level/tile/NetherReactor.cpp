...
NetherReactor::NetherReactor(int id, int tex, const Material *material)
...
bool NetherReactor::use(Level *level, int x, int y, int z, Player *player) {
  ...
  if (!canSpawnStartNetherReactor(level, x, y, z, player))
    return false;
  player->displayClientMessage("Active!");
  NetherReactorTileEntity *reactor =
      static_cast<NetherReactorTileEntity *>(level->getTileEntity(x, y, z));
  if (reactor != NULL) {
