...
public:
  ...
  bool mayPick(int data, bool liquid) { return liquid && data == 0; }
  ...
  int getRenderShape() { return Tile::SHAPE_WATER; }
  ...
  int getRenderLayer() {
    return (material == Material::water) ? Tile::RENDERLAYER_BLEND
                                         : Tile::RENDERLAYER_OPAQUE;
  }
  ...
  void animateTick(Level *level, int x, int y, int z, Random *random) {
    if (material == Material::water && random->nextInt(64) == 0) {
      ...
      if (d > 0 && d < 8) {
        // level->playSound(x + 0.5f, y + 0.5f, z + 0.5f, "liquid.water",
        // random.nextFloat() * 0.25f + 0.75f, random.nextFloat() * 1.0f +
        // 0.5f);
      }
    }
    if (material == Material::lava) {
...
protected:
  ...
  void fizz(Level *level, int x, int y, int z) {
    // level->playSound(x + 0.5f, y + 0.5f, z + 0.5f, "random.fizz", 0.5f, 2.6f
...
private:
  void updateLiquid(Level *level, int x, int y, int z) {
    ...
    if (material == Material::lava) {
      ...
      if (water || level->getMaterial(x + 1, y, z) == Material::water)
        water = true;
      //            if (water || level->getMaterial(x, y - 1, z) ==
      //            Material::water) water = true; // NM
  ...
  Vec3 getFlow(LevelSource *level, int x, int y, int z) {
    ...
    if (level->getData(x, y, z) >= 8) {
      bool ok = false;
      if (ok || shouldRenderFace(level, x, y, z - 1, 2))
        ok = true;
      if (ok || shouldRenderFace(level, x, y, z + 1, 3))
        ok = true;
      if (ok || shouldRenderFace(level, x - 1, y, z, 4))
        ok = true;
      if (ok || shouldRenderFace(level, x + 1, y, z, 5))
        ok = true;
      if (ok || shouldRenderFace(level, x, y + 1, z - 1, 2))
        ok = true;
      if (ok || shouldRenderFace(level, x, y + 1, z + 1, 3))
        ok = true;
      if (ok || shouldRenderFace(level, x - 1, y + 1, z, 4))
        ok = true;
      if (ok || shouldRenderFace(level, x + 1, y + 1, z, 5))
        ok = true;
      if (ok)
