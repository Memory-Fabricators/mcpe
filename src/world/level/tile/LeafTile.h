...
public:
  ...
  LeafTile(int id, int tex)
      : super(id, tex, Material::leaves, false), oTex(tex), checkBuffer(NULL) {
    setTicking(true);
  }
  ...
  int getRenderLayer() {
    return isSolidRender() ? Tile::RENDERLAYER_OPAQUE
                           : Tile::RENDERLAYER_ALPHATEST;
  }
  ...
  int getResource(int data, Random *random) { return Tile::sapling->id; }
  ...
  bool isSolidRender() { return !allowSame; }

  int getTexture(int face, int data) {
    if ((data & LEAF_TYPE_MASK) == EVERGREEN_LEAF) {
      return (this == Tile::leaves) ? tex + 5 * 16 : tex - 16;
    }
    return tex;
  }
...
protected:
  int getSpawnResourcesAuxValue(int data) { return data & LEAF_TYPE_MASK; }
