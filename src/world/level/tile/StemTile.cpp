#include "StemTile.h"
StemTile::StemTile(int id, Tile *fruit) : super(id, 15 + 6 * 16), fruit(fruit) {
  setTicking(true);
  float ss = 0.125f;
  this->setShape(0.5f - ss, 0, 0.5f - ss, 0.5f + ss, 0.25f, 0.5f + ss);
}

bool StemTile::mayPlaceOn(int tile) { return tile == Tile::farmland->id; }
  ...
  if (level->getRawBrightness(x, y + 1, z) >= Level::MAX_BRIGHTNESS - 6) {
    ...
    if (random->nextInt((int)(25 / growthSpeed) + 1) == 0) {
      ...
      if (age < 7) {
        ...
        if (level->getTile(x - 1, y, z) == fruit->id)
          return;
        if (level->getTile(x + 1, y, z) == fruit->id)
          return;
        if (level->getTile(x, y, z - 1) == fruit->id)
          return;
        if (level->getTile(x, y, z + 1) == fruit->id)
          return;
        for (int a = 0; a < 4; ++a) {
...
int StemTile::getTexture(int face, int data) { return tex; }
...
void StemTile::spawnResources(Level *level, int x, int y, int z, int data,
  ...
  if (fruit == Tile::melon)
    seed = Item::seeds_melon;
  for (int i = 0; i < 3; i++) {
    if (level->random.nextInt(5 * 3) > data)
      continue;
    popResource(level, x, y, z, ItemInstance(seed));
  }
}

int StemTile::getResource(int data, Random *random) { return -1; }
