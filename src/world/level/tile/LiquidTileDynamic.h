...
public:
  ...
  void tick(Level *level, int x, int y, int z, Random *random) {
    ...
    int dropOff = 1;
    if (material == Material::lava && !level->dimension->ultraWarm)
      dropOff = 2;
    ...
    if (depth > 0) {
      ...
      if (maxCount >= 2 && material == Material::water) {
        // Only spread spring if it's on top of an existing spring, or on top of
        // solid ground.
    ...
    if (canSpreadTo(level, x, y - 1, z)) {
      ...
      if (neighbor >= 8)
        return;
      if (spreads[0])
        trySpreadTo(level, x - 1, y, z, neighbor);
      if (spreads[1])
        trySpreadTo(level, x + 1, y, z, neighbor);
      if (spreads[2])
        trySpreadTo(level, x, y, z - 1, neighbor);
      if (spreads[3])
        trySpreadTo(level, x, y, z + 1, neighbor);
    }
  }
...
private:
  ...
  bool isWaterBlocking(Level *level, int x, int y, int z) {
    ...
    if (t == 0)
      return false;
    if (t == Tile::door_wood->id /*|| t == Tile::door_iron->id */ ||
        t == Tile::sign->id || t == Tile::ladder->id || t == Tile::reeds->id) {
      return true;
    }
