...
bool ExternalFileLevelStorage::readPlayerData(const std::string &filename,
  ...
  do {
    ...
    if (version == 1) {
      ...
      if (pos.z < 0.5f)
        pos.z = 0.5f;
      if (pos.x > (LEVEL_WIDTH - 0.5f))
        pos.x = LEVEL_WIDTH - 0.5f;
      if (pos.z > (LEVEL_DEPTH - 0.5f))
        pos.z = LEVEL_DEPTH - 0.5f;
      if (pos.y < 0)
        pos.y = 64;
...
void ExternalFileLevelStorage::tick() {
    ...
    for (int z = 0; z < CHUNK_CACHE_WIDTH; z++) {
      for (int x = 0; x < CHUNK_CACHE_WIDTH; x++) {
        ...
        if (chunk && chunk->unsaved) {
          ...
          chunk->unsaved =
              false; // not actually saved, but in our working list at least
        }
      }
    }
  ...
  if (tickCount - lastSavedEntitiesTick >
