...
void ExternalFileLevelStorageSource::addLevelSummaryIfExists(
    LevelSummaryList &dest, const char *dirName) {
...
void ExternalFileLevelStorageSource::deleteLevel(const std::string &levelId) {
  ...
  if (!DeleteDirectory(
          path)) { // If we couldn't delete whole folder, try to remove chunks..
...
void ExternalFileLevelStorageSource::renameLevel(
    const std::string &oldLevelId_, const std::string &newLevelName_) {
...
bool ExternalFileLevelStorageSource::isNewLevelIdAcceptable(
