...
public:
  TreeFeature(bool doUpdate, int trunkType = TreeTile::NORMAL_TRUNK)
      : super(doUpdate), trunkType(trunkType) {}
  ...
  bool place(Level *level, Random *random, int x, int y, int z) {
    ...
    for (int yy = y - 3 + treeHeight; yy <= y + treeHeight; yy++) {
      ...
      for (int xx = x - offs; xx <= x + offs; xx++) {
        ...
        for (int zz = z - offs; zz <= z + offs; zz++) {
          int zo = zz - (z);
          if (std::abs(xo) == offs && std::abs(zo) == offs &&
              (random->nextInt(2) == 0 || yo == 0))
            continue;
          if (!Tile::solid[level->getTile(xx, yy, zz)])
            placeBlock(level, xx, yy, zz, ((Tile *)Tile::leaves)->id);
        }
      }
    }
    for (int hh = 0; hh < treeHeight; hh++) {
      ...
      if (t == 0 || t == ((Tile *)Tile::leaves)->id)
        placeBlock(level, x, y + hh, z, Tile::treeTrunk->id, trunkType);
    }
    return true;
  }
