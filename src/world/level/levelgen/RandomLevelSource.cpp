...
RandomLevelSource::RandomLevelSource(Level *level, long seed, int version,
    ...
    : random(seed), level(level), lperlinNoise1(&random, 16),
      ...
      spawnMobs(spawnMobs), pnr(NULL), ar(NULL), br(NULL), sr(NULL), dr(NULL),
      fi(NULL), fis(NULL)
// biomes(NULL)
{
  for (int i = 0; i < 32; ++i)
  ...
  for (int xc = 0; xc < xChunks; xc++) {
    for (int zc = 0; zc < xChunks; zc++) {
      for (int yc = 0; yc < 128 / CHUNK_HEIGHT; yc++) {
        ...
        float s0a =
            (buffer[((xc + 0) * zSize + (zc + 0)) * ySize + (yc + 1)] - s0) *
            yStep;
        float s1a =
            (buffer[((xc + 0) * zSize + (zc + 1)) * ySize + (yc + 1)] - s1) *
            yStep;
        float s2a =
            (buffer[((xc + 1) * zSize + (zc + 0)) * ySize + (yc + 1)] - s2) *
            yStep;
        float s3a =
            (buffer[((xc + 1) * zSize + (zc + 1)) * ySize + (yc + 1)] - s3) *
            yStep;
          ...
          for (int x = 0; x < CHUNK_WIDTH; x++) {
            ...
            for (int z = 0; z < CHUNK_WIDTH; z++) {
              ...
              if (yc * CHUNK_HEIGHT + y < waterHeight) {
                if (temp < SNOW_CUTOFF &&
                    yc * CHUNK_HEIGHT + y >= waterHeight - 1) {
                  tileId = Tile::ice->id;
                } else {
...
void RandomLevelSource::buildSurfaces(int xOffs, int zOffs,
  ...
  for (int x = 0; x < 16; x++) {
    for (int z = 0; z < 16; z++) {
      ...
      int runDepth =
          (int)(depthBuffer[x + z * 16] / 3 + 3 + random.nextFloat() * 0.25f);
...
void RandomLevelSource::postProcess(ChunkSource *parent, int xt, int zt) {
  ...
  for (int x = xo + 8; x < xo + 8 + 16; x++)
    for (int z = zo + 8; z < zo + 8 + 16; z++) {
      ...
      if (temp < SNOW_CUTOFF) {
        if (y > 0 && y < 128 && level->isEmptyTile(x, y, z) &&
...
LevelChunk *RandomLevelSource::getChunk(int xOffs, int zOffs) {
  ...
  Biome **biomes = level->getBiomeSource()->getBiomeBlock(
