...
Path::Path() : nodes(NULL), length(0), index(0), id(++p) {}

Path::~Path() { destroy(); }

bool Path::isEmpty() const { return length == 0 || nodes == NULL; }
...
Node *Path::currentPos() { return nodes[index]; }

Vec3 Path::currentPos(Entity *e) const { return getPos(e, index); }

void Path::next() { index++; }

void Path::setSize(int size) { length = size; }
...
Node *Path::get(int i) const { return nodes[i]; }
...
bool Path::endsInXZ(const Vec3 &pos) const {
  ...
  if (end == NULL)
    return false;
  return end->x == Mth::floor(pos.x) && end->z == Mth::floor(pos.z);
}
