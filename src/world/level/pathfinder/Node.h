...
public:
  Node(int x = 0, int y = 0, int z = 0)
      : x(x), y(y), z(z), heapIdx(-1), g(0), h(0), f(0), cameFrom(NULL),
        closed(false), hash(createHash(x, y, z)) {}
...
public:
  ...
  bool operator<(const TNode &rhs) const {
    if (node->z != rhs.node->z)
      return node->z < rhs.node->z;
    if (node->x != rhs.node->x)
      return node->x < rhs.node->x;
