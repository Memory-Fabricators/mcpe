...
SimpleFoodData::SimpleFoodData() : foodLevel(0) {}

void SimpleFoodData::eat(int food) {
  foodLevel = Mth::Min(food + foodLevel, FoodConstants::MAX_FOOD);
}

void SimpleFoodData::eat(FoodItem *item) { eat(item->getNutrition()); }
...
void SimpleFoodData::tick(Player *player) {
  if (foodLevel > 0 && player->isHurt()) {
    ...
    if (!level->isClientSide) {
            ...
            SetHealthPacket packet(SetHealthPacket::HEALTH_MODIFY_OFFSET -
    healAmount); level->raknetInstance->send(packet);
    }
    */
  }
}
