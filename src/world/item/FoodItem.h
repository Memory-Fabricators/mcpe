...
public:
  FoodItem(int id, int nutrition, bool isMeat, float saturationMod = 0.6f)
      : super(id), nutrition(nutrition), _isMeat(isMeat),
  ...
  int getUseDuration(ItemInstance *itemInstance) { return EAT_DURATION; }

  UseAnim::UseAnimation getUseAnimation() { return UseAnim::eat; }
...
float getSaturationModifier() {
  ...
  */
  ...
  bool isMeat() { return _isMeat; }

  /*
FoodItem* setEatEffect(int id, int durationInSecods, int amplifier, float
effectProbability) { effectId = id; effectDurationSeconds = durationInSecods;
