...
public:
  ...
  int getUseDuration(ItemInstance *itemInstance) { return 20 * 60 * 60; }
  ...
  // /*public*/ ItemInstance use(ItemInstance instance, Level* level, Player*
  // player) {
  //            level->playSound(player, "random.bow", 1.0f, 1 /
  //            (random.nextFloat() * 0.4f + 0.8f)); if (!level->isOnline)
