...
bool DyePowderItem::useOn(ItemInstance *itemInstance, Player *player,
  ...
  if (itemInstance->getAuxValue() == WHITE) {
    ...
    if (tile == Tile::sapling->id) {
      ...
      if (!level->isOnline) {
      if (((Mushroom) Tile::tiles[tile]).growTree(level, x, y, z,
      level->random)) { itemInstance.count--;
      }
      }
      return true;
      }*/
      ...
      if (!level->isClientSide) {
        ...
        for (int j = 0; j < 32; j++) {
          ...
          for (int i = 0; i < j / 16; i++) {
            ...
            if (level->getTile(xx, yy - 1, zz) != Tile::grass->id ||
          ...
          if (level->getTile(xx, yy, zz) == 0) {
            ...
            level->setTileAndData(xx, yy, zz, Tile::tallgrass.id,
            TallGrass.TALL_GRASS); } else*/
