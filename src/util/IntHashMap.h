...
template <class V> class IntHashMap {
...
public:
  IntHashMap() : loadFactor(0.75f) {
    threshold = (int)(DEFAULT_INITIAL_CAPACITY * DEFAULT_LOAD_FACTOR);
    table = new Entry[DEFAULT_INITIAL_CAPACITY];
  }

  ~IntHashMap() { delete[] table; }
  ...
  /*public*/ bool containsKey(int key) { return getEntry(key) != NULL; }
  ...
  /*private*/
  template <class V> class Entry<V> {
    const int key;
    V value;
    Entry<V> *next;
    const int hash;
    ...
    const int hashCode() { return hash(key); }
