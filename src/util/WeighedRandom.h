...
public:
  ...
  public:
    ...
    WeighedRandomItem() : randomWeight(-128) {}

    WeighedRandomItem(int randomWeight) : randomWeight(randomWeight) {}

    bool isValid() { return randomWeight >= 0; }
  };

  template <typename T> static int getTotalWeight(const T &items) {
    int totalWeight = 0;
  ...
  //     WeighedRandomItem[] items, int totalWeight) {
  //
  //         if (totalWeight <= 0) {
