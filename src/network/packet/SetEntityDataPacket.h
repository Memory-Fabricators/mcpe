...
public:
  SetEntityDataPacket() : deletePackedItems(false) {}

  SetEntityDataPacket(int id, SynchedEntityData &entityData)
      : id(id), deletePackedItems(false), packedItems(entityData.packDirty()) {}
  ...
  void write(RakNet::BitStream *bitStream) {
    bitStream->Write(static_cast<RakNet::MessageID>(
        static_cast<int>(ID_USER_PACKET_ENUM) + PACKET_SETENTITYDATA));
    bitStream->Write(id);
  ...
  SynchedEntityData::DataList &getUnpackedData() { return packedItems; }
