...
public:
  MoveEntityPacket() : hasRot(false) {}

  // PACKET_MOVEENTITY is unknown and undefined (and not used)
  void write(RakNet::BitStream *bitStream) {
    bitStream->Write(static_cast<RakNet::MessageID>(
        static_cast<int>(ID_USER_PACKET_ENUM) + PACKET_MOVEENTITY));
  }
...
public:
  MoveEntityPacket_PosRot() { hasRot = true; }

  // MoveEntityPacket_PosRot(int entityId, float x, float y, float z, float
  // yRot, float xRot) { 	set(entityId, x, y, z, yRot, xRot);
  // }
  ...
  void write(RakNet::BitStream *bitStream) {
    bitStream->Write(static_cast<RakNet::MessageID>(
        static_cast<int>(ID_USER_PACKET_ENUM) + PACKET_MOVEENTITY_POSROT));
