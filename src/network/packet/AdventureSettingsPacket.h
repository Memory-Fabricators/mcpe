...
public:
  ...
  AdventureSettingsPacket(const AdventureSettings &settings) : flags(0) {
  ...
  void write(RakNet::BitStream *bitStream) {
    bitStream->Write(static_cast<RakNet::MessageID>(
        static_cast<int>(ID_USER_PACKET_ENUM) + PACKET_ADVENTURESETTINGS));
    bitStream->Write(flags);
  }

  void read(RakNet::BitStream *bitStream) { bitStream->Read(flags); }
