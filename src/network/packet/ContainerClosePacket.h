...
public:
  ...
  ContainerClosePacket(int containerId) : containerId(containerId) {}

  void read(RakNet::BitStream *bitStream) { bitStream->Read(containerId); }

  void write(RakNet::BitStream *bitStream) {
    bitStream->Write(static_cast<RakNet::MessageID>(
        static_cast<int>(ID_USER_PACKET_ENUM) + PACKET_CONTAINERCLOSE));
