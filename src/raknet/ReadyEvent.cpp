...
int RakNet::ReadyEvent::RemoteSystemCompBySystemAddress(
...
bool ReadyEvent::IsEventSet(int eventId) {
  ...
  if (objectExists) {
    return readyEventNodeList[eventIndex]->eventStatus == ID_READY_EVENT_SET ||
           readyEventNodeList[eventIndex]->eventStatus ==
...
bool ReadyEvent::AddToWaitList(int eventId, SystemAddress address) {
  ...
  // Don't do this, otherwise if we are trying to start a 3 player game, it will
  // not allow the 3rd player to hit ready if the first two players have already
  // done so
  // if (IsLocked(eventIndex))
  //	return false; // Not in the list, but event is already completed, or is
  //starting to complete, and adding more waiters would fail this.
  ...
  if (address == UNASSIGNED_SYSTEM_ADDRESS) {
    for (i = 0; i < rakPeerInterface->GetMaximumNumberOfPeers(); i++) {
      SystemAddress internalAddress =
          rakPeerInterface->GetSystemAddressFromIndex(i);
...
ReadyEventSystemStatus ReadyEvent::GetReadyStatus(int eventId,
  ...
  if (objectExists) {
    ...
    unsigned systemIndex =
        ren->systemList.GetIndexFromKey(address, &objectExists);
    if (objectExists == false)
...
void ReadyEvent::OnReadyEventPacketUpdate(Packet *packet) {
  ...
  if (objectExists) {
    ...
    bool systemExists;
    unsigned systemIndex =
        ren->systemList.GetIndexFromKey(packet->systemAddress, &systemExists);
    if (systemExists) {
...
void ReadyEvent::OnReadyEventQuery(Packet *packet) {
  ...
  if (objectExists) {
    ...
    // Force the non-default send, because our initial send may have arrived at
    // a system that didn't yet create the ready event
    if (objectExists)
...
void ReadyEvent::UpdateReadyStatus(unsigned eventIndex) {
  ...
  if (ren->eventStatus == ID_READY_EVENT_SET) {
    // If you are set, and no other systems are ID_READY_EVENT_UNSET, then
    // change your status to ID_READY_EVENT_ALL_SET
    anyUnset = false;
    for (i = 0; i < ren->systemList.Size(); i++) {
    ...
    // If you are all set, and any systems are ID_READY_EVENT_UNSET, then change
    // your status to ID_READY_EVENT_SET
    anyUnset = false;
    for (i = 0; i < ren->systemList.Size(); i++) {
      if (ren->systemList[i].lastReceivedStatus == ID_READY_EVENT_UNSET) {
        anyUnset = true;
...
void ReadyEvent::SendReadyUpdate(unsigned eventIndex, unsigned systemIndex,
  ...
  ReadyEventNode *ren = readyEventNodeList[eventIndex];
  RakNet::BitStream bs;
  // I do this rather than write true or false, so users that do not use
  // BitStreams can still read the data
  if ((ren->eventStatus != ren->systemList[systemIndex].lastSentStatus) ||
    ...
    SendUnified(&bs, HIGH_PRIORITY, RELIABLE_ORDERED, channel,
                ren->systemList[systemIndex].systemAddress, false);
...
void ReadyEvent::PushCompletionPacket(unsigned eventId) {
  ...
  // Not necessary
  /*
  // Pass a packet to the user that we are now completed, as setting ourselves
  to signaled was the last thing being waited on Packet *p =
  AllocatePacketUnified(sizeof(MessageID)+sizeof(int)); RakNet::BitStream
  bs(p->data, sizeof(MessageID)+sizeof(int), false); bs.SetWriteOffset(0);
