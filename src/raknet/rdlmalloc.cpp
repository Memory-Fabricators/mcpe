...
#define INITIAL_LOCK(s)                                                        \
  (!InitializeCriticalSectionAndSpinCount((s), 0x80000000 | 4000))
...
#else /* ONLY_MSPACES */
#if MSPACES
#define internal_malloc(m, b) (m == gm) ? rdlmalloc(b) : rak_mspace_malloc(m, b)
#define internal_free(m, mem)                                                  \
  if (m == gm)                                                                 \
    rdlfree(mem);                                                              \
  else                                                                         \
    rak_mspace_free(m, mem);
#else /* MSPACES */
...
static void *internal_realloc(mstate m, void *oldmem, size_t bytes) {
  ...
  if (!PREACTION(m)) {
    ...
    if (RTCHECK(ok_address(m, oldp) && ok_inuse(oldp) && ok_next(oldp, next) &&
                ok_pinuse(next))) {
      ...
      if ((((size_t)(mem)) % alignment) != 0) { /* misaligned */
        ...
        */
        char *br = (char *)mem2chunk(
            (size_t)(((size_t)(mem + alignment - SIZE_T_ONE)) & -alignment));
...
size_t rdlmalloc_footprint(void) { return gm->footprint; }

size_t dlmalloc_max_footprint(void) { return gm->max_footprint; }

#if !NO_MALLINFO
struct mallinfo rdlmallinfo(void) { return internal_mallinfo(gm); }
#endif /* NO_MALLINFO */

void rdlmalloc_stats() { internal_malloc_stats(gm); }
...
* new DL_PLATFORM_WIN32 sbrk, mmap, munmap, lock code from
<Walter@GeNeSys-e.de>. Thanks also to Andreas Mueller <a.mueller at
paradatec.de>, and Anonymous.
...
* Added Several DL_PLATFORM_WIN32 cleanups from Martin C. Fong <mcfong at
yahoo.com>
