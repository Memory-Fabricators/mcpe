...
/// Allocates CloudQueryRow and the row data. Override to use derived classes or
...
struct RAK_DLL_EXPORT CloudKey {
  ...
  /// Identifies the primary key. This is intended to be a major category, such
  /// as the name of the application Must be non-empty
  RakNet::RakString primaryKey;

  /// Identifies the secondary key. This is intended to be a subcategory
  /// enumeration, such as PLAYER_LIST or RUNNING_SCORES
...
struct RAK_DLL_EXPORT CloudQuery {
  ...
  /// This query is run on uploads from all clients, and those that match the
  /// combination of primaryKey and secondaryKey are potentially returned If you
  /// pass more than one key at a time, the results are concatenated so if you
  /// need to differentiate between queries then send two different queries
  DataStructures::List<CloudKey> keys;

  /// If limiting the number of rows to return, this is the starting offset into
  /// the list. Has no effect unless maxRowsToReturn is > 0
  uint32_t startingRowIndex;

  /// Maximum number of rows to return. Actual number may still be less than
  /// this. Pass 0 to mean no-limit.
  uint32_t maxRowsToReturn;

  /// If true, automatically get updates as the results returned to you change.
  /// Unsubscribe with CloudMemoryClient::Unsubscribe()
...
struct RAK_DLL_EXPORT CloudQueryResult {
  ...
  /// Results returned from query. If there were multiple keys in
  /// CloudQuery::keys then see resultKeyIndices
  DataStructures::List<CloudQueryRow *> rowsReturned;

  /// If there were multiple keys in CloudQuery::keys, then each key is
  /// processed in order and the result concatenated to rowsReturned The
  /// starting index of each query is written to resultKeyIndices For example,
  /// if CloudQuery::keys had 4 keys, returning 3 rows, 0, rows, 5 rows, and 12
  /// rows then resultKeyIndices would be 0, 3, 3, 8
  DataStructures::List<uint32_t> resultKeyIndices;

  /// Whatever was passed to CloudClient::Get() as
  /// CloudQuery::subscribeToResults
  bool subscribeToResults;
