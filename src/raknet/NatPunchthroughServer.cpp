...
int RakNet::NatPunchthroughServer::NatPunchthroughUserComp(
    const RakNetGUID &key, User *const &data) {
  if (key < data->guid)
    return -1;
  if (key > data->guid)
    return 1;
      ...
      for (j = 0; j < user->connectionAttempts.Size(); j++) {
        ...
        if (connectionAttempt->sender == user) {
                                                    ...
                                                    // sometimes false positives
          {
            ...
            // expecting it due to external code In that case, recipient would
            // never get any response if the sender dropped
            outgoingBs.Reset();
            outgoingBs.Write((MessageID)ID_NAT_TARGET_UNRESPONSIVE);
            ...
            if (natPunchthroughServerDebugInterface) {
              ...
              // 8/01/09 Fixed bug where this was after
              // DeleteConnectionAttempt()
...
void NatPunchthroughServer::OnClosedConnection(
  ...
  if (objectExists) {
    ...
    for (connectionAttemptIndex = 0;
      ...
      // due to external code In that case, recipient would never get any
      // response if the sender dropped
  ...
  for (i = 0; i < users.Size(); i++) {
    ...
    gprIndex = users[i]->groupPunchthroughRequests.GetIndexFromKey(
        rakNetGUID, &objectExists);
    if (objectExists) {
      //			printf("DEBUG %i\n", __LINE__);
...
void NatPunchthroughServer::StartPunchthroughForUser(User *user) {
  ...
  for (i = 0; i < user->connectionAttempts.Size(); i++) {
    ...
    if (otherUser->isReady) {
      ...
      // 4/29/09 Write sessionID so we don't use returned port for a system we
      // don't want
      outgoingBs.Write(connectionAttempt->sessionId);
      rakPeerInterface->Send(&outgoingBs, HIGH_PRIORITY, RELIABLE_ORDERED, 0,
