...
/// \brief Contains ConsoleServer , used to plugin to your game to accept remote
/// console-based connections
///
...
/// \brief The main entry point for the server portion of your remote console
/// application support.
/// \details ConsoleServer takes one TransportInterface and one or more
/// CommandParserInterface (s) The TransportInterface will be used to send data
/// between the server and the client.  The connecting client must support the
/// protocol used by your derivation of TransportInterface . TelnetTransport and
/// RakNetTransport are two such derivations . When a command is sent by a
/// remote console, it will be processed by your implementations of
...
public:
  ...
  /// \brief Call this with a derivation of TransportInterface so that the
  /// console server can send and receive commands
  ...
  /// \brief Remove an implementation of CommandParserInterface previously added
  ...
  /// \param[in] _prompt Null-terminated string of the prompt to use. If you
  /// want a newline, be sure to use /r/n
