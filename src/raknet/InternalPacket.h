/// \file
/// \brief \b [Internal] A class which stores a user message, and all
/// information associated with sending and receiving that message.
///
...
/// This is the counter used for holding packet numbers, so we can detect
/// duplicate packets.  It should be large enough that if the variables
...
/// This is the counter used for holding ordered packet numbers, so we can
/// detect out-of-order packets.  It should be large enough that if the
/// variables were to wrap, the newly wrapped values would no longer be in use.
...
struct InternalPacketFixedSizeTransmissionHeader {
  /// A unique numerical identifier given to this user message. Used to identify
  /// reliable messages on the network
  MessageNumberType reliableMessageNumber;
  /// The ID used as identification for ordering messages. Also included in
  /// sequenced messages
  OrderingIndexType orderingIndex;
  // Used only with sequenced messages
  OrderingIndexType sequencingIndex;
  /// What ordering channel this packet is on, if the reliability type uses
  /// ordering channels
  unsigned char orderingChannel;
  /// The ID of the split packet, if we have split packets.  This is the maximum
  /// number of split messages we can send simultaneously per connection.
...
/// Used in InternalPacket when pointing to sharedDataBlock, rather than
/// allocating itself
...
struct InternalPacket : public InternalPacketFixedSizeTransmissionHeader {
  ...
  /// Has this message number been assigned yet?  We don't assign until the
  /// message is actually sent. This fixes a bug where pre-determining message
  /// numbers and then sending a message on a different channel creates a huge
  /// gap. This causes performance problems and causes those messages to
  /// timeout.
  bool messageNumberAssigned;
  /// Was this packet number used this update to track windowing drops or
  /// increases?  Each packet number is only used once per update.
  ...
  /// Buffer is a pointer to the actual data, assuming this packet has data at
  /// all
  unsigned char *data;
  ...
  enum AllocationScheme {
    ...
    /// data points to a larger block of data, where the larger block is
    /// reference counted. internalPacketRefCountedData is used in this case
    REF_COUNTED,

    /// If allocation scheme is STACK, data points to stackData and should not
    /// be deallocated This is only used when sending. Received packets are
  ...
  /// If the reliability type requires a receipt, then return this number with
  /// it
