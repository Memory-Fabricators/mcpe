...
/// \details This will connect RakPeer to all connecting peers, and all peers
/// the connecting peer knows about.
///
...
/// \brief Fully connected mesh plugin, revision 2
/// \details This will connect RakPeer to all connecting peers, and all peers
/// the connecting peer knows about.<BR> It will also calculate which system has
/// been running longest, to find out who should be host, if you need one system
/// to act as a host
/// \pre You must also install the ConnectionGraph2 plugin in order to use
...
public:
  ...
  /// When the message ID_REMOTE_NEW_INCOMING_CONNECTION arrives, we try to
  /// connect to that system If \a attemptConnection is false, you can manually
  /// connect to all systems listed in ID_REMOTE_NEW_INCOMING_CONNECTION with
  /// ConnectToRemoteNewIncomingConnections()
  /// \note This will not work on any console. It will also not work if NAT
  /// punchthrough is needed. Generally, this should be false and you should
  /// connect manually. It is here for legacy reasons.
  /// \param[in] attemptConnection If true, we try to connect to any systems we
  /// are notified about with ID_REMOTE_NEW_INCOMING_CONNECTION, which comes
  ...
  /// \details Will return UNASSIGNED_RAKNET_GUID if we are not connected to
  /// anyone, or if we are connected and are calculating the host If
  /// includeCalculating is true, will return the estimated calculated host as
  /// long as the calculation is nearly complete includeCalculating should be
  /// true if you are taking action based on another system becoming host,
  /// because not all host calculations may complete at the exact same time
  /// \sa ConnectionGraph2::GetLowestAveragePingSystem() . If you need one
  /// system in the peer to peer group to relay data, have the host call this
  /// function after host migration, and use that system
  ...
  /// \return System address of whichever system is host. Always returns
  /// something, even though it may be our own system.
  ...
  /// \param[in] includeCalculating If true, and we are currently calculating a
  /// new host, return the new host if the calculation is nearly complete
  /// \return If our system is host
  bool IsConnectedHost(void) const;

  /// \brief Automatically add new connections to the fully connected mesh.
  /// Each remote system that you want to check should be added as a
  /// participant, either through SetAutoparticipateConnections() or by calling
  /// this function
  ...
  /// Call this to reset the running time of the host just before
  /// joining/creating a game room for networking
  void ResetHostCalculation(void);

  /// \brief if SetAutoparticipateConnections() is called with false, then you
  /// need to use AddParticipant before these systems will be added to the mesh
  /// FullyConnectedMesh2 will track who is the who host among a fully connected
  /// mesh of participants Each remote system that you want to check should be
  /// added as a participant, either through SetAutoparticipateConnections() or
  ...
  /// \param[in] connectionPassword Password passed to
  /// RakPeerInterface::Connect()
  /// \param[in] connectionPasswordLength Password length passed to
  /// RakPeerInterface::Connect()
...
protected:
  ...
  // Used to track how long RakNet has been running. This is so we know who has
  ...
  // totalConnectionCount is roughly maintained across all systems, and
  // increments by 1 each time a new system connects to the mesh It is always
  // kept at the highest known value It is used as the high 4 bytes for new
  // FCMGuids. This causes newer values of FCM2Guid to be higher than lower
  // values. The lowest value is the host.
  unsigned int totalConnectionCount;

  // Our own ourFCMGuid. Starts at unassigned (0). Assigned once we send
  // ID_FCM2_REQUEST_FCMGUID and get back ID_FCM2_RESPOND_CONNECTION_COUNT
  FCM2Guid ourFCMGuid;
...
Send to that system a request for their fcmGuid, totalConnectionCount. Inform
startupTime. else Send to that system a request for their fcmGuid. Inform total
connection count, our fcmGuid

OnRequestGuid()
if (sender by guid is not a participant)
{
        // They added us as a participant, but we didn't add them. This can be
caused by lag where both participants are not added at the same time.
