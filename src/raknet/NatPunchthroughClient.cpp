...
bool NatPunchthroughClient::OpenNATGroup(
    DataStructures::List<RakNetGUID> destinationSystems,
    const SystemAddress &facilitator) {
  ConnectionState cs = rakPeerInterface->GetConnectionState(facilitator);
  if (cs != IS_CONNECTED)
    return false;
...
void NatPunchthroughClient::Update(void) {
  ...
  if (sp.nextActionTime && sp.nextActionTime < time) {
    ...
    if (sp.testMode == SendPing::TESTING_INTERNAL_IPS) {
      SendOutOfBand(sp.internalIds[sp.attemptCount],
      ...
      if (sp.attemptCount >= pc.MAXIMUM_NUMBER_OF_INTERNAL_IDS_TO_CHECK) {
        ...
        if (pc.INTERNAL_IP_WAIT_AFTER_ATTEMPTS > 0) {
          ...
          // TESTING: Try sending to unused ports on the remote system to
          // reserve our own ports while not getting banned
      ...
      // TESTING: Try sending to unused ports on the remote system to reserve
      // our own ports while not getting banned
      sp.testMode = SendPing::SEND_WITH_TTL;
      // sp.testMode=SendPing::TESTING_EXTERNAL_IPS_FACILITATOR_PORT_TO_FACILITATOR_PORT;
      sp.attemptCount = 0;
      ...
      /*
      If I understand correctly, getting a datagram on a particular address that
      was not previously used causes that the port for that address to not be
      used for when a reply would have otherwise been sent back from that
      address.

      PHASE 1:
      1. System 1 and 2 send to each other.
      2. Due to latency, system 1 sends first using the server port.
      3. System 2 gets the datagram and no longer uses the server port. Instead,
      it replies using port 1024.
      4. System 1 gets the reply. The source port is wrong, so it is rejected.

      To put it another way, if a router gets a datagram on a port that was not
      previously used, it will not reply on that port. However, if it doesn't
      reply on that port, the message will not be accepted by the remote system.
      */

      // Send to unused port. We do not want the message to arrive, just to open
      // our router's table
      SystemAddress sa = sp.targetAddress;
      int ttlSendIndex;
      for (ttlSendIndex = 0; ttlSendIndex <= pc.MAX_PREDICTIVE_PORT_RANGE;
           ttlSendIndex++) {
      ...
      // Wait 250 milliseconds for next stage. The delay is so that even with
      // timing errors both systems send out the datagram with TTL before either
      // sends a real one
      sp.testMode =
      ...
      if (sp.attemptCount > pc.MAX_PREDICTIVE_PORT_RANGE) {
        // From 1024 disabled, never helps as I've seen, but slows down the
        // process by half
      ...
      if (sp.attemptCount > pc.MAX_PREDICTIVE_PORT_RANGE) {
        if (natPunchthroughDebugInterface) {
          ...
          natPunchthroughDebugInterface->OnClientMessage(
              RakNet::RakString("Likely bidirectional punchthrough failure to "
      ...
      UpdateGroupPunchOnNatResult(sp.facilitator, sp.targetGuid,
                                  sp.targetAddress, 1);
    }
    ...
    if (sp.testMode == SendPing::PUNCHING_FIXED_PORT) {
      ...
      if (++sp.retryCount >= sp.punchingFixedPortAttempts) {
        if (natPunchthroughDebugInterface) {
          ...
          sp.targetGuid.ToString(guidString);
          natPunchthroughDebugInterface->OnClientMessage(
              RakNet::RakString("Likely unidirectional punchthrough failure to "
                                "guid %s, system address %s.",
                                guidString, ipAddressString));
        }
...
void NatPunchthroughClient::OnPunchthroughFailure(void) {
  if (pc.retryOnFailure == false) {
    if (natPunchthroughDebugInterface) {
      ...
      sp.targetGuid.ToString(guidString);
      natPunchthroughDebugInterface->OnClientMessage(
          RakNet::RakString("Failed punchthrough once. Returning failure to "
                            "guid %s, system address %s to user.",
                            guidString, ipAddressString));
    }
  ...
  for (i = 0; i < failedAttemptList.Size(); i++) {
    if (failedAttemptList[i].guid == sp.targetGuid) {
      if (natPunchthroughDebugInterface) {
        ...
        sp.targetGuid.ToString(guidString);
        natPunchthroughDebugInterface->OnClientMessage(
            RakNet::RakString("Failed punchthrough twice. Returning failure to "
                              "guid %s, system address %s to user.",
                              guidString, ipAddressString));
      }
  ...
  if (rakPeerInterface->GetConnectionState(sp.facilitator) != IS_CONNECTED) {
    if (natPunchthroughDebugInterface) {
      ...
      sp.targetGuid.ToString(guidString);
      natPunchthroughDebugInterface->OnClientMessage(
          RakNet::RakString("Not connected to facilitator, so cannot retry "
                            "punchthrough after first failure. Returning "
                            "failure onj guid %s, system address %s to user.",
                            guidString, ipAddressString));
    }
  ...
  if (natPunchthroughDebugInterface) {
    ...
    sp.targetGuid.ToString(guidString);
    natPunchthroughDebugInterface->OnClientMessage(
        RakNet::RakString("First punchthrough failure on guid %s, system "
                          "address %s. Reattempting.",
                          guidString, ipAddressString));
  }
  ...
  // If we are the sender, try again, immediately if possible, else added to the
  // queue on the faciltiator
  if (sp.weAreSender)
...
PluginReceiveResult NatPunchthroughClient::OnReceive(Packet *packet) {
  ...
  case ID_OUT_OF_BAND_INTERNAL:
    if (packet->length >= 2 &&
      ...
      // sp.targetGuid==packet->guid is because the internal IP addresses
      // reported may include loopbacks not reported by RakPeer::IsLocalIP()
  ...
  case ID_NAT_TARGET_UNRESPONSIVE: {
      ...
      if (failedAttemptList[i].guid == targetGuid) {
        if (natPunchthroughDebugInterface) {
          ...
          natPunchthroughDebugInterface->OnClientMessage(RakNet::RakString(
              "Punchthrough retry to guid %s failed due to %s.", guidString,
              reason));
        }
        ...
        // Don't need to return failed, the other messages indicate failure
        // anyway
        /*
    ...
    if (natPunchthroughDebugInterface) {
      ...
      targetGuid.ToString(guidString);
      natPunchthroughDebugInterface->OnClientMessage(
          RakNet::RakString("Punchthrough attempt to guid %s failed due to %s.",
                            guidString, reason));
    }
    ...
    {
            SendPunchthrough(pendingOpenNAT[0].destination,
    pendingOpenNAT[0].facilitator); sp.nextActionTime=0;
    }
    */
  } break;
  case ID_TIMESTAMP:
...
void NatPunchthroughClient::OnConnectAtTime(Packet *packet) {
  ...
  */
  ...
  if (pc.MAXIMUM_NUMBER_OF_INTERNAL_IDS_TO_CHECK > 0) {
    ...
    // TESTING: Try sending to unused ports on the remote system to reserve our
    // own ports while not getting banned
    sp.testMode = SendPing::SEND_WITH_TTL;
    // sp.testMode=SendPing::TESTING_EXTERNAL_IPS_FACILITATOR_PORT_TO_FACILITATOR_PORT;
  }
...
void NatPunchthroughClient::SendOutOfBand(SystemAddress sa, MessageID oobId) {
  ...
  if (natPunchthroughDebugInterface) {
    ...
    if (oobId == ID_NAT_ESTABLISH_UNIDIRECTIONAL)
      natPunchthroughDebugInterface->OnClientMessage(
          RakNet::RakString("Sent OOB ID_NAT_ESTABLISH_UNIDIRECTIONAL to guid "
                            "%s, system address %s.",
                            guidString, ipAddressString));
    else
      natPunchthroughDebugInterface->OnClientMessage(
          RakNet::RakString("Sent OOB ID_NAT_ESTABLISH_BIDIRECTIONAL to guid "
                            "%s, system address %s.",
                            guidString, ipAddressString));
  }
}
void NatPunchthroughClient::OnNewConnection(const SystemAddress &systemAddress,
  ...
  // Try to track new port mappings on the router. Not reliable, but better than
  // nothing.
...
void NatPunchthroughClient::OnClosedConnection(
  ...
  if (sp.facilitator == systemAddress) {
    // If we lose the connection to the facilitator, all previous failures not
    // currently in progress are returned as such
    unsigned int i = 0;
...
/*
unsigned int NatPunchthroughClient::GetPendingOpenNATIndex(RakNetGUID
destination, const SystemAddress &facilitator)
{
        unsigned int i;
...
NatPunchthroughClient::GetPunchthroughConfiguration(void) {
  return &pc;
}
...
void NatPunchthroughClient::IncrementExternalAttemptCount(RakNet::Time time,
                                                          RakNet::Time delta) {
...
void NatPunchthroughClient::UpdateGroupPunchOnNatResult(
    SystemAddress facilitator, RakNetGUID targetSystem,
    SystemAddress targetSystemAddress, int result) {
