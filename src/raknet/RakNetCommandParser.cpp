...
RakNetCommandParser::RakNetCommandParser() {
  RegisterCommand(4, "Startup",
                  "( unsigned short maxConnections, unsigned short localPort, "
  ...
  RegisterCommand(1, "SetMaximumIncomingConnections",
  ...
  RegisterCommand(1, "GetIndexFromSystemAddress",
                  "( const SystemAddress systemAddress );");
  RegisterCommand(1, "GetSystemAddressFromIndex", "( int index );");
  RegisterCommand(2, "AddToBanList",
                  "( const char *IP, RakNet::TimeMS milliseconds=0 );");
  ...
  RegisterCommand(1, "GetAveragePing",
                  "( const SystemAddress systemAddress );");
  RegisterCommand(1, "GetLastPing",
                  "( const SystemAddress systemAddress ) const;");
  RegisterCommand(1, "GetLowestPing",
                  "( const SystemAddress systemAddress ) const;");
  ...
  RegisterCommand(2, "SetTimeoutTime",
...
bool RakNetCommandParser::OnCommand(const char *command, unsigned numParameters,
  ...
  if (strcmp(command, "Startup") == 0) {
    ...
    ReturnResult(peer->Startup((unsigned short)atoi(parameterList[0]),
                               &socketDescriptor, 1),
                 command, transport, systemAddress);
  } else if (strcmp(command, "InitializeSecurity") == 0) {
    ReturnResult(peer->InitializeSecurity(parameterList[0], parameterList[1]),
                 command, transport, systemAddress);
    ...
    ReturnResult(peer->IsInSecurityExceptionList(parameterList[1]), command,
    ...
    ReturnResult(peer->GetMaximumIncomingConnections(), command, transport,
                 systemAddress);
  } else if (strcmp(command, "Connect") == 0) {
    ReturnResult(peer->Connect(parameterList[0],
                               (unsigned short)atoi(parameterList[1]),
                               parameterList[2], atoi(parameterList[3])) ==
                     RakNet::CONNECTION_ATTEMPT_STARTED,
                 command, transport, systemAddress);
  } else if (strcmp(command, "Disconnect") == 0) {
    peer->Shutdown(atoi(parameterList[0]),
                   (unsigned char)atoi(parameterList[1]));
    ReturnResult(command, transport, systemAddress);
    ...
    peer->CloseConnection(SystemAddress(parameterList[0]),
                          atoi(parameterList[1]) != 0,
                          (unsigned char)atoi(parameterList[2]));
    ReturnResult(command, transport, systemAddress);
  } else if (strcmp(command, "GetConnectionState") == 0) {
    ReturnResult((int)peer->GetConnectionState(SystemAddress(parameterList[0])),
                 command, transport, systemAddress);
    ...
    ReturnResult(peer->GetSystemAddressFromIndex(atoi(parameterList[0])),
                 command, transport, systemAddress);
    ...
    ReturnResult(peer->IsBanned(parameterList[0]), command, transport,
                 systemAddress);
    ...
    ReturnResult(peer->GetAveragePing(SystemAddress(parameterList[0])), command,
                 transport, systemAddress);
  } else if (strcmp(command, "GetLastPing") == 0) {
    ReturnResult(peer->GetLastPing(SystemAddress(parameterList[0])), command,
                 transport, systemAddress);
  } else if (strcmp(command, "GetLowestPing") == 0) {
    ReturnResult(peer->GetLowestPing(SystemAddress(parameterList[0])), command,
                 transport, systemAddress);
    ...
    ReturnResult(peer->GetExternalID(SystemAddress(parameterList[0])), command,
                 transport, systemAddress);
  } else if (strcmp(command, "SetTimeoutTime") == 0) {
    peer->SetTimeoutTime(atoi(parameterList[0]),
                         SystemAddress(parameterList[1]));
    ReturnResult(command, transport, systemAddress);
  }
  /*
  ...
  */
    ...
    ReturnResult(peer->GetMTUSize(UNASSIGNED_SYSTEM_ADDRESS), command,
                 transport, systemAddress);
  } else if (strcmp(command, "GetNumberOfAddresses") == 0) {
    ReturnResult((int)peer->GetNumberOfAddresses(), command, transport,
                 systemAddress);
  } else if (strcmp(command, "GetLocalIP") == 0) {
    ReturnResult((char *)peer->GetLocalIP(atoi(parameterList[0])), command,
                 transport, systemAddress);
    ...
    peer->AdvertiseSystem(parameterList[0],
                          (unsigned short)atoi(parameterList[1]),
                          parameterList[2], atoi(parameterList[3]));
    ReturnResult(command, transport, systemAddress);
