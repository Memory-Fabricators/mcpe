...
void FileListTransfer::Send(FileList *fileList,
  ...
  // Write the chunk header, which contains the frequency table, the total
  // number of files, and the total number of bytes
...
bool FileListTransfer::DecodeFile(Packet *packet,
  ...
  if (isTheFileAndIsNotDownloadProgress) {
    ...
    // Remote system is sending a complete file, but the file is large enough
    // that we get ID_PROGRESS_NOTIFICATION from the transport layer
    fileListReceiver->downloadHandler->OnFileProgress(&fps);
  }
...
void FileListTransfer::OnReferencePush(Packet *packet,
  ...
  if (finished) {
    ...
    if (isTheFileAndIsNotDownloadProgress) {
      ...
      // This is a download progress notification for a file chunk using
      // incremental read interface We don't have all the data for this chunk
      // yet
...
int SendIRIToAddressCB(FileListTransfer::ThreadData threadData,
  ...
  *returnOutput = false;
  ...
  // Was previously using GetStatistics to get outgoing buffer size, but TCP
  // with UnifiedSend doesn't have this
  ...
  for (ftpIndex = 0;
    ...
    if (ftpr->systemAddress == systemAddress) {
      ...
      bytesRead = ftp->incrementalReadInterface->GetFilePart(
          ftp->fileListNode.fullPathToFile, ftp->currentOffset, ftp->chunkSize,
          buff, ftp->fileListNode.context);
      ...
      while (done && ftp->currentOffset == 0 && ftpr->filesToPush.Size() >= 2 &&
        ...
        bytesRead = ftp->incrementalReadInterface->GetFilePart(
            ftp->fileListNode.fullPathToFile, ftp->currentOffset,
