...
Packet *TelnetTransport::Receive(void) {
  ...
  */
  ...
  // Get this guy's cursor buffer.  This is real bullcrap that I have to do
  // this.
  unsigned i;
  ...
  // Hack - I don't know what the hell this is about but cursor keys send 3
  // characters at a time.  I can block these
  // Up=27,91,65
  // Down=27,91,66
  // Right=27,91,67
  // Left=27,91,68
...
SystemAddress TelnetTransport::HasNewIncomingConnection(void) {
  ...
  // 03/16/06 Can't force the stupid windows telnet to use line mode or local
  // echo so now I have to track all the remote players and their input buffer
...
bool TelnetTransport::ReassembleLine(
    TelnetTransport::TelnetClient *remoteClient, unsigned char c) {
  if (c == '\n') {
