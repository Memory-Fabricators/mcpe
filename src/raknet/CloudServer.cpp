  ...
  if (time > nextGetRequestsCheck) {
    ...
    while (i < getRequests.Size()) {
      if (time - getRequests[i]->requestStartTime > 3000) {
        // Remote server is not responding, just send back data with whoever did
        // respond
...
void CloudServer::OnPostRequest(Packet *packet) {
  ...
  if (keyDataListExists == false) {
    ...
    if (forceAddress != UNASSIGNED_SYSTEM_ADDRESS) {
      ...
      cloudData->serverSystemAddress.SetPort(
...
void CloudServer::OnClosedConnection(
  ...
  if (objectExists) {
    // Update remoteServerResponses by removing this server and sending the
    // response if it is now complete
      ...
      if (keyDataRepositoryExists) {
        ...
        if (keyDataExists) {
          ...
          if (cloudData->IsUnused()) {
            ...
            if (cloudDataList->IsNotUploaded()) {
              // Tell other servers that this key is no longer uploaded, so they
              // do not request it from us
              RemoveUploadedKeyFromServers(cloudDataList->key);
            }
  ...
  for (queryIndex = 0;
    ...
    if (dataRepositoryExists) {
      ...
      if (cloudDataList->uploaderCount > 0) {
        // Return all keyData that was uploaded by specificSystems, or all if
        // not specified
...
void CloudServer::UnsubscribeFromKey(
  ...
  if (specificSystems.Size() == 0) {
    // Remove global subscriber. If returns false, have to remove specific
    // subscribers
