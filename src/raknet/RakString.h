...
public:
  ...
  // Lets you modify the string. Do not make the string longer - however, you
  // can make it shorter, or change the contents. Pointer is only valid in the
  // scope of RakString itself
  ...
  /// Searches the string for the content specified in stringToFind and returns
  /// the position of the first occurrence in the string. Search only includes
  /// characters on or after position pos, ignoring any possible occurrences in
  /// previous locations.
  ...
  /// Sets a copy of a substring of str as the new content. The substring is the
  /// portion of str that begins at the character position pos and takes up to n
  /// characters (it takes less than n if the end of str is reached before).
  /// \param[in] str The string to copy in
  /// \param[in] pos The position on str to start the copy
  /// \param[in] n How many chars to copy
  /// \return Returns the string, note that the current string is set to that
  /// value as well
  ...
  /// Does the given IP address match the IP address encoded into this string,
  ...
  /// \param[in] writeLanguageId encode the languageId variable in the stream.
  /// If false, 0 is assumed, and DeserializeCompressed will not look for this
  /// variable in the stream (saves bandwidth)
  ...
  /// \param[in] bs Bitstream to serialize from
  /// \param[in] readLanguageId If true, looks for the variable langaugeId in
  /// the data stream. Must match what was passed to SerializeCompressed
  /// \return true if the deserialization was successful
  /// \pre StringCompressor::AddReference must have been called to instantiate
  /// the class (Happens automatically from RakPeer::Startup())
