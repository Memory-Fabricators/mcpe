/// \file
/// \brief This will write all incoming and outgoing network messages to the
/// local console screen.  See derived functions for other outputs
///
...
/// This will write all incoming and outgoing messages to the console window, or
/// to a file if you override it and give it this functionality.
...
public:
  ...
  // Translate the supplied parameters into an output line - overloaded version
  // that takes a MessageIdentifier and translates it into a string (numeric or
  // textual representation based on printId); this calls the second version
  // which takes a const char* argument for the messageIdentifier
  virtual void
  FormatLine(char *into, const char *dir, const char *type,
  ...
  /// Events on low level sends and receives.  These functions may be called
  ...
  // Print or hide acks (clears up the screen not to print them but is worse for
  // debugging)
