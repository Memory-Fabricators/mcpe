...
/// \brief Connection graph plugin, version 2. Tells new systems about existing
/// and new connections
///
...
/// \details Sends ID_REMOTE_CONNECTION_LOST,
/// ID_REMOTE_DISCONNECTION_NOTIFICATION, ID_REMOTE_NEW_INCOMING_CONNECTION<BR>
/// All identifiers are followed by SystemAddress, then RakNetGUID
/// Also stores the list for you, which you can access with
...
public:
  ...
  /// \brief Given a remote system identified by RakNetGUID, return the list of
  /// SystemAddresses and RakNetGUIDs they are connected to
  /// \param[in] remoteSystemGuid Which system we are referring to. This only
  /// works for remote systems, not ourselves.
  /// \param[out] saOut A preallocated array to hold the output list of
  /// SystemAddress. Can be 0 if you don't care.
  /// \param[out] guidOut A preallocated array to hold the output list of
  /// RakNetGUID. Can be 0 if you don't care.
  /// \param[in,out] outLength On input, the size of \a saOut and \a guidOut. On
  /// output, modified to reflect the number of elements actually written
  ...
  /// Returns the average ping between two systems in the connection graph.
  /// Returns -1 if no connection exists between those systems
  ...
  /// If you need one system in the peer to peer group to relay data, have the
  /// FullyConnectedMesh2 host call this function after host migration, and use
  /// that system
  RakNetGUID GetLowestAveragePingSystem(void) const;

  /// \brief If called with false, then new connections are only added to the
  /// connection graph when you call ProcessNewConnection();
  /// \details This is useful if you want to perform validation before
  /// connecting a system to a mesh, or if you want a submesh (for example a
  /// server cloud)
  /// \param[in] b True to automatically call ProcessNewConnection() on any new
  ...
  /// \return Value passed to SetAutoProcessNewConnections(), or the default of
  /// true if it was never called
  bool GetAutoProcessNewConnections(void) const;

  /// \brief If you call SetAutoProcessNewConnections(false);, then you will
  /// need to manually call ProcessNewConnection() on new connections
  /// \details On ID_NEW_INCOMING_CONNECTION or ID_CONNECTION_REQUEST_ACCEPTED,
  /// adds that system to the graph Do not call ProcessNewConnection() manually
  /// otherwise
