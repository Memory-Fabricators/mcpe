/// \file
/// \brief All the message identifiers used by RakNet.  Message identifiers
/// comprise the first byte of any message.
///
...
enum DefaultMessageIDTypes {
  ...
  /// C2S: Initial query: Header(1), OfflineMesageID(16), Protocol number(1),
  /// Pad(toMTU), sent with no fragment set. If protocol fails on server,
  ...
  /// C2S: Header(1), OfflineMesageID(16), Cookie(4, if HasSecurity is true on
  /// the server), clientSupportsSecurity(1 bit), handshakeChallenge (if has
  /// security on both server and client), remoteBindingAddress(6), MTU(2),
  /// client GUID(8) Connection slot allocated if cookie is valid, server is not
  /// full, GUID and IP not already in use.
  ID_OPEN_CONNECTION_REQUEST_2,
  /// S2C: Header(1), OfflineMesageID(16), server GUID(8), mtu(2), doSecurity(1
  /// bit), handshakeAnswer (if do security is true)
  ...
  /// RakPeer - We passed a public key to RakPeerInterface::Connect(), but the
  /// other system did not have security turned on
  ID_OUR_SYSTEM_REQUIRES_SECURITY,
  /// RakPeer - Wrong public key passed to RakPeerInterface::Connect()
  ID_PUBLIC_KEY_MISMATCH,
  /// RakPeer - Same as ID_ADVERTISE_SYSTEM, but intended for internal use
  /// rather than being passed to the user. Second byte indicates type. Used
  /// currently for NAT punchthrough for receiver port advertisement. See
  /// ID_NAT_ADVERTISE_RECIPIENT_PORT
  ID_OUT_OF_BAND_INTERNAL,
  /// If RakPeerInterface::Send() is called where PacketReliability contains
  /// _WITH_ACK_RECEIPT, then on a later call to RakPeerInterface::Receive() you
  /// will get ID_SND_RECEIPT_ACKED or ID_SND_RECEIPT_LOSS. The message will be
  /// 5 bytes long, and bytes 1-4 inclusive will contain a number in native
  /// order containing a number that identifies this message. This number will
  ...
  /// RakPeerInterface::Receive() you will get ID_SND_RECEIPT_ACKED or
  /// ID_SND_RECEIPT_LOSS. The message will be 5 bytes long, and bytes 1-4
  /// inclusive will contain a number in native order containing a number that
  /// identifies this message. This number will be returned by
  /// RakPeerInterface::Send() or RakPeerInterface::SendList().
  /// ID_SND_RECEIPT_LOSS means that an ack for the message did not arrive (it
  /// may or may not have been delivered, probably not). On disconnect or
  /// shutdown, you will not get ID_SND_RECEIPT_LOSS for unsent messages, you
  ...
  /// RakPeer - In a client/server environment, our connection request to the
  /// server has been accepted.
  ID_CONNECTION_REQUEST_ACCEPTED,
  /// RakPeer - Sent to the player when a connection request cannot be completed
  /// due to inability to connect.
  ID_CONNECTION_ATTEMPT_FAILED,
  /// RakPeer - Sent a connect request to a system we are currently connected
  /// to.
  ...
  /// RakPeer - The system specified in Packet::systemAddress has disconnected
  /// from us.  For the client, this would mean the server has shutdown.
  ...
  ID_INVALID_PASSWORD,
  // RAKNET_PROTOCOL_VERSION in RakNetVersion.h does not match on the remote
  // system what we have on our system This means the two systems cannot
  // communicate. The 2nd byte of the message contains the value of
  // RAKNET_PROTOCOL_VERSION for the remote system
  ID_INCOMPATIBLE_PROTOCOL_VERSION,
  // Means that this IP address connected recently, and can't connect again as a
  // security measure. See
  /// RakPeer::SetLimitIPConnectionFrequency()
  ID_IP_RECENTLY_CONNECTED,
  /// RakPeer - The sizeof(RakNetTime) bytes following this byte represent a
  /// value which is automatically modified by the difference in system times
  /// between the sender and the recipient. Requires that you call
  /// SetOccasionalPing.
  ID_TIMESTAMP,
  /// RakPeer - Pong from an unconnected system.  First byte is
  /// ID_UNCONNECTED_PONG, second sizeof(RakNet::TimeMS) bytes is the ping,
  /// following bytes is system specific enumeration data.
  /// Read using bitstreams
  ID_UNCONNECTED_PONG,
  /// RakPeer - Inform a remote system of our IP/Port. On the recipient, all
  /// data past ID_ADVERTISE_SYSTEM is whatever was passed to the data parameter
  ID_ADVERTISE_SYSTEM,
  // RakPeer - Downloading a large message. Format is ID_DOWNLOAD_PROGRESS
  // (MessageID), partCount (unsigned int),
  ///  partTotal (unsigned int),
  /// partLength (unsigned int), first part data (length <= MAX_MTU_SIZE). See
  /// the three parameters partCount, partTotal
  ///  and partLength in OnFileProgress in FileListTransferCBInterface.h
  ID_DOWNLOAD_PROGRESS,

  /// ConnectionGraph2 plugin - In a client/server environment, a client other
  /// than ourselves has disconnected gracefully.
  ///   Packet::systemAddress is modified to reflect the systemAddress of this
  ///   client.
  ID_REMOTE_DISCONNECTION_NOTIFICATION,
  /// ConnectionGraph2 plugin - In a client/server environment, a client other
  /// than ourselves has been forcefully dropped.
  ///  Packet::systemAddress is modified to reflect the systemAddress of this
  ///  client.
  ID_REMOTE_CONNECTION_LOST,
  /// ConnectionGraph2 plugin: Bytes 1-4 = count. for (count items) contains
  /// {SystemAddress, RakNetGUID, 2 byte ping}
  ...
  /// Autopatcher plugin - Get a list of files that have changed since a certain
  /// date
  ...
  /// Autopatcher plugin - Returned to the user: An error from the database
  /// repository for the autopatcher.
  ...
  /// NATPunchthrough plugin: Destination system is not connected to the server.
  /// Bytes starting at offset 1 contains the
  ...
  /// ID_NAT_GET_MOST_RECENT_PORT. Possibly the plugin is not installed.
  ...
  /// NATPunchthrough plugin: The server lost the connection to the destination
  /// system while setting up punchthrough.
  ///  Possibly the plugin is not installed. Bytes starting at offset 1 contains
  ///  the RakNetGUID  destination field of NatPunchthroughClient::OpenNAT().
  ID_NAT_CONNECTION_TO_TARGET_LOST,
  /// NATPunchthrough plugin: This punchthrough is already in progress. Possibly
  /// the plugin is not installed.
  ///  Bytes starting at offset 1 contains the RakNetGUID destination field of
  ///  NatPunchthroughClient::OpenNAT().
  ID_NAT_ALREADY_IN_PROGRESS,
  /// NATPunchthrough plugin: This message is generated on the local system, and
  /// does not come from the network.
  ...
  /// NATPunchthrough plugin: Punchthrough succeeded. See packet::systemAddress
  /// and packet::guid. Byte 1 contains 1 if you are the sender,
  ...
  /// packet::guid contains the facilitator field of
  /// NatPunchthroughClient::OpenNAT() Data format starts at byte 1:<BR> (char)
  /// passedSystemsCount,<BR> (RakNetGuid, SystemAddress) (for
  /// passedSystemsCount),<BR> (char) ignoredSystemsCount (caused by
  /// ID_NAT_TARGET_NOT_CONNECTED, ID_NAT_CONNECTION_TO_TARGET_LOST,
  /// ID_NAT_TARGET_UNRESPONSIVE),<BR> RakNetGuid (for ignoredSystemsCount),<BR>
  ...
  /// packet::guid contains the facilitator field of
  /// NatPunchthroughClient::OpenNAT() See ID_NAT_GROUP_PUNCH_FAILED for data
  /// format
  ...
  /// ReadyEvent plugin - Request of ready event state - used for pulling data
  /// when newly connecting
  ...
  /// FileListTransfer transferring large files in chunks that are read only
  /// when needed, to save memory
  ...
  /// RakNetGuid. The old host can be read out using BitStream->Read(RakNetGuid)
  /// starting on byte 1 This is not returned until connected to a remote system
  /// If the oldHost is UNASSIGNED_RAKNET_GUID, then this is the first time the
  ...
  /// Cannot switch to the desired team because it is full. However, if someone
  /// on that team leaves, you will
  ///  get ID_TEAM_BALANCER_SET_TEAM later. Byte 1 contains the team you
  ///  requested to join. Following bytes contain NetworkID of which member.
  ID_TEAM_BALANCER_REQUESTED_TEAM_CHANGE_PENDING,
  /// Cannot switch to the desired team because all teams are locked. However,
  /// if someone on that team leaves,
  ///  you will get ID_TEAM_BALANCER_SET_TEAM later. Byte 1 contains the team
  ///  you requested to join.
  ID_TEAM_BALANCER_TEAMS_LOCKED,
  /// Team balancer plugin informing you of your team. Byte 1 contains the team
  /// you requested to join. Following bytes contain NetworkID of which member.
  ...
  /// The password we used to challenge the other system passed, meaning the
  /// other system has called TwoWayAuthentication::AddPassword() with the same
  /// password we passed to TwoWayAuthentication::Challenge() You can read the
  /// identifier used to challenge as follows: RakNet::BitStream
  /// bs(packet->data, packet->length, false);
  ...
  /// TwoWayAuthentication::Challenge(), and the challenge failed. If the other
  /// system must pass the challenge to stay connected, you should call
  /// RakPeer::CloseConnection() to terminate the connection to the other
  /// system.
  ID_TWO_WAY_AUTHENTICATION_INCOMING_CHALLENGE_FAILURE,
  /// The other system did not add the password we used to
  /// TwoWayAuthentication::AddPassword() You can read the identifier used to
  /// challenge as follows: RakNet::BitStream bs(packet->data, packet->length,
  /// false); bs.IgnoreBytes(sizeof(MessageID)); RakNet::RakString password;
  /// bs.Read(password);
  ID_TWO_WAY_AUTHENTICATION_OUTGOING_CHALLENGE_FAILURE,
  /// The other system did not respond within a timeout threshhold. Either the
  /// other system is not running the plugin or the other system was blocking on
  /// some operation for a long time. You can read the identifier used to
  /// challenge as follows: RakNet::BitStream bs(packet->data, packet->length,
  /// false); bs.IgnoreBytes(sizeof(MessageID)); RakNet::RakString password;
  /// bs.Read(password);
