...
/// \brief A plugin to provide a simple way to compress and incrementally send
/// the files in the FileList structure.
///
...
/// \brief A plugin to provide a simple way to compress and incrementally send
/// the files in the FileList structure.
/// \details Similar to the DirectoryDeltaTransfer plugin, except that it
/// doesn't send deltas based on pre-existing files or actually write the files
/// to disk.
///
/// Usage:
/// Call SetupReceive to allow one file set to arrive.  The value returned by
/// FileListTransfer::SetupReceive() is the setID that is allowed. It's up to
/// you to transmit this value to the other system, along with information
/// indicating what kind of files you want to get. The other system should then
/// prepare a FileList and call FileListTransfer::Send(), passing the return
/// value of FileListTransfer::SetupReceive() as the \a setID parameter to
...
public:
  ...
  /// \brief Optionally start worker threads when using
  /// _incrementalReadInterface for the Send() operation
  ...
  /// \param[in] deleteHandler True to delete the handler when it is no longer
  /// needed.  False to not do so.
  /// \param[in] allowedSender Which system to allow files from.
  /// \return A set ID value, which should be passed as the \a setID value to
  /// the Send() call on the other system.  This value will be returned in the
  ...
  /// \param[in] fileList A list of files.  The data contained in FileList::data
  /// will be sent incrementally and compressed among all files in the set
  /// \param[in] rakPeer The instance of RakNet to use to send the message. Pass
  /// 0 to use the instance the plugin is attached to
  /// \param[in] recipient The address of the system to send to
  /// \param[in] setID The return value of SetupReceive() which was previously
  /// called on \a recipient
  ...
  /// _incrementalReadInterface will be used to read the file in chunks of size
  /// \a chunkSize
