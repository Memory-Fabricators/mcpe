...
enum RNSPerSecondMetrics {
  ...
  /// USER_MESSAGE_BYTES_PUSHED. A message would be pushed, but not yet sent,
...
struct RAK_DLL_EXPORT RakNetStatistics {
  ...
  /// This value should be true if you send more data per second than your
  /// bandwidth capacity
  bool isLimitedByCongestionControl;

  /// If \a isLimitedByCongestionControl is true, what is the limit, in bytes
  /// per second?
  ...
  /// If \a isLimitedByOutgoingBandwidthLimit is true, what is the limit, in
  /// bytes per second?
  ...
  /// rising over time, you are exceeding the bandwidth capacity. See
  /// BPSLimitByCongestionControl
  unsigned int messagesInResendBuffer;

  /// How many bytes are waiting in the resend buffer. See also
  /// messagesInResendBuffer
  uint64_t bytesInResendBuffer;

  /// Over the last second, what was our packetloss? This number will range from
  /// 0.0 (for none) to 1.0 (for 100%)
