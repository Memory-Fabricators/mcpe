...
ReplicaManager3::~ReplicaManager3() {
  if (autoDestroyConnections) {
    // Clear() calls DeallocConnection(), which is pure virtual and cannot be
...
RakNet::Connection_RM3 *ReplicaManager3::PopConnection(unsigned int index) {
  ...
  // This might be wrong, I am relying on the variable creatingSystemGuid which
  // is transmitted automatically from the first system to reference the object.
  // However, if an object changes owners then it is not going to be returned
  // here, and therefore QueryActionOnPopConnection() will not be called for the
  // new owner.
...
void ReplicaManager3::Reference(RakNet::Replica3 *replica3) {
  ...
  if (index != (unsigned int)-1) {
    ...
    for (pushIdx = 0; pushIdx < connectionList.Size(); pushIdx++) {
      Connection_RM3::ConstructionMode constructionMode =
          connectionList[pushIdx]->QueryConstructionMode();
      if (constructionMode == Connection_RM3::QUERY_REPLICA_FOR_CONSTRUCTION ||
          constructionMode ==
              Connection_RM3::QUERY_REPLICA_FOR_CONSTRUCTION_AND_DESTRUCTION) {
...
void ReplicaManager3::Clear(void) {
  if (autoDestroyConnections) {
    ...
    // Clear out downloadGroup even if not auto destroying the connection, since
    // the packets need to go back to RakPeer
    for (unsigned int i = 0; i < connectionList.Size(); i++)
...
void ReplicaManager3::SetWorldID(unsigned char id) { worldId = id; }

// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

unsigned char ReplicaManager3::GetWorldID(void) const { return worldId; }
  ...
  if (constructionMode == QUERY_REPLICA_FOR_CONSTRUCTION ||
    ...
    while (index < queryToConstructReplicaList.Size()) {
      ...
      if (constructionState == RM3CS_ALREADY_EXISTS_REMOTELY ||
        ...
        for (index=0; index < (unsigned int) RM3_NUM_OUTPUT_BITSTREAM_CHANNELS;
        index++)
        {
                ...
                res!=RM3SR_SERIALIZED_UNIQUELY)
        {
                ...
                if (SendSerialize(replica, allIndices, sp.outputBitstream,
        sp.messageTimestamp, sp.pro, replicaManager3->GetRakPeerInterface(),
    ...
    for (idx2 = 0; idx2 < destroyedReplicasCulled.Size(); idx2++) {
      ...
      // If this assert hits, the user tried to destroy a replica that doesn't
  ...
  if (time - lastAutoSerializeOccurance >= autoSerializeInterval) {
    ...
    for (index = 0; index < connectionList.Size(); index++) {
      ...
      while (index2 < connection->queryToSerializeReplicaList.Size()) {
        ...
        sp.whenLastSerialized = connection->queryToSerializeReplicaList[index2]
                                    ->replica->whenLastSerialized;
        ssicr = connection->SendSerializeIfChanged(
            index2, &sp, GetRakPeerInterface(), GetWorldID(), this);
        if (ssicr == SSICR_SENT_DATA) {
          connection->queryToSerializeReplicaList[index2]
              ->replica->whenLastSerialized = time;
          index2++;
...
ReplicaManager3::OnConstruction(Packet *packet, unsigned char *packetData,
  ...
  for (index = 0; index < constructionObjectListSize; index++) {
    ...
    Replica3 *existingReplica =
        networkIDManager->GET_OBJECT_FROM_ID<Replica3 *>(networkId);
    ...
    if (actuallyCreateObject) {
      ...
      // Go past the bitStream written to with WriteAllocationID(). Necessary in
      // case the user didn't read out the bitStream the same way it was written
...
ReplicaManager3::OnDownloadStarted(Packet *packet, unsigned char *packetData,
  ...
  if (connection->QueryGroupDownloadMessages() &&
      // ID_DOWNLOAD_STARTED will be processed twice, being processed the second
      // time once ID_DOWNLOAD_COMPLETE arrives. However, the second time
      // groupConstructionAndSerialize will be set to true so it won't be
      // processed a third time
      connection->groupConstructionAndSerialize == false) {
    // These messages will be held by the plugin and returned when the download
    // is complete
...
ReplicaManager3::OnDownloadComplete(Packet *packet, unsigned char *packetData,
  ...
  if (connection->groupConstructionAndSerialize == true &&
    ...
    // Push this one to be last too. It will be processed again, but the second
    // time groupConstructionAndSerialize will be false and downloadGroup will
    // be empty, so it will go past this block
...
SendSerializeIfChangedResult Connection_RM3::SendSerialize(
  ...
  for (channelIndex = 0; channelIndex < RM3_NUM_OUTPUT_BITSTREAM_CHANNELS;
    ...
    if (channelHasData) {
      ...
      // Crap, forgot this line, was a huge bug in that I'd only send to the
      // first 3 systems
...
SendSerializeIfChangedResult Connection_RM3::SendSerializeIfChanged(
  ...
  RakNet::Replica3 *replica =
      queryToSerializeReplicaList[queryToSerializeIndex]->replica;
  ...
  for (int i = 0; i < RM3_NUM_OUTPUT_BITSTREAM_CHANNELS; i++) {
    ...
    if (queryToSerializeReplicaList[queryToSerializeIndex]
            ->lastSerializationResultBS)
      sp->lastSentBitstream[i] =
          &queryToSerializeReplicaList[queryToSerializeIndex]
               ->lastSerializationResultBS->bitStream[i];
    else
  ...
  // This is necessary in case the user in the Serialize() function for some
  // reason read the bitstream they also wrote WIthout this code, the Write
  // calls to another bitstream would not write the entire bitstream
  BitSize_t sum = 0;
  for (int z = 0; z < RM3_NUM_OUTPUT_BITSTREAM_CHANNELS; z++) {
...
void Connection_RM3::OnDownloadFromThisSystem(Replica3 *replica3,
  ...
  if (constructionMode == QUERY_REPLICA_FOR_CONSTRUCTION ||
    ...
    for (j = 0; j < queryToConstructReplicaList.Size(); j++) {
      if (queryToConstructReplicaList[j]->replica->GetNetworkID() ==
          replica3->GetNetworkID()) {
...
void Connection_RM3::OnConstructToThisConnection(
  ...
  constructedReplicaList.Insert(lsr->replica, lsr, true, _FILE_AND_LINE_);
  // assert(queryToDestructReplicaList.GetIndexOf(lsr->replica)==(unsigned
  // int)-1);
  queryToDestructReplicaList.Push(lsr, _FILE_AND_LINE_);
  // assert(queryToSerializeReplicaList.GetIndexOf(lsr->replica)==(unsigned
  // int)-1);
  queryToSerializeReplicaList.Push(lsr, _FILE_AND_LINE_);
  ValidateLists(replicaManager);
}
...
void Connection_RM3::OnSendDestructionFromQuery(
  ...
  for (j = 0; j < queryToSerializeReplicaList.Size(); j++) {
    if (queryToSerializeReplicaList[j]->replica->GetNetworkID() ==
        lsr->replica->GetNetworkID()) {
      queryToSerializeReplicaList.RemoveAtIndex(j);
      break;
    }
  }
  for (j = 0; j < constructedReplicaList.Size(); j++) {
    if (constructedReplicaList[j]->replica->GetNetworkID() ==
        lsr->replica->GetNetworkID()) {
      constructedReplicaList.RemoveAtIndex(j);
      break;
    }
  }
  // assert(queryToConstructReplicaList.GetIndexOf(lsr->replica)==(unsigned
  // int)-1);
  queryToConstructReplicaList.Push(lsr, _FILE_AND_LINE_);
  ValidateLists(replicaManager);
}
...
queryToConstructReplicaList
  ...
  {
          if
(constructedReplicaList[idx]->replica->replicaManager!=replicaManager)
          {
                  int a=5;
                  assert(a==0);
                  int *b=0;
  ...
  {
          if
(queryToConstructReplicaList[idx]->replica->replicaManager!=replicaManager)
          {
                  int a=5;
                  assert(a==0);
                  int *b=0;
...
void Connection_RM3::SendConstruction(
  ...
  // Otherwise, if objects rely on each other being created the same tick to be
  // valid, this won't always be true
  ...
  for (newListIndex = 0; newListIndex < newObjects.Size(); newListIndex++) {
    ...
    if (actuallyCreateObject) {
      ...
      bsOut.Write(offsetStart2); // overwritten to point to after the call to
                                 // WriteAllocationID
      bsOut.AlignWriteToByteBoundary(); // Give the user an aligned bitStream in
                                        // case they use memcpy
  ...
  // Immediately send serialize after construction if the replica object already
  // has saved data If the object was serialized identically, and does not
  // change later on, then the new connection never gets the data
  SerializeParameters sp;
  sp.whenLastSerialized = 0;
  RakNet::BitStream emptyBs;
...
RM3ConstructionState Replica3::QueryConstruction_ClientConstruction(
  ...
  // Send back to the owner client too, because they couldn't assign the network
  // ID
  if (isThisTheServer)
    return RM3CS_SEND_CONSTRUCTION;
...
RM3ConstructionState Replica3::QueryConstruction_PeerToPeer(
  ...
  if (p2pMode == R3P2PM_SINGLE_OWNER) {
    ...
    // RM3CS_NEVER_CONSTRUCT will not send the object, and will not Serialize()
    // it
    return RM3CS_NEVER_CONSTRUCT;
