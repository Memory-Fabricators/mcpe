/// \file EmailSender.h
/// \brief Rudimentary class to send email from code.  Don't expect anything
/// fancy.
///
...
public:
  ...
  /// \param[in] attachedFiles List of files to attach to the email. (Can be 0
  /// to send none).
  /// \param[in] doPrintf true to output SMTP info to console(for debugging?)
  /// \param[in] password Used if the server uses AUTHENTICATE PLAIN over TLS
  /// (such as gmail)
  /// \return 0 on success, otherwise a string indicating the error message
  const char *Send(const char *hostAddress, unsigned short hostPort,
...
protected:
  const char *GetResponse(TCPInterface *tcpInterface,
