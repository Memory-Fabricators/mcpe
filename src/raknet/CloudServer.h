...
/// If the data is smaller than this value, an allocation is avoid. However,
/// this value exists for every row
...
/// \brief Zero or more instances of CloudServerQueryFilter can be attached to
/// CloudServer to restrict client queries All attached instances of
/// CloudServerQueryFilter on each corresponding operation, from all directly
/// connected clients If any attached instance returns false for a given
/// operation, that operation is silently rejected
/// \ingroup CLOUD_GROUP
...
public:
  ...
  /// \brief Max bytes returned by a download. If the number of bytes would
  /// exceed this amount, the returned list is truncated However, if this would
  /// result in no rows downloaded, then one row will be returned.
  /// \param[in] bytes Max bytes a client can download from a single Get(). 0
  /// means unlimited.
  void SetMaxBytesPerDownload(uint64_t bytes);

  /// \brief Add a server, which is assumed to be connected in a fully connected
  /// mesh to all other servers and also running the CloudServer plugin The
  /// other system must also call AddServer before getting the subscription
  /// data, or it will be rejected. Sending a message telling the other system
  /// to call AddServer(), followed by calling AddServer() locally, would be
  ...
  /// \brief Report the specified SystemAddress to client queries, rather than
  /// what RakPeer reads. This is useful if you already know your public IP This
  /// only applies to future updates, so call it before updating to apply to all
  /// queries
  ...
  /// \brief Adds a callback called on each query. If all filters returns true
  /// for an operation, the operation is allowed. If the filter was already
  /// added, the function silently fails
  /// \param[in] filter An externally allocated instance of
  /// CloudServerQueryFilter. The instance must remain valid until it is removed
  /// with RemoveQueryFilter() or RemoveAllQueryFilters()
  void AddQueryFilter(CloudServerQueryFilter *filter);

  /// \brief Removes a callback added with AddQueryFilter()
  /// The instance is not deleted, only unreferenced. It is up to the user to
  /// delete the instance, if necessary
  /// \param[in] filter An externally allocated instance of
  /// CloudServerQueryFilter. The instance must remain valid until it is removed
  /// with RemoveQueryFilter() or RemoveAllQueryFilters()
  void RemoveQueryFilter(CloudServerQueryFilter *filter);

  /// \brief Removes all instances of CloudServerQueryFilter added with
  /// AddQueryFilter(). The instances are not deleted, only unreferenced. It is
  /// up to the user to delete the instances, if necessary
...
protected:
  ...
  struct CloudData {
    ...
    unsigned char
        *allocatedData; // Uses allocatedData instead of stackData if length of
                        // data exceeds CLOUD_SERVER_DATA_STACK_SIZE
    unsigned char *dataPtr; // Points to either stackData or allocatedData
    uint32_t dataLengthBytes;
    bool isUploaded;

    /// System address of server that is holding this data, and the client is
    /// connected to
    ...
    /// RakNetGUID of server that is holding this data, and the client is
    /// connected to
    ...
    /// When the key data changes from this particular system, notify these
    /// subscribers This list mutually exclusive with
  ...
  struct RemoteServer {
    ...
    // This server needs to know about these keys when they are updated or
    // deleted
    DataStructures::OrderedList<CloudKey, CloudKey, CloudKeyComp>
        subscribedKeys;
    // This server has uploaded these keys, and needs to know about Get()
    // requests
    DataStructures::OrderedList<CloudKey, CloudKey, CloudKeyComp> uploadedKeys;
  ...
  struct GetRequest {
    ...
    // When request started. If takes too long for a response from another
    // system, can abort remaining systems
    RakNet::Time requestStartTime;

    // Assigned by server that gets the request to identify response. See
    // nextGetRequestId
    uint32_t requestId;
...
// The subscription can be further be defined as only subscribing to keys
// uploaded by or changed by a given system. It is possible to subscribe to keys
// not yet uploaded, or uploaded to another system
//
// Operations:
//
// 1. SubscribeToKey() - Get() operation with subscription
// A. Add to key subscription list for the client, which contains a keyId /
// specificUploaderList pair B. Send to remote servers that for this key, they
// should send us updates C. (Done, get operation returns current values)
//
...
// B. If no one else is subscribing to this key for any system, notify remote
// servers we no longer need subscription updates
//
// Internal operations:
//
// 1. Find if any connected client has subscribed to a given key
// A. This is used add and remove our subscription for this key to remote
// servers
//
// 2. For a given key and updating address, find all connected clients that care
// A. First find connected clients that have subscribed to this key, regardless
// of address B. Then find connected clients that have subscribed to this key
...
//
// 4. For a given client (such as on disconnect), remove all records of their
// subscriptions
