...
LevelRenderer::LevelRenderer(Minecraft *mc)
    : mc(mc), textures(mc->textures), level(NULL), cullStep(0),
      ...
      chunkLists(0), xChunks(0), yChunks(0), zChunks(0),

      chunks(NULL), sortedChunks(NULL),

      xMinChunk(0), yMinChunk(0), zMinChunk(0), xMaxChunk(0), yMaxChunk(0),
      zMaxChunk(0),

      lastViewDistance(-1),

      noEntityRenderFrames(2), totalEntities(0), renderedEntities(0),
      culledEntities(0),

      occlusionCheck(false), totalChunks(0), offscreenChunks(0),
      renderedChunks(0), occludedChunks(0), emptyChunks(0),

      chunkFixOffs(0), xOld(-9999), yOld(-9999), zOld(-9999),
  ...
  LOGI("numBuffers: %d\n", numListsOrBuffers);
  // for (int i = 0; i < numListsOrBuffers; ++i) printf("bufId %d: %d\t", i,
  // chunkBuffers[i]);
...
void LevelRenderer::allChanged() {
  ...
  // View distance is a chunk count; do not encode it as a bit shift.
  int dist = 16 * Mth::clamp(lastViewDistance, 1, MAX_VIEW_DISTANCE);
...
int LevelRenderer::renderChunks(int from, int to, int layer, float alpha) {
  ...
  for (int i = from; i < to; i++) {
    ...
    if (!sortedChunks[i]->empty[layer] && sortedChunks[i]->visible &&
    ...
    for (int i = 0; i < amount; i++) {
      ...
      if (!force) {
        if (chunk->distanceToSqr(player) > 16 * 16) {
          if (chunk->visible) {
            if (i >= MAX_VISIBLE_REBUILDS_PER_FRAME)
              return false;
          } else {
            if (i >= MAX_INVISIBLE_REBUILDS_PER_FRAME)
              return false;
          }
        }
      } else {
      ...
      dirtyChunks.erase(
          std::find(dirtyChunks.begin(), dirtyChunks.end(), chunk)); // @q: s-i?
      chunk->setClean();
    }
      ...
      if (!force) {
        if (chunk->distanceToSqr(player) > 1024.0f) {
          ...
          for (index = 0; index < count; index++) {
            if (toAdd[index] != NULL &&
                dirtyChunkSorter(toAdd[index], chunk) == false) {
              break;
            }
          }
...
void LevelRenderer::renderHit(Player *player, const HitResult &h, int mode,
  ...
  //           float br = ((float) (util.Mth::sin(System.currentTimeMillis() /
  //           100.0f)) * 0.2f + 0.8f); glColor4f2(br, br, br, ((float)
  //           (util.Mth::sin(System.currentTimeMillis() / 200.0f)) * 0.2f +
  //           0.5f));
  ...
  //          if (h.f == 5) x++;
  //          /*
  //           * t.begin(); t.noColor(); Tile.tiles[tileType].tesselate(level,
  //           x,
  //           * y, z, t); t.end();
  //           */
  //      }
...
void LevelRenderer::renderEntities(Vec3 cam, Culler *culler, float a) {
  ...
  EntityRenderDispatcher::yOff = TileEntityRenderDispatcher::yOff =
...
std::string LevelRenderer::gatherStats1() {
  std::stringstream ss;
  ss << "C: " << renderedChunks << "/" << totalChunks
     << ". F: " << offscreenChunks << ", O: " << occludedChunks
     << ", E: " << emptyChunks << "\n";
  return ss.str();
}
...
//        return "E: " + renderedEntities + "/" + totalEntities + ". B: " +
//        culledEntities + ", I: " + ((totalEntities - culledEntities) -
//        renderedEntities);
//    }
//
...
#ifdef OPENGL_ES
  drawArrayVT(skyBuffer, skyVertexCount, 24, GL_TRIANGLES);
#endif
  glEnable2(GL_TEXTURE_2D);
}
...
void LevelRenderer::renderClouds(float alpha) {
  ...
  float yOffs =
      (float)(mc->player->yOld + (mc->player->y - mc->player->yOld) * alpha);
  int s = 32;
  int d = 256 / s;
  Tesselator &t = Tesselator::instance;

  // glBindTexture(GL_TEXTURE_2D,
  // texturesloadTexture("/environment/clouds.png"));
  ...
  float xo =
      mc->player->xo + (mc->player->x - mc->player->xo) * alpha + time * 0.03f;
...
void LevelRenderer::addParticle(const std::string &name, float x, float y,
  ...
  // Particle* p = NULL;
  // if (name == "hugeexplosion") p = new HugeExplosionSeedParticle(level, x, y,
  // z, xa, ya, za); else if (name == "largeexplode") p = new
  // HugeExplosionParticle(textures, level, x, y, z, xa, ya, za);
  ...
  if (distanceSquared > particleDistance * particleDistance)
    return;
  ...
  if (name == "bubble")
    mc->particleEngine->add(new BubbleParticle(level, x, y, z, xa, ya, za));
  else if (name == "crit")
    mc->particleEngine->add(new CritParticle2(level, x, y, z, xa, ya, za));
  else if (name == "smoke")
    mc->particleEngine->add(new SmokeParticle(level, x, y, z, xa, ya, za));
  // else if (name == "note") mc->particleEngine->add(new NoteParticle(level, x,
  // y, z, xa, ya, za));
  else if (name == "explode")
    mc->particleEngine->add(new ExplodeParticle(level, x, y, z, xa, ya, za));
  else if (name == "flame")
    mc->particleEngine->add(new FlameParticle(level, x, y, z, xa, ya, za));
  else if (name == "lava")
    mc->particleEngine->add(new LavaParticle(level, x, y, z));
  // else if (name == "splash") mc->particleEngine->add(new
  // SplashParticle(level, x, y, z, xa, ya, za));
  else if (name == "largesmoke")
    mc->particleEngine->add(
        new SmokeParticle(level, x, y, z, xa, ya, za, 2.5f));
  ...
  // else if (name == "snowballpoof") mc->particleEngine->add(new
  // BreakingItemParticle(level, x, y, z, Item::snowBall)); else if (name ==
  // "slime") mc->particleEngine->add(new BreakingItemParticle(level, x, y, z,
  // Item::slimeBall)); else if (name == "heart") mc->particleEngine->add(new
  // HeartParticle(level, x, y, z, xa, ya, za));
...
/*
void LevelRenderer::addParticle(ParticleType::Id name, float x, float y, float
z, float xa, float ya, float za, int data) { float xd =
mc->cameraTargetPlayer->x - x; float yd = mc->cameraTargetPlayer->y - y; float
zd = mc->cameraTargetPlayer->z - z;
...
return;
        ...
        if (name == ParticleType::bubble)		mc->particleEngine->add(
new BubbleParticle(level, x, y, z, xa, ya, za) ); else if (name ==
ParticleType::crit)		mc->particleEngine->add(new CritParticle2(level,
x, y, z, xa, ya, za) ); else if (name == ParticleType::smoke)
mc->particleEngine->add(new SmokeParticle(level, x, y, z, xa, ya, za) ); else if
(name == ParticleType::explode)		mc->particleEngine->add( new
ExplodeParticle(level, x, y, z, xa, ya, za) ); else if (name ==
ParticleType::flame)		mc->particleEngine->add( new
FlameParticle(level, x, y, z, xa, ya, za) ); else if (name ==
ParticleType::lava)		mc->particleEngine->add( new LavaParticle(level,
x, y, z) ); else if (name == ParticleType::largesmoke)	mc->particleEngine->add(
new SmokeParticle(level, x, y, z, xa, ya, za, 2.5f) ); else if (name ==
ParticleType::reddust)		mc->particleEngine->add( new
RedDustParticle(level, x, y, z, xa, ya, za) ); else if (name ==
ParticleType::iconcrack)	mc->particleEngine->add( new
BreakingItemParticle(level, x, y, z, xa, ya, za, Item::items[data]) );

        //switch (name) {
        //	case ParticleType::bubble:		p = new
BubbleParticle(level, x, y, z, xa, ya, za); break;
        //	case ParticleType::crit:		p = new
CritParticle2(level, x, y, z, xa, ya, za); break;
        //	case ParticleType::smoke:		p = new
SmokeParticle(level, x, y, z, xa, ya, za); break;
        //	//case ParticleType::note: p = new NoteParticle(level, x, y, z,
xa, ya, za); break;
        //	case ParticleType::explode:		p = new
ExplodeParticle(level, x, y, z, xa, ya, za); break;
        //	case ParticleType::flame:		p = new
FlameParticle(level, x, y, z, xa, ya, za); break;
        //	case ParticleType::lava:		p = new
LavaParticle(level, x, y, z); break;
        //	//case ParticleType::splash: p = new SplashParticle(level, x, y,
z, xa, ya, za); break;
        //	case ParticleType::largesmoke:	p = new SmokeParticle(level, x,
y, z, xa, ya, za, 2.5f); break;
        //	case ParticleType::reddust:		p = new
RedDustParticle(level, x, y, z, xa, ya, za); break;
        //	case ParticleType::iconcrack:	p = new
BreakingItemParticle(level, x, y, z, xa, ya, za, Item::items[data]); break;
        //	//case ParticleType::snowballpoof: p = new
BreakingItemParticle(level, x, y, z, Item::snowBall); break;
        //	//case ParticleType::slime: p = new BreakingItemParticle(level,
x, y, z, Item::slimeBall); break;
        //	//case ParticleType::heart: p = new HeartParticle(level, x, y,
z, xa, ya, za); break;
        //	default:
...
void LevelRenderer::renderHitSelect(Player *player, const HitResult &h,
  ...
  if (mode == 0) {
    ...
    // LOGI("block: %d - %d (%s)\n", tileId, level->getData(h.x, h.y, h.z),
    // tile==NULL?"null" : tile->getDescriptionId().c_str() );
