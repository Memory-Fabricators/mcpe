...
void ChestRenderer::render(TileEntity *entity, float x, float y, float z,
  ...
  if (chest->level) {
    ...
    if (tile != NULL && data == 0) {
      ((ChestTile *)tile)
          ->recalcLockDir(chest->level, chest->x, chest->y, chest->z);
      data = chest->getData();
    }

    chest->checkNeighbors();
  }
  if (chest->n != NULL || chest->w != NULL)
    return;
  ...
  if (chest->n != NULL) {
    float open2 =
        chest->n->oOpenness + (chest->n->openness - chest->n->oOpenness) * a;
    if (open2 > open)
      open = open2;
  }
  if (chest->w != NULL) {
    float open2 =
        chest->w->oOpenness + (chest->w->openness - chest->w->oOpenness) * a;
    if (open2 > open)
      open = open2;
  }
