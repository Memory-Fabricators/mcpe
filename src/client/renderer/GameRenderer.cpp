...
GameRenderer::GameRenderer(Minecraft *mc)
    : mc(mc), renderDistance(0), _tick(0), _lastTickT(0), fovOffset(0),
      fovOffsetO(0), fov(1), oFov(1), _setupCameraFov(0), zoom(1), zoom_x(0),
      zoom_y(0), cameraRoll(0), cameraRollO(0), pickDirection(1, 0, 0),

      thirdDistance(4), thirdDistanceO(4), thirdRotation(0), thirdRotationO(0),
      thirdTilt(0), thirdTiltO(0),

      fogBr(0), fogBrO(0),

      fr(0), fg(0), fb(0), _rotX(0), _rotY(0), _rotXlast(0), _rotYlast(0),
...
GameRenderer::~GameRenderer() { delete itemInHandRenderer; }
...
void GameRenderer::setupCamera(float a, int eye) {
  // View distance is a chunk count. The previous shift-based calculation
  // only worked for the original 0..3 setting and becomes undefined at 32.
  renderDistance = 16.0f * (float)Mth::clamp(mc->options.viewDistance, 1,
                                             MAX_VIEW_DISTANCE);
#if defined(ANDROID)
  ...
  if (mc->options.anaglyph3d)
    glTranslatef2(-(eye * 2 - 1) * stereoScale, 0, 0);
  if (zoom != 1) {
...
void GameRenderer::moveCameraToPlayer(float a) {
  ...
  if (player->isPlayer() && ((Player *)player)->isSleeping()) {
    ...
    if (!mc->options.fixedCamera) {
      ...
      mc->options
          .thirdPersonView /* || (player->isPlayer() && !player->isAlive())*/) {
...
void GameRenderer::pick(float a) {
  ...
  if (mc->cameraTargetPlayer == NULL)
    return;
  if (!mc->cameraTargetPlayer->isAlive())
    return;
  ...
  if (hovered != NULL) {
    ...
    // if we don't have a hit result, attempt to hit the edge of the block we
    // are standing on (this is an pocket edition simplification to help
    // building floors)
    // LOGI("hovered : %d (%f)\n", mc->hitResult.type, viewVec.y);
    if (pickDirection.y < -.7f) {
      // looking down by more than roughly 45 degrees, fetch a hit to the block
      // standing on
...
void GameRenderer::tick(int nTick, int maxTick) {
  ...
  float distanceRatio =
      (float)mc->options.viewDistance / (float)MAX_VIEW_DISTANCE;
  distanceRatio = Mth::clamp(distanceRatio, 0.0f, 1.0f);
  float whiteness = 1.0f - distanceRatio;
...
void GameRenderer::onGraphicsReset() {
  if (itemInHandRenderer)
    itemInHandRenderer->onGraphicsReset();
}
