...
MobRenderer::MobRenderer(Model *model, float shadow)
    : model(model), armor(NULL) {
  shadowRadius = shadow;
}

MobRenderer::~MobRenderer() { delete model; }
...
float MobRenderer::getBob(Mob *mob, float a) { return (mob->tickCount + a); }

void MobRenderer::additionalRendering(Mob *mob, float a) {}

void MobRenderer::onGraphicsReset() {
  if (model)
    model->onGraphicsReset();
  if (armor)
    armor->onGraphicsReset();
}

int MobRenderer::prepareArmor(Mob *mob, int layer, float a) { return -1; }

float MobRenderer::getFlipDegrees(Mob *mob) { return 90; }

int MobRenderer::getOverlayColor(Mob *mob, float br, float a) { return 0; }
