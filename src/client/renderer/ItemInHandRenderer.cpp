...
ItemInHandRenderer::ItemInHandRenderer(Minecraft *mc)
    : mc(mc), lastSlot(-1), height(0), oHeight(0), lastIconRendered(0),
      lastItemRendered(0),
      // selectedItem(NULL),
      item(0, 1, 0) {
...
void ItemInHandRenderer::renderItem(Mob *mob, ItemInstance *item) {
  // Stopwatch& w = handler.get("item:" +
  // Tile::tiles[item->id]->getDescriptionId()); w.start();
...
void ItemInHandRenderer::render(float a) {
  ...
  if (item != NULL) {
    ...
    bool noSwingAnimation =
        isUsing && (isBow || (item->getUseAnimation() != UseAnim::none));
    ...
    if (player->getUseItemDuration() > 0) {
      ...
      if (anim == UseAnim::bow) {
        ...
        float timeHeld =
            (item->getUseDuration() - (player->getUseItemDuration() - a + 1));
        ...
        if (pow > 0.1f) {
          glTranslatef(
              0, Mth::sin((timeHeld - 0.1f) * 1.3f) * 0.01f * (pow - 0.1f), 0);
        }
    ...
    EntityRenderer *er =
        EntityRenderDispatcher::getInstance()->getRenderer(mc->player);
