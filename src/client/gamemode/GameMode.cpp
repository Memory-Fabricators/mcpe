...
GameMode::GameMode(Minecraft *minecraft)
    : minecraft(minecraft), destroyProgress(0), oDestroyProgress(0),
      destroyTicks(0), destroyDelay(0) {}
...
bool GameMode::useItemOn(Player *player, Level *level, ItemInstance *item,
  ...
  if (t == Tile::invisible_bedrock->id)
    return false;
...
float GameMode::getPickRange() { return 5.0f; }

void GameMode::initPlayer(Player *player) { initAbilities(player->abilities); }
...
void GameMode::tick() { oDestroyProgress = destroyProgress; }
