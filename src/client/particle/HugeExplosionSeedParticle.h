...
public:
  HugeExplosionSeedParticle(Level *level, float x, float y, float z, float xa,
                            float ya, float za)
      : super(level, x, y, z, 0, 0, 0), life(0), lifeTime(8) {}
  ...
  void tick() {
    for (int i = 0; i < 6; i++) {
      ...
      level->addParticle(PARTICLETYPE(largeexplode), xx, yy, zz,
