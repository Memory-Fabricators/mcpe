...
ModelPart::ModelPart(const std::string &id)
    : id(id), model(NULL), xTexOffs(0), yTexOffs(0) {
  _init();
}

ModelPart::ModelPart(int xTexOffs /*= 0*/, int yTexOffs /*= 0*/)
    : xTexOffs(xTexOffs), yTexOffs(yTexOffs), model(NULL) {
  _init();
}

ModelPart::ModelPart(Model *model, int xTexOffs /*= 0*/, int yTexOffs /*= 0*/)
    : model(model), xTexOffs(xTexOffs), yTexOffs(yTexOffs) {
  _init();
}

ModelPart::~ModelPart() { clear(); }
...
// int tex) { 	cubes.push_back(new Cube(this, xTexOffs, yTexOffs, x0, y0, z0,
// w,
// h, d, tex));
// }
...
void ModelPart::render(float scale) {
  ...
  if (xRot != 0 || yRot != 0 || zRot != 0) {
    ...
    if (zRot != 0)
      glRotatef2(zRot * c, 0.0f, 0.0f, 1.0f);
    if (yRot != 0)
      glRotatef2(yRot * c, 0.0f, 1.0f, 0.0f);
    if (xRot != 0)
      glRotatef2(xRot * c, 1.0f, 0.0f, 0.0f);
...
void ModelPart::draw() {
#ifdef OPENGL_ES
  drawArrayVT_NoState(vboId, cubes.size() * 2 * 3 * 6, 24, GL_TRIANGLES);
#else
  glCallList(list);
#endif
}
...
void ModelPart::renderHorrible(float scale) {
  ...
  if (xRot != 0 || yRot != 0 || zRot != 0) {
    ...
    glTranslatef2(x * scale, y * scale, z * scale);
    if (zRot != 0)
      glRotatef2(zRot * Mth::RADDEG, 0.0f, 0.0f, 1.0f);
    if (yRot != 0)
      glRotatef2(yRot * Mth::RADDEG, 0.0f, 1.0f, 0.0f);
    if (xRot != 0)
      glRotatef2(xRot * Mth::RADDEG, 1.0f, 0.0f, 0.0f);
