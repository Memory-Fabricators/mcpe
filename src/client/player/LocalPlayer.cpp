...
LocalPlayer::LocalPlayer(Minecraft *minecraft, Level *level, User *user,
  ...
  if (user != NULL) {
    if (user->name.length() > 0)
      // customTextureUrl = "http://s3.amazonaws.com/MinecraftSkins/" +
      // user.name + ".png";
      this->name = user->name;
  }
}
...
void LocalPlayer::tick() {
  ...
  if (minecraft->isOnline()) {
    if (std::abs(x - sentX) > .1f || std::abs(y - sentY) > .01f ||
  ...
  if (!minecraft->screen && containerMenu) {
    ...
    if (!hasPostedError) {
      minecraft->gui.postError(
          ErrorCodes::ContainerRefStillExistsAfterDestruction);
      hasPostedError = true;
    }
  }
#endif
  // LOGI("biome: %s\n", level->getBiomeSource()->getBiome((int)x >> 4, (int)z
  // >> 4)->name.c_str());
}
...
void LocalPlayer::move(float xa, float ya, float za) {
  ...
  if (this == minecraft->player && minecraft->options.isFlying) {
    ...
    if (autoJumpTime <= 0 && autoJumpEnabled) {
      // auto-jump when crossing the middle of a tile, and the tile in the front
      // is blocked
      bool jump = false;
      if (Mth::floor(prevX * 2.0f) != Mth::floor(newX * 2.0f) ||
          Mth::floor(prevZ * 2.0f) != Mth::floor(newZ * 2.0f)) {
...
#ifndef STANDALONE_SERVER
  if (e->isItemEntity())
    minecraft->particleEngine->add(new TakeAnimationParticle(
        minecraft->level, (ItemEntity *)e, this, -0.5f));
#endif
}
...
float LocalPlayer::getFieldOfViewModifier() {
  ...
  if (abilities.flying)
    targetFov *= 1.1f;
  targetFov *=
      ((walkingSpeed * getWalkingSpeedModifier()) / DEFAULT_WALK_SPEED + 1) / 2;
...
bool LocalPlayer::isSneaking() { return input->sneaking; }
...
void LocalPlayer::respawn() { minecraft->respawnPlayer(); }
