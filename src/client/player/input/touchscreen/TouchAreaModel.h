...
public:
  ...
  virtual float centerX() { return _x0 + (_x1 - _x0) * .5f; }

  virtual float centerY() { return _y0 + (_y1 - _y0) * .5f; }
...
public:
  CircleArea(float x, float y, float r) : _x(x), _y(y), _rr(r * r) {}
...
public:
  OrArea(IArea *a, IArea *b) : _a(a), _b(b) {}
...
public:
  AndArea(IArea *a, IArea *b) : _a(a), _b(b) {}
...
public:
  XorArea(IArea *a, IArea *b) : _a(a), _b(b) {}
...
public:
  NotArea(IArea *a) : _a(a) {}

  ~NotArea() {
    if (deleteMe && _a->deleteMe)
      delete _a;
  }

  virtual bool isInside(float x, float y) { return !_a->isInside(x, y); }
...
public:
  DifferenceArea(IArea *a, IArea *b) : _a(a), _b(b) {}
