...
// View distance is measured in chunks. Matches Voxy.
const int MAX_VIEW_DISTANCE = 2048;
...
return NULL;
    ...
    */
    ...
    bool isProgress() const { return _isProgress; }

    bool isBoolean() const { return _isBoolean; }

    bool isInt() const { return (!_isBoolean && !_isProgress); }

    int getId() { return _ordinal; }
...
public:
  ...
  void toggle(const Option *option, int dir) {
    ...
    if (option == &Option::RENDER_DISTANCE)
      viewDistance = (viewDistance + dir) % (MAX_VIEW_DISTANCE + 1);
    if (viewDistance < 0)
      viewDistance += MAX_VIEW_DISTANCE + 1;
