...
PaneCraftingScreen::PaneCraftingScreen(int craftingSize)
    : craftingSize(craftingSize), currentCategory(-1), currentItem(NULL),
      pane(NULL), btnCraft(1), btnClose(2, ""), selectedCategoryButton(NULL),
      guiBackground(NULL), guiSlotCategory(NULL), guiSlotCategorySelected(NULL),
      numCategories(4) {
...
void PaneCraftingScreen::init() {
  ...
  guiPaneFrame = builder.createSymmetrical(IntRectangle(0, 20, 8, 8), 1, 2)
                     ->setExcluded(1 << 4);
  guiSlotCategory = builder.createSymmetrical(IntRectangle(8, 32, 8, 8), 2, 2);
  guiSlotCategorySelected =
      builder.createSymmetrical(IntRectangle(0, 32, 8, 8), 2, 2);
...
void PaneCraftingScreen::recheckRecipes() {
  ...
  for (unsigned int i = 0; i < _items.size(); ++i) {
    ...
    item->inventoryCount =
        ip.getCount(ItemPack::getIdForItemInstance(&item->item));
    ...
    for (unsigned int j = 0; j < items.size(); ++j) {
      ...
      int has = 0;
      if (!Recipe::isAnyAuxValue(&jtem) &&
          (jtem.getAuxValue() == Recipe::ANY_AUX_VALUE)) {
  ...
  if (minecraft->player) {
    ...
    for (unsigned int i = 0; i < currentItem->neededItems.size(); ++i) {
      ...
      if (Tile::sandStone->id == req.item.id &&
        ...
        toRemove.count =
            minecraft->player->inventory->removeResource(toRemove, true);
...
bool PaneCraftingScreen::renderGameBehind() { return false; }

bool PaneCraftingScreen::closeOnPlayerHurt() { return true; }
...
CraftButton::CraftButton(int id)
    : super(id, ""), bg(NULL), bgSelected(NULL), numItems(0) {}
...
IntRectangle CraftButton::getItemPos(int i) { return IntRectangle(); }
