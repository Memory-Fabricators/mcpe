...
static void setIfNotSet(bool &ref, bool condition) { ref = (ref || condition); }
...
FurnaceScreen::FurnaceScreen(Player *player, FurnaceTileEntity *furnace)
    : super(new FurnaceMenu(furnace)), //@huge @attn
      inventoryPane(NULL), btnFuel(FurnaceTileEntity::SLOT_FUEL),
      btnIngredient(FurnaceTileEntity::SLOT_INGREDIENT),
      btnResult(FurnaceTileEntity::SLOT_RESULT), btnClose(4, ""),
      bHeader(5, "Furnace"), guiBackground(NULL), guiSlot(NULL),
      guiSlotMarked(NULL), guiSlotMarker(NULL), guiPaneFrame(NULL),
      player(player), furnace(furnace),
      selectedSlot(FurnaceTileEntity::SLOT_INGREDIENT), doRecreatePane(false),
      lastBurnTypeId(0), descWidth(90)
// guiSlotItem(NULL),
...
void FurnaceScreen::init() {
  ...
  guiPaneFrame = builder.createSymmetrical(IntRectangle(28, 42, 4, 4), 1, 1)
                     ->setExcluded(1 << 4);
...
void FurnaceScreen::setupPositions() {
  ...
  // Right - Slots, description etc
  {
    ...
    btnIngredient.width = btnFuel.width = btnResult.width =
        (int)guiSlot->getWidth();
    btnIngredient.height = btnFuel.height = btnResult.height =
        (int)guiSlot->getHeight();
...
bool FurnaceScreen::isAllowed(int slot) { // LOGI("items.size, index: %d, %d\n",
                                          // inventoryItems.size(), slot);
  if (slot >= (int)inventoryItems.size())
    return false;
  ...
  else if (selectedSlot == btnIngredient.id)
...
void FurnaceScreen::clearItems() {
  for (unsigned int i = 0; i < listFuel.size(); ++i)
    delete listFuel[i];
  for (unsigned int i = 0; i < listIngredient.size(); ++i)
    delete listIngredient[i];
  listFuel.clear();
...
bool FurnaceScreen::canMoveToFurnace(int inventorySlot,
  ...
  if (!jitem || jitem->isNull())
    return true;
  if (ItemInstance::isStackable(item, jitem) &&
      jitem->count < jitem->getMaxStackSize())
    return true;
...
bool FurnaceScreen::handleAddItem(int slot, const ItemInstance *item) {
  ...
  if (item->id == furnaceItem->id) {
    ...
    const int maxMovedCount =
        furnaceItem->getMaxStackSize() - furnaceItem->count;
    if (maxMovedCount <= 0)
      return false;
