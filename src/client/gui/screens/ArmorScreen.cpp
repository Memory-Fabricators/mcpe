...
ArmorScreen::ArmorScreen()
    : inventoryPane(NULL), btnArmor0(0), btnArmor1(1), btnArmor2(2),
      btnArmor3(3), btnClose(4, ""), bHeader(5, "Armor"), guiBackground(NULL),
      guiSlot(NULL), guiPaneFrame(NULL), guiPlayerBg(NULL),
      doRecreatePane(false), descWidth(90)
// guiSlotItem(NULL),
...
bool ArmorScreen::isAllowed(int slot) { return true; }
...
bool ArmorScreen::canMoveToSlot(int slot, const ItemInstance *item) {
  return ItemInstance::isArmorItem(item) && ((ArmorItem *)item)->slot == slot;
}
...
void ArmorScreen::renderPlayer(float xo, float yo) {
  ...
  player->walkAnimPos =
      getTimeS() * player->walkAnimSpeed * SharedConstants::TicksPerSecond;
