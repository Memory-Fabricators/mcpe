...
IngameBlockSelectionScreen::IngameBlockSelectionScreen()
    : selectedItem(0), _blockList(NULL), _pendingClose(false),
      bArmor(4, "Armor"), bDone(3, ""),
      // bDone   (3, "Done"),
      bMenu(2, "Menu"), bCraft(1, "Craft"), bHeader(0, "Select blocks") {}

IngameBlockSelectionScreen::~IngameBlockSelectionScreen() { delete _blockList; }
...
void IngameBlockSelectionScreen::init() {
  ...
  // LOGI("> %d - %s\n", i, inventory->getItem(i)?
  // inventory->getItem(i)->getDescriptionId().c_str() : "<-->\n");
...
bool IngameBlockSelectionScreen::addItem(const InventoryPane *pane,
  ...
  minecraft->soundEngine->playUI(
      "random.pop", 0.3f, 0.3f); // 1.0f + 0.2f*(Mth::random()-Mth::random()));
#else
  minecraft->soundEngine->playUI(
      "random.pop2", 1.0f, 0.3f); // 1.0f + 0.2f*(Mth::random()-Mth::random()));
#endif
...
#ifdef DEMO_MODE
  if (slot >= (minecraft->isCreativeMode() ? 28 : 27))
    return false;
#endif
  return true;
}
