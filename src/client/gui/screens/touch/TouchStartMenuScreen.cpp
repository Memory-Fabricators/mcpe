...
//
BuyButton::BuyButton(int id) : super(id, "") {
  ImageDef def;
...
StartMenuScreen::StartMenuScreen()
    : bHost(2, "Start Game"), bJoin(3, "Join Game"), bOptions(4, "Options"),
      bBuy(5), bTest(9, "Create") {
  ImageDef def;
  bJoin.width = 75;
...
void StartMenuScreen::buttonClicked(::Button *button) {
  ...
  if (button->id == bJoin.id) {
    ...
    minecraft->platform()->createUserInput(
        DialogDefinitions::DIALOG_DEMO_FEATURE_DISABLED);
#else
...
void StartMenuScreen::_updateLicense() {
  ...
  if (LicenseCodes::isReady(id)) {
    if (LicenseCodes::isOk(id))
      ...
      bool hasBuyButton =
          minecraft->platform()->hasBuyButtonWhenInvalidLicense();
