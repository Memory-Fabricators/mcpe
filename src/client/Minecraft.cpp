...
const char *Minecraft::progressMessages[] = {
    "Locating server", "Building terrain", "Preparing", "Saving chunks"};
...
Minecraft::Minecraft()
    : user(NULL), level(NULL), player(NULL), cameraTargetPlayer(NULL),
      levelRenderer(NULL), gameRenderer(NULL),
#ifndef STANDALONE_SERVER
      particleEngine(NULL), _perfRenderer(NULL),
#endif
      _commandServer(NULL),
#ifndef STANDALONE_SERVER
      textures(NULL),
#endif
      lastTickTime(-1), lastTime(0), ticksSinceLastUpdate(0), gameMode(NULL),
      mouseGrabbed(true), missTime(0), pause(false), _running(false), timer(20),
#ifndef STANDALONE_SERVER
      gui(this),
#endif
      netCallback(NULL),
#ifndef STANDALONE_SERVER
      screen(NULL), font(NULL),
#endif
      screenMutex(false),
#ifndef STANDALONE_SERVER
...
//_respawnPlayerTicks(-1),
      ...
      _isSuperFast(false),
#endif
      _powerVr(false), commandPort(4711), reserved_d1(0), reserved_d2(0),
...
#endif
      storageSource->selectLevel(levelId, false), levelName, settings,
...
void Minecraft::setLevel(Level *level, const std::string &message /* ="" */,
  ...
  if (level != NULL) {
    ...
    if (threadedLevelCreation) {
      ...
      // Exceptions are disabled (-fno-exceptions): std::thread construction
      // failures terminate the process, so there is no error to recover from.
...
void Minecraft::prepareLevel(const std::string &title) {
  ...
  // Dont update lights if we load the level (ok, actually just with leveldata
  // version=1.+(?))
  if (!level->isNew())
  ...
  if (level->isNew()) {
    level->setInitialSpawn(); // @note: should obviously be called from Level
                              // itself
    level->saveLevelData();
...
void Minecraft::tickInput() {
  ...
  while (Mouse::next()) {
    ...
    if (passedTime > 200)
      continue; // @note: As long Mouse::clear CLEARS the whole buffer, it's
                // safe to break here
    // But since it might be rewritten anyway (and hopefully there aren't a lot
    ...
    if (allowGuiClicks && e.action == MouseAction::ACTION_LEFT &&
        e.data == MouseAction::DATA_DOWN) {
    ...
    if (mouseDiggable && options.useMouseForDigging) {
            if (Mouse::getEventButton() == MouseAction::ACTION_LEFT &&
    Mouse::getEventButtonState()) { handleMouseClick(MouseAction::ACTION_LEFT);
                    lastClickTick = ticks;
            }
            if (Mouse::getEventButton() == MouseAction::ACTION_RIGHT &&
    Mouse::getEventButtonState()) { handleMouseClick(MouseAction::ACTION_RIGHT);
                    lastClickTick = ticks;
            }
    }
    */
    ...
    if (isPressed) {
      ...
      if (key == Keyboard::KEY_L)
        options.viewDistance =
            (options.viewDistance + 1) % (MAX_VIEW_DISTANCE + 1);
      ...
      if (key == Keyboard::KEY_G) {
        ...
        /*
        std::vector<AABB>& boxs = level->getCubes(NULL, AABB(128.1f, 73, 128.1f,
        128.9f, 74.9f, 128.9f)); LOGI("boxes: %d\n", (int)boxs.size());
        */
      }
      ...
      if (key == Keyboard::KEY_Z || key == 108) {
        for (int i = 0; i < 1; ++i) {
          ...
          float dx =
              4 - 8 * Mth::random() + 4 * Mth::sin(Mth::DEGRAD * player->yRot);
          float dz =
...
void Minecraft::handleMouseDown(int button, bool down) {
  ...
  if (button == MouseAction::ACTION_LEFT && missTime > 0)
    return;
  if (down && hitResult.type == TILE && button == MouseAction::ACTION_LEFT &&
      !hitResult.indirectHit) {
    int x = hitResult.x;
    int y = hitResult.y;
    int z = hitResult.z;
...
void Minecraft::handleBuildAction(BuildActionIntention *action) {
  ...
  if (!hitResult.isHit()) {
    ...
    if (action->isRemove()) {
      ...
      // oldTile->getDescriptionId().c_str(), x, y, z,
      // oldTile->getBrightness(level, x, y, z), oldTile->getBrightness(level,
      // x, y+1, z));
...
bool Minecraft::isOnline() { return netCallback != NULL; }
...
void Minecraft::_levelGenerated() {
  ...
  // if we are client side, we trust the server to have given us a correct
  // position
...
void Minecraft::respawnPlayer() {
  ...
  //
  // LOGI("Has entity? %d, %d\n", level->getEntity(player->entityId),
  // slowCheck);
...
void Minecraft::onGraphicsReset() {
  ...
  if (levelRenderer)
    levelRenderer->onGraphicsReset();
  if (gameRenderer)
    gameRenderer->onGraphicsReset();
...
int Minecraft::getProgressStatusId() { return progressStageStatusId; }
...
void Minecraft::setIsCreativeMode(bool isCreative) {
  ...
  if (!gameMode || isCreative != _isCreativeMode) {
    delete gameMode;
    if (isCreative)
      gameMode = new CreativeMode(this);
    else
...
bool Minecraft::isCreativeMode() { return _isCreativeMode; }
