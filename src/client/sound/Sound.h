...
public:
  ...
  SoundDesc(char *data, int size, int channels, int width, int rate)
      : buffer(data), frames(data), size(size), channels(channels),
        byteWidth(width), frameRate(rate) {
    numFrames = size / (channels * byteWidth);
  }

  SoundDesc(char *data) : buffer(data) {
  ...
  float length() const { return ((float)numFrames) / frameRate; }
