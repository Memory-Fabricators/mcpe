...
JNIEXPORT void JNICALL Java_com_mojang_minecraftpe_MainActivity_nativeStopThis(
...
void android_main(struct android_app *state) {
  ...
  while (1) {
    ...
    while ((ident = ALooper_pollAll(0, NULL, &events, (void **)&source)) >= 0) {
      // Process this event.
      // This will call the function pointer android_app::onInputEvent() which
      // in our case is engine_handle_input()
      if (source != NULL) {
        if (source->id == 2) {
          // Back button is intercepted by the ime on android 4.1/4.2 resulting
          // in the application stopping to respond.
