...
public:
  // RakDataOutput() {}
  RakDataOutput(RakNet::BitStream &bitstream) : _bitStream(bitstream) {}

  virtual void writeBytes(const void *data, int bytes) {
    _bitStream.WriteBits((const unsigned char *)data, bytes * 8);
  }

  RakNet::BitStream &getBitStream() { return _bitStream; }
...
public:
  // RakDataOutput() {}
  RakDataInput(RakNet::BitStream &bitstream) : _bitStream(bitstream) {}
