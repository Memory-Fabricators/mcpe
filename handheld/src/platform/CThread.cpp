...
CThread::CThread(pthread_fn threadFunc, void *threadParam) {
  ...
  m_threadHandle = CreateThread(NULL,          // pointer to security attributes
  ...
  /*int error =*/pthread_create(&m_thread, &m_attributes, mp_threadFunc,
                                threadParam);
#endif
#ifdef MACOSX
  mp_threadFunc = (TaskProc)threadFunc;

  MPCreateTask(mp_threadFunc, // pointer to thread function
               threadParam,   // argument for new thread
