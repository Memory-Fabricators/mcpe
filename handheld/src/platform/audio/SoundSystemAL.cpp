...
// typedef ALvoid	AL_APIENTRY	(*alBufferDataStaticProcPtr) (const
// ALint bid, ALenum format, ALvoid *data, ALsizei size, ALsizei freq); ALvoid
// alBufferDataStaticProc(const ALint bid, ALenum format, ALvoid* data, ALsizei
// size, ALsizei freq)
//{
//	static alBufferDataStaticProcPtr proc = NULL;
//
//     if (proc == NULL) {
//         proc = (alBufferDataStaticProcPtr) alcGetProcAddress(NULL, (const
//         ALCchar*) "alBufferDataStatic");
//     }
//
//     if (proc)
...
SoundSystemAL::SoundSystemAL()
    : available(true), context(0), device(0), _rotation(-9999.9f) {
  _buffers.reserve(64);
  init();
}
...
bool SoundSystemAL::getBufferId(const SoundDesc &sound, ALuint *buf) {
  ...
  // sound.byteWidth, sound.frames, sound.size, sound.frameRate);
  checkError();

  // LOGI("Sound ch: %d, fmt: %d, frames: %p, len: %f, fr: %d, sz: %d, numfr:
  // %d\n", sound.channels, format, sound.frames, sound.length(),
  // sound.frameRate, sound.size, sound.numFrames);
