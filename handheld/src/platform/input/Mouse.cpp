...
MouseDevice::MouseDevice()
    : _index(-1), _x(0), _xOld(0), _y(0), _yOld(0), _dx(DELTA_NOTSET),
      _dy(DELTA_NOTSET), _firstMovementType(0) {
...
char MouseDevice::getButtonState(int buttonId) {
  if (buttonId < MouseAction::ACTION_LEFT ||
      buttonId > MouseAction::ACTION_WHEEL)
    return 0;
...
bool MouseDevice::wasFirstMovement() { return _firstMovementType == 1; }
...
void MouseDevice::rewind() { _index = -1; }

bool MouseDevice::getEventButtonState() {
  return _inputs[_index].data == MouseAction::DATA_DOWN;
}

char MouseDevice::getEventButton() { return _inputs[_index].action; }
...
char Mouse::getButtonState(int buttonId) {
  return _instance.getButtonState(buttonId);
}

bool Mouse::isButtonDown(int buttonId) {
  return _instance.isButtonDown(buttonId);
}
