...
void MinecraftApp::updateStats() {
  ...
  if (now >= lastTime + 1000) {
    if (player) {
      ...
      // externalStoragePath.c_str(), _n/5);
...
void MinecraftApp::testJoiningAndDestruction() {
  if (_state == -1) {
    ...
    // 1),
    // LightUpdate(LightLayer::Sky, 0, 0, 0, 1, 1, 1) }; LOGI("size of
    // lightupdate: %lu == %d\n", sizeof(LightUpdate), (const char*)&sz[1] -
    // (const char*)&sz[0]);
    _stateTicksLeft = 100;
    _state = 0;
  }
...
bool NinecraftApp::hasPlayedDemo() {
        std::string filename = externalStoragePath +
"/games/com.mojang/minecraftpe/played_demo"; FILE* fp = fopen(filename.c_str(),
"r"); if (!fp) return false; fclose(fp); return true;
}
*/
