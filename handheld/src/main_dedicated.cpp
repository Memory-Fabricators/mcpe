...
int main(int numArguments, char *pszArgs[]) {
  ...
  if (aSettings.getShowHelp()) {
    ...
    printf("-------------------------------------------------------\n");
    printf("--cachepath - Path to where the server can store temp stuff (not "
           "sure if this is used) [default: \"%s\"]\n",
           defaultSettings.getCachePath().c_str());
    printf("--externalpath - The path to the place where the server should "
           "store the levels. [default: \"%s\"]\n",
    ...
    printf("--serverkey - The key that the server should use for API calls. "
           "[default: \"%s\"]\n",
  ...
  ((MAIN_CLASS *)g_app)
      ->selectLevel(aSettings.getLevelDir(), aSettings.getLevelName(),
                    settings);
  ((MAIN_CLASS *)g_app)->hostMultiplayer(aSettings.getPort());

  std::cout << "Level has been generated in " << getTimeS() - startTime
            << std::endl;
