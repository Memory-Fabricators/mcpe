...
public:
  CameraEntity(Level *level) : super(level), followEntityId(-1) {
    moveTo(128, 72, 128, 0, 90.0f);
  }

  void follow(int entityId) { followEntityId = entityId; }
...
template <typename T> static std::string ToStringOk(const T &a) {
  std::stringstream ss;
  ss << a << "\n";
  return ss.str();
}
template <typename T> static std::string ToStringOk(const T &a, const T &b) {
  std::stringstream ss;
  ss << a << "," << b << "\n";
  return ss.str();
}
...
CommandServer::CommandServer(Minecraft *mc)
    : mc(mc), serverSocket(0), restoreBuffer(0), inited(false) {
...
std::string CommandServer::parse(ConnectedClient &client,
  ...
  // Settings
  if (cmd.find("player.setting") == 0 || cmd.find("world.setting") == 0) {
    int value;
    static char name[1024];
    if (rest.find(",") >= 100)
      return Fail;
    if (2 != sscanf(rest.c_str(), "%[^,],%d", name, &value))
      return Fail;
...
std::string CommandServer::handleSetSetting(const std::string &setting,
  ...
  if (setting == "autojump")
    mc->player->autoJumpEnabled = status;

  AdventureSettingsPacket::Flags flag = (AdventureSettingsPacket::Flags)0;
  if (setting == "nametags_visible")
    flag = AdventureSettingsPacket::ShowNameTags;
  if (setting == "world_immutable")
    flag = AdventureSettingsPacket::WorldImmutable;
