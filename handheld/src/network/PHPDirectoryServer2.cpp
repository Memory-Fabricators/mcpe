/// \file
/// \brief Contains WebGameList, a client for communicating with a HTTP list of
/// game servers
///
...
// Column with this header contains the IP address of the game, passed to
// UploadTable()
...
PHPDirectoryServer2::PHPDirectoryServer2() : nextRepost(0) {
...
void PHPDirectoryServer2::SetField(RakNet::RakString columnName,
  ...
  if (columnName == GAME_NAME_COMMAND || columnName == GAME_PORT_COMMAND ||
      columnName == LAST_UPDATE_COMMAND) {
    RakAssert(
        "PHPDirectoryServer2::SetField attempted to set reserved column name" &&
        0);
    return;
  }
...
void PHPDirectoryServer2::UploadAndDownloadTable(
    RakNet::RakString uploadPassword, RakNet::RakString downloadPassword,
...
void PHPDirectoryServer2::PushColumnsAndValues(
    DataStructures::List<RakNet::RakString> &columns,
    DataStructures::List<RakNet::RakString> &values) {
  ...
  for (i = 0; i < columns.Size() && i < values.Size(); i++) {
    if (columns[i].IsEmpty() == false) {
      ...
      if (row == 0) {
        row =
            lastDownloadedTable.AddRow(lastDownloadedTable.GetAvailableRowId());
      }
...
void PHPDirectoryServer2::SendOperation(void) {
  ...
  for (unsigned i = 0; i < fields.Size(); i++) {
    ...
    value.URLEncode();
    outgoingMessageBody += RakString(
        "\001%s\001%s", fields.GetKeyAtIndex(i).C_String(), value.C_String());
  }
