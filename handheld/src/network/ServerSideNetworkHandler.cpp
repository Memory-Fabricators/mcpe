...
void ServerSideNetworkHandler::redistributePacket(
    Packet *packet, const RakNet::RakNetGUID &fromPlayer) {
...
void ServerSideNetworkHandler::onNewClient(
...
void ServerSideNetworkHandler::handle(const RakNet::RakNetGUID &source,
  ...
  while (newPlayer->y > 0) {
    ...
    if (level->getCubes(newPlayer, newPlayer->bb).size() == 0)
      break;
    newPlayer->y += 1;
  }
...
void ServerSideNetworkHandler::onReady_ClientGeneration(
...
void ServerSideNetworkHandler::handle(const RakNet::RakNetGUID &source,
  ...
  // cpacket.chunkData.GetNumberOfBytesUsed()); LOGI("size: %d\n",
...
void ServerSideNetworkHandler::handle(const RakNet::RakNetGUID &source,
  ...
  if (!player)
    return;
  if (rakPeer->GetMyGUID() == player->owner)
    return;
...
void ServerSideNetworkHandler::handle(const RakNet::RakNetGUID &source,
  ...
  if (entity && entity->isPlayer()) {
    ...
    if (t == Tile::invisible_bedrock)
      return;
    if (t && t->use(level, x, y, z, player))
      return;
    if (packet->item.isNull())
      return;
    ...
    // Don't have to redistribute (ugg.. this will mess up), cause tileUpdated
    // is sent
    // redistributePacket(packet, source);
  }
}
...
void ServerSideNetworkHandler::handle(const RakNet::RakNetGUID &source,
  ...
  if (ContainerType::FURNACE == p->containerMenu->containerType) {
    ...
    // packet->item.toString().c_str());
    p->containerMenu->setSlot(packet->slot, &packet->item);
    // p->containerMenu->setSlot(packet->slot, packet->item.isNull()? NULL :
    // &packet->item);
  }
...
Player *ServerSideNetworkHandler::getPlayer(const RakNet::RakNetGUID &source) {
  for (unsigned int i = 0; i < level->players.size(); ++i)
    if (source == level->players[i]->owner)
