...
public:
  AddPlayerPacket() : _entityData(NULL) {}

  AddPlayerPacket(const Player *p)
      : owner(p->owner), name(p->name.c_str()), entityId(p->entityId), x(p->x),
        y(p->y - p->heightOffset), z(p->z), xRot(p->xRot), yRot(p->yRot),
        carriedItemId(0), carriedItemAuxValue(0),
  ...
  void write(RakNet::BitStream *bitStream) {
    bitStream->Write(static_cast<RakNet::MessageID>(
        static_cast<int>(ID_USER_PACKET_ENUM) + PACKET_ADDPLAYER));
