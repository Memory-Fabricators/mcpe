...
public:
  ...
  RemoveEntityPacket(int entityId) : entityId(entityId) {}

  void write(RakNet::BitStream *bitStream) {
    bitStream->Write(static_cast<RakNet::MessageID>(
        static_cast<int>(ID_USER_PACKET_ENUM) + PACKET_REMOVEENTITY));

    bitStream->Write(entityId);
  }

  void read(RakNet::BitStream *bitStream) { bitStream->Read(entityId); }
