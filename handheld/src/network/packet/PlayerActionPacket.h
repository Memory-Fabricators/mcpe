...
public:
  ...
  PlayerActionPacket() : x(0), y(0), z(0), action(0), face(0), entityId(0) {}

  PlayerActionPacket(int action, int x, int y, int z, int face, int entityId)
      : x(x), y(y), z(z), face(face), action(action), entityId(entityId) {}
  ...
  void write(RakNet::BitStream *bitStream) {
    bitStream->Write(static_cast<RakNet::MessageID>(
        static_cast<int>(ID_USER_PACKET_ENUM) + PACKET_PLAYERACTION));
