...
public:
  ...
  LoginPacket()
      : clientNetworkVersion(-1), clientNetworkLowestSupportedVersion(-1) {}

  LoginPacket(const RakNet::RakString &clientName, int clientVersion)
      : clientName(clientName), clientNetworkVersion(clientVersion),
        clientNetworkLowestSupportedVersion(clientVersion) {}

  void write(RakNet::BitStream *bitStream) {
    bitStream->Write(static_cast<RakNet::MessageID>(
        static_cast<int>(ID_USER_PACKET_ENUM) + PACKET_LOGIN));
