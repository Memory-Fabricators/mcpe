...
public:
  ...
  ReadyPacket() : type(READY_UNDEFINED) {}

  ReadyPacket(char type) : type(type) {}

  void write(RakNet::BitStream *bitStream) {
    bitStream->Write(static_cast<RakNet::MessageID>(
        static_cast<int>(ID_USER_PACKET_ENUM) + PACKET_READY));
    bitStream->Write(type);
  }

  void read(RakNet::BitStream *bitStream) { bitStream->Read(type); }
