...
public:
  ...
  AddItemEntityPacket(const ItemEntity *itemEntity)
      : id(itemEntity->entityId), itemId(itemEntity->item.id),
        itemCount(itemEntity->item.count),
        auxValue(itemEntity->item.getAuxValue()), x(itemEntity->x),
        y(itemEntity->y), z(itemEntity->z),
  ...
  void write(RakNet::BitStream *bitStream) {
    bitStream->Write(static_cast<RakNet::MessageID>(
        static_cast<int>(ID_USER_PACKET_ENUM) + PACKET_ADDITEMENTITY));
    bitStream->Write(id);
