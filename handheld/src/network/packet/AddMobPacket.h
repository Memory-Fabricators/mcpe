...
public:
  ...
  AddMobPacket(const Mob *mob)
      : entityId(mob->entityId), type(mob->getEntityTypeId()), x(mob->x),
        y(mob->y), z(mob->z), xRot(mob->xRot), yRot(mob->yRot),
        _entityData(mob->getEntityData()) {}

  void write(RakNet::BitStream *bitStream) {
    bitStream->Write(static_cast<RakNet::MessageID>(
        static_cast<int>(ID_USER_PACKET_ENUM) + PACKET_ADDMOB));
