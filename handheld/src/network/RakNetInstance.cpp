...
RakNetInstance::RakNetInstance()
    : rakPeer(NULL), _isServer(false), _isLoggedIn(false) {
...
bool RakNetInstance::connect(const char *host, int port) {
  ...
  if (result == RakNet::RAKNET_STARTED) {
    RakNet::ConnectionAttemptResult connectResult =
        rakPeer->Connect(host, port, NULL, 0, NULL, 0, 12, 500, 0);
...
bool RakNetInstance::isProbablyBroken() { return rakPeer->errorState < -100; }
void RakNetInstance::resetIsBroken() { rakPeer->errorState = 0; }
...
int RakNetInstance::handleUnconnectedPong(const RakNet::RakString &data,
  ...
  if (!emptyNameOrLonger ||
      appIdentifier.StrCmp(data.SubStr(0, appIdentifier.GetLength())) != 0)
    return -1;
...
void RakNetInstance::setIsLoggedIn(bool status) { _isLoggedIn = status; }
