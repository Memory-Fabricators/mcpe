/// \file
/// \brief Contains PHPDirectoryServer2, a client for communicating with a HTTP
...
///
/// PHPDirectoryServer2 works with the HTTPConnection class (which works with
/// the TCPInterface class) in order to communiate with DirectoryServer.php
/// found under Samples/PHPDirectoryServer2
class PHPDirectoryServer2 {
public:
  PHPDirectoryServer2();
  virtual ~PHPDirectoryServer2();

  /// Associate PHPDirectoryServer2 with the HTTPConnection class it will
  /// communicate through
  /// \param[in] _http The instance of HTTP connection we will communicate
  /// through
  /// \param[in] _path The path to the PHP file on the remote server. For
  /// example, if the path is mysite.com/raknet/DirectoryServer.php then you
  /// would enter raknet/DirectoryServer.php
  ...
  /// Store the game name and port with UploadTable, rather than SetField, as
  /// these columns are required and use reserved column names
  /// \param[in] columnName The name of the column to store
  /// \param[in] value What value to hold for the uploaded row (only one row can
  ...
  /// all values are stored as strings in PHP
  ...
  /// 2. Pass the value returned by HTTPConnection::Read() to
  /// PHPDirectoryServer2::ProcessHTTPRead().
  /// 3. The return value of PHPDirectoryServer2::ProcessHTTPRead() will be
  /// HTTP_RESULT_EMPTY
  /// \param[in] uploadPassword The upload password set in the PHP page itself
  /// when you first uploaded and viewed it in the webpage.
  /// \param[in] gameName Every entry must have a game name. Pass it here.
  /// \param[in] gamePort Every entry must have a game port. Pass it here. The
  /// IP address will be stored automatically, or you can manually set it by
  /// passing a field named _System_Address
  /// \param[in] autoRepost Tables must be uploaded every 60 seconds or they get
  ...
  /// PHPDirectoryServer2::ProcessHTTPRead().
  /// 3. The return value of PHPDirectoryServer2::ProcessHTTPRead() will be
  /// HTTP_RESULT_GOT_TABLE or HTTP_RESULT_EMPTY
  /// 4. On HTTP_RESULT_GOT_TABLE, use GetLastDownloadedTable() to read the
  /// results.
  /// \param[in] downloadPassword The download password set in the PHP page
  /// itself when you first uploaded and viewed it in the webpage.
  void DownloadTable(RakNet::RakString downloadPassword);

  /// Same as calling DownloadTable immediately followed by UploadTable, except
  /// only the download result is returned
  /// \param[in] uploadPassword The upload password set in the PHP page itself
  /// when you first uploaded and viewed it in the webpage.
  /// \param[in] downloadPassword The download password set in the PHP page
  /// itself when you first uploaded and viewed it in the webpage.
  ...
  /// into DataStructures::Table, and a copy stored internally which can be
  /// retrieved by GetLastDownloadedTable();
  ...
  /// Call this periodically - it will handle connection states and refreshing
