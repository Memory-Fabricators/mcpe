...
class JVMAttacher {
public:
  JVMAttacher(JavaVM *vm) : _vm(vm), _isAttached(false), _env(NULL) {
...
public:
  AppPlatform_android()
      : _vm(NULL), _isInited(false),

        instance(0), _activityClass(0),

        _methodSaveScreenshot(0), _postScreenshotToFacebook(0),
        _getImageData(0), _readAssetFile(0), _methodPlaySound(0),
        _showDialog(0), _methodTick(0), _methodFinish(0),

        _methodUserInputInitiate(0), _methodUserInputStatus(0),
        _methodUserInputString(0),

        _methodGetDateString(0),

        _methodCheckLicense(0), _methodHasBuyButton(0), _methodBuyGame(0),
        _methodVibrate(0), _methodSupportsTouchscreen(0),
        _methodSetIsPowerVR(0), _methodIsNetworkEnabled(0),

        _getScreenWidth(0), _getScreenHeight(0),
        _methodGetPixelsPerMillimeter(0), _methodGetPlatformStringVar(0),

        _classWindow(0), _classContext(0), _fieldINPUT_METHOD_SERVICE(0),
        _classInputManager(0), _methodGetSystemService(0), _methodGetWindow(0),
        _methodGetDecorView(0), _methodShowSoftInput(0), _classView(0),
        _methodGetWindowToken(0), _methodHideSoftInput(0),

        _screenWidth(854), _screenHeight(480), _hasSetPowerVR(false),
        _nativeActivity(0),
  ...
  // Note, this has to be called from the main thread (e.g. do it from
  // JNI_onLoad) Somewhere between calling this, and calling the AppPlatform
  // methods, this->instance will be assigned by its creator
  int init(JavaVM *vm) {
    if (_isInited)
      return -1;
    ...
    _methodUserInputInitiate =
        env->GetMethodID(_activityClass, "initiateUserInput", "(I)V");
    _methodUserInputStatus =
        env->GetMethodID(_activityClass, "getUserInputStatus", "()I");
    ...
    _methodHasBuyButton = env->GetMethodID(
        _activityClass, "hasBuyButtonWhenInvalidLicense", "()Z");
    _methodBuyGame = env->GetMethodID(_activityClass, "buyGame", "()V");

    _methodVibrate = env->GetMethodID(_activityClass, "vibrate", "(I)V");
    _methodSupportsTouchscreen =
        env->GetMethodID(_activityClass, "supportsTouchscreen", "()Z");
    _methodSetIsPowerVR =
        env->GetMethodID(_activityClass, "setIsPowerVR", "(Z)V");
    _methodIsNetworkEnabled =
        env->GetMethodID(_activityClass, "isNetworkEnabled", "(Z)Z");

    _methodGetPixelsPerMillimeter =
        env->GetMethodID(_activityClass, "getPixelsPerMillimeter", "()F");
    _methodGetPlatformStringVar = env->GetMethodID(
        _activityClass, "getPlatformStringVar", "(I)Ljava/lang/String;");

    _classWindow =
        (jclass)env->NewGlobalRef(env->FindClass("android/view/Window"));
    _classContext =
        (jclass)env->NewGlobalRef(env->FindClass("android/content/Context"));
    _classView = (jclass)env->NewGlobalRef(env->FindClass("android/view/View"));
    _classInputManager = (jclass)env->NewGlobalRef(
  ...
  int initConsts() {
    ...
    jmethodID fWidth =
        env->GetMethodID(_activityClass, "getScreenWidth", "()I");
    jmethodID fHeight =
        env->GetMethodID(_activityClass, "getScreenHeight", "()I");
  ...
  float getPixelsPerMillimeter() {
    if (!_isInited)
      return 10;
    if (!_methodGetPixelsPerMillimeter)
      return 10;
  ...
  StringVector getUserInput() {
    ...
    jobjectArray sa =
        (jobjectArray)env->CallObjectMethod(instance, _methodUserInputString);
  ...
  std::string getDateString(int s) {
    ...
    std::string out;
    jstring dateString =
        (jstring)env->CallObjectMethod(instance, _methodGetDateString, s);
  ...
  std::string getPlatformStringVar(int s) {
    if (!_isInited)
      return "";
    if (!_methodGetPlatformStringVar)
      return "";
    ...
    std::string out;
    jstring stringVar = (jstring)env->CallObjectMethod(
        instance, _methodGetPlatformStringVar, s);
  ...
  virtual void showKeyboard(bool bShow) {
    ...
    jobject INPUT_METHOD_SERVICE =
        env->GetStaticObjectField(_classContext, _fieldINPUT_METHOD_SERVICE);

    jobject lInputMethodManager = env->CallObjectMethod(
        instance, _methodGetSystemService, INPUT_METHOD_SERVICE);

    jobject lWindow = env->CallObjectMethod(instance, _methodGetWindow);

    jobject lDecorView = env->CallObjectMethod(lWindow, _methodGetDecorView);

    if (bShow) {
      // Runs lInputMethodManager.showSoftInput(...).
      jboolean lResult = env->CallBooleanMethod(
          lInputMethodManager, _methodShowSoftInput, lDecorView, lFlags);
    } else {
      // Runs lWindow.getViewToken()
      jobject lBinder =
          env->CallObjectMethod(lDecorView, _methodGetWindowToken);
      jboolean lRes = env->CallBooleanMethod(
          lInputMethodManager, _methodHideSoftInput, lBinder, lFlags);
    }
  }
