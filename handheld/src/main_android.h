#ifndef MAIN_CLASS
#error                                                                         \
    "Error: MAIN_CLASS must have been defined to your main class (e.g. #define MAIN_CLASS MyApp)"
#endif
...
static int engine_init_display(struct ENGINE *engine) {
  ...
  for (int i = 0; i < numConfigs; ++i) {
    EGLSurface surf =
        eglCreateWindowSurface(display, configs[i], engine->app->window, NULL);
...
static int32_t handle_keyboard_input(struct android_app *app,
  ...
  if (AInputEvent_getType(event) == AINPUT_EVENT_TYPE_KEY) {
    ...
    bool pressedBack =
        (keyCode == 4) && ((AKeyEvent_getMetaState(event) & AMETA_ALT_ON) == 0);
    ...
    if (!isRepeat) {
      ...
      // unsigned char msg[2] = {(unsigned char)(keyCode), (unsigned
      // char)(isDown)}; broadcastData(BroadcastPort, msg, 2);
      ...
      if (!pressedBack) {
        ...
        if (isDown) {
          int uniChar = ((struct ENGINE *)app->userData)
                            ->appContext.platform->getKeyFromKeyCode(
                                keyCode, metaState, deviceId);
...
static int32_t handle_touch_input(struct android_app *app, AInputEvent *event) {
  if (AInputEvent_getType(event) != AINPUT_EVENT_TYPE_MOTION)
    return 0;
...
static int32_t engine_handle_input(struct android_app *app,
  ...
  const bool isTouchscreen = ((struct ENGINE *)app->userData)
                                 ->appContext.platform->supportsTouchscreen();
  ;
  if (isTouchscreen) {
...
static void engine_handle_cmd(struct android_app *app, int32_t cmd) {
  ...
  case APP_CMD_INIT_WINDOW:
    ...
    if (engine->app->window != NULL) {
      ...
      // ANativeWindow_setBuffersGeometry(engine->app->window, engine->width,
      // engine->height, 1);
    }
    break;
