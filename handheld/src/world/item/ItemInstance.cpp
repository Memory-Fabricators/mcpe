...
ItemInstance::ItemInstance() { init(0, 0, 0); }

ItemInstance::ItemInstance(const Tile *tile) { init(tile->id, 1, 0); }
...
ItemInstance::ItemInstance(const Item *item) { init(item->id, 1, 0); }
...
bool ItemInstance::isNull() const { return (id | count | auxValue) == 0; }

void ItemInstance::setNull() { id = count = auxValue = 0; }
...
int ItemInstance::getDamageValue() const { return auxValue; }
...
std::string ItemInstance::toString() const {
  std::stringstream ss;
  ss << count << " x " << getDescriptionId() << "(" << id << ")" << "@"
     << auxValue;
  return ss.str();
}
...
bool ItemInstance::matches(const ItemInstance *b) const {
  return (id == b->id) && (count == b->count) && (auxValue == b->auxValue);
}
...
bool ItemInstance::operator==(const ItemInstance &rhs) const {
  return id == rhs.id && auxValue == rhs.auxValue && count == rhs.count;
}
...
int ItemInstance::getUseDuration() { return getItem()->getUseDuration(this); }
