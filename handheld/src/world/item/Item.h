...
public:
  ...
  Item(int id)
      : id(256 + id), craftingRemainingItem(NULL), maxStackSize(MAX_STACK_SIZE),
        maxDamage(32), category(-1), _handEquipped(false),
  ...
  virtual bool canBeDepleted() { return maxDamage > 0 && !_isStackedByData; }

  virtual int getIcon(int auxValue) { return icon; }
  ...
  virtual int getMaxStackSize() { return maxStackSize; }

  virtual int getLevelDataForAuxValue(int auxValue) { return 0; }

  virtual bool isStackedByData() { return _isStackedByData; }
  ...
  virtual int getMaxDamage() { return maxDamage; }
  ...
  virtual int getAttackDamage(Entity *entity) { return 1; }

  virtual bool canDestroySpecial(const Tile *tile) const { return false; }
  ...
  virtual bool isHandEquipped() const { return _handEquipped; }
  ...
  virtual std::string getDescriptionId() const { return descriptionId; }
  ...
  virtual Item *getCraftingRemainingItem() { return craftingRemainingItem; }
