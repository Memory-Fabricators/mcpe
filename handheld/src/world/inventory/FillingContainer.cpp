...
void FillingContainer::setItem(int slot, ItemInstance *item) {
  ...
  if (ItemList *p = getSlotList(slot)) {
    ItemList &pile = *p;
    if (pile[slot])
      *pile[slot] = item ? *item : ItemInstance();
    else
      pile[slot] = new ItemInstance(item ? *item : ItemInstance());
  }
}
  ...
  for (int i = inventoryList->size() - 1; i >= 0; --i) {
    ...
    if (item != NULL) {
      ...
      if (slot < numLinkedSlots) {
        if (slot < (int)items.size() && item->id == MAGIX_VAL &&
            item->count == MAGIX_VAL) {
      ...
      // else if (slot >= 100 && slot < armor.length + 100) armor[slot - 100] =
      // item;
      else {
        if (slot >= MAX_SLOTS) {
...
std::string FillingContainer::getName() const { return "Inventory"; }
...
bool FillingContainer::stillValid(Player *player) { return !player->removed; }

bool FillingContainer::contains(ItemInstance *itemInstance) const {
  for (unsigned int i = 0; i < items.size(); i++) {
    if (items[i] != NULL && items[i]->matches(itemInstance))
      return true;
  }
  return false;
}
...
int FillingContainer::getNonEmptySlot(int tileId) const {
  for (unsigned int i = numLinkedSlots; i < items.size(); i++) {
    if (items[i] != NULL && items[i]->id == tileId && items[i]->count > 0)
      return i;
  }
  return -1;
}
...
int FillingContainer::getNonEmptySlot(int tileId, int data) const {
  for (unsigned int i = numLinkedSlots; i < items.size(); i++) {
    if (items[i] != NULL && items[i]->id == tileId &&
        items[i]->getAuxValue() == data && items[i]->count > 0)
      return i;
  }
  return -1;
}
...
int FillingContainer::getNumLinkedSlots() { return numLinkedSlots; }
...
bool FillingContainer::linkSlot(int selectionSlot, int inventorySlot,
  ...
  if (propagate) {
    ...
    for (; i < numLinkedSlots - 1; ++i)
      if (linkedSlots[i].inventorySlot == inventorySlot)
        break;
