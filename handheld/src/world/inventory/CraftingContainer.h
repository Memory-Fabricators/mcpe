...
public:
  CraftingContainer(/*AbstractContainerMenu* menu,*/ int w, int h)
      : super(ContainerType::WORKBENCH), size(w * h), width(w) /*,
                                                       menu(menu)*/
  {
    items = new ItemInstance[size];
  }

  ~CraftingContainer() { delete[] items; }
  ...
  int getContainerSize() const { return size; }
  int getMaxStackSize() const { return Container::LARGE_MAX_STACK_SIZE; }
  std::string getName() const { return "Crafting"; }

  void setContainerChanged() {}

  bool stillValid(Player *player) { return true; }
