...
public:
  OffsetPosTranslator() : xo(0), yo(0), zo(0) {}
  OffsetPosTranslator(float xo, float yo, float zo) : xo(xo), yo(yo), zo(zo) {}
  void to(float &x, float &y, float &z) {
    x += xo;
    y += yo;
    z += zo;
  }
  void to(int &x, int &y, int &z) {
    x += (int)xo;
    y += (int)yo;
    z += (int)zo;
  }
  void from(float &x, float &y, float &z) {
    x -= xo;
    y -= yo;
    z -= zo;
  }
