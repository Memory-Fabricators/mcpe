...
int MobSpawner::tick(Level *level, bool spawnEnemies, bool spawnFriendlies) {
  ...
  // Spawn friendlies == loop over whole map, and disable Monster spawning this
  // tick
  if (spawnFriendlies) {
    spawnEnemies = false;
    for (int i = 0; i < 256; ++i)
    ...
    // Only spawn mobs, check around one player per tick (@todo: optimize the
    // "count instances of"?)
    static unsigned int _pid = 0;
    if (++_pid >= level->players.size())
      _pid = 0;
  ...
  chunkLoop:
    for (std::map<ChunkPos, bool>::iterator it = chunksToPoll.begin();
      ...
      if (level->isSolidBlockingTile(xStart, yStart, zStart))
        continue;
      ...
      for (int dd = 0; dd < 3; dd++) {
        ...
        for (int ll = 0; ll < 4; ll++) {
          ...
          if (isSpawnPositionOk(mobCategory, level, x, y, z)) {
            ...
            Mob *tmp =
                MobFactory::getStaticTestMob(currentMobType.mobClassId, level);
            if (!tmp)
              continue;
