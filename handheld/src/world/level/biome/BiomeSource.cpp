...
BiomeSource::BiomeSource()
    : temperatureMap(NULL), downfallMap(NULL), noiseMap(NULL),
      lenTemperatures(0), lenDownfalls(0), lenNoises(0), lenBiomes(0),
      temperatures(NULL), downfalls(NULL), noises(NULL), biomes(NULL) {
...
BiomeSource::BiomeSource(Level *level)
    : rndTemperature(level->getSeed() * 9871),
      ...
      rndNoise(level->getSeed() * 543321), lenTemperatures(0), lenDownfalls(0),
      lenNoises(0), lenBiomes(0), temperatures(NULL), downfalls(NULL),
      noises(NULL), biomes(NULL) {
...
Biome **BiomeSource::getBiomeBlock(Biome **biomes__, int x, int z, int w,
  ...
  for (int yy = 0; yy < w; yy++) {
    for (int xx = 0; xx < h; xx++) {
      ...
      float temperature =
          (temperatures[pp] * 0.15f + 0.7f) * split1 + noise * split2;
      split2 = 0.002f;
      split1 = 1 - split2;
...
// float* BiomeSource::getDownfallBlock( /*float* downfalls__,*/ int x, int z,
// int w, int h )
//{
//	//const int size = w * h;
