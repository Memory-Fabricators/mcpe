...
void Biome::recalc() {
  ...
  Biome::iceDesert->topMaterial = Biome::iceDesert->material =
      (char)Tile::sand->id;
}
...
void Biome::initBiomes() {
  ...
  seasonalForest =
      (new Biome())->setColor(0x9BE023)->setName("Seasonal Forest");
...
float Biome::adjustScale(float scale) { return scale; }

float Biome::adjustDepth(float depth) { return depth; }
...
float Biome::getCreatureProbability() { return 0.08f; }
