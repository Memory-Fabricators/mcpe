...
public:
  ...
  int getResourceCount(Random *random) { return 1; }

  int getResource(int data, Random *random) { return Tile::obsidian->id; }
  ...
  void poofParticles(Level *level, int x, int y, int z) {
    ...
    for (int i = 0; i < 6; i++) {
      ...
      if (i == 0 && !level->isSolidBlockingTile(x, y + 1, z))
        yy = y + 1 + r;
      if (i == 1 && !level->isSolidBlockingTile(x, y - 1, z))
        yy = y + 0 - r;
      if (i == 2 && !level->isSolidBlockingTile(x, y, z + 1))
        zz = z + 1 + r;
      if (i == 3 && !level->isSolidBlockingTile(x, y, z - 1))
        zz = z + 0 - r;
      if (i == 4 && !level->isSolidBlockingTile(x + 1, y, z))
        xx = x + 1 + r;
...
private:
  ...
  /*    void wasExploded(Level* level, int x, int y, int z) {
          ...
          ItemEntity* item = new ItemEntity(level, x + xo, y + yo, z + zo, new
     ItemInstance(id)); item->throwTime = 10; level->addEntity(item);
      }*/
};
