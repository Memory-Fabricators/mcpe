...
FurnaceTileEntity::FurnaceTileEntity()
    : super(TileEntityType::Furnace), Container(ContainerType::FURNACE),
      litTime(0), litDuration(0), tickCount(0), finished(false),
...
int FurnaceTileEntity::getContainerSize() const { return NumItems; }
...
bool FurnaceTileEntity::isFinished() { return _canBeFinished && finished; }
...
bool FurnaceTileEntity::stillValid(Player *player) {
  if (level->getTileEntity(x, y, z) != this)
    return false;
  if (player->distanceToSqr(x + 0.5f, y + 0.5f, z + 0.5f) > 8 * 8)
    return false;
  return true;
}
...
bool FurnaceTileEntity::isSlotEmpty(int slot) { return items[slot].isNull(); }
