...
TileEntity::TileEntity(int tileEntityType)
    : data(-1), type(tileEntityType), remove(false), level(NULL), tile(NULL),
      clientSideOnly(false), rendererId(TR_DEFAULT_RENDERER),
...
bool TileEntity::isRemoved() const { return remove; }
...
bool TileEntity::isFinished() { return false; }

Packet *TileEntity::getUpdatePacket() { return NULL; }

bool TileEntity::isType(int Type) { return type == Type; }
