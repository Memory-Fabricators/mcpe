...
ChestTileEntity::ChestTileEntity()
    : super(TileEntityType::Chest),
      ...
      tickInterval(0), openCount(0), openness(0), oOpenness(0),
      hasCheckedNeighbors(false), n(NULL), s(NULL), w(NULL), e(NULL) {
  // rendererId = TR_CHEST_RENDERER;
}

int ChestTileEntity::getContainerSize() const { return ItemsSize; }

ItemInstance *ChestTileEntity::getItem(int slot) { return items[slot]; }
...
void ChestTileEntity::setItem( int slot, ItemInstance* item )
{
        ...
        if (item != NULL && item->count > getMaxStackSize()) item->count =
getMaxStackSize(); this->setChanged();
}
*/

std::string ChestTileEntity::getName() const { return "container.chest"; }
...
void ChestTileEntity::load(CompoundTag *base) {
  ...
  for (int i = 0; i < inventoryList->size(); i++) {
    ...
    if (slot >= 0 && slot < ItemsSize) {
      if (!items[slot])
        items[slot] = new ItemInstance(); //@chestodo
      items[slot]->load(tag);
    }
  }
}
