...
NetherReactorTileEntity::NetherReactorTileEntity()
    : super(TileEntityType::NetherReactor), isInitialized(false), progress(0),
      curLevel(0), hasFinished(false) {}

bool NetherReactorTileEntity::shouldSave() { return true; }
...
std::string NetherReactorTileEntity::getName() const { return "NetherReactor"; }
...
ItemInstance NetherReactorTileEntity::GetLowOddsSpawnItem() {
  if (level->random.nextInt(10) <= 9) {
    ...
    static Tile *tiles[] = {Tile::bookshelf};
...
void NetherReactorTileEntity::tickGlowingRedstoneTransformation(
    int currentTime) {
  switch (currentTime) {
  case 2:
    return turnLayerToGlowingObsidian(0, Tile::stoneBrick->id);
  case 3:
    return turnLayerToGlowingObsidian(1, Tile::stoneBrick->id);
  case 4:
    return turnLayerToGlowingObsidian(2, Tile::stoneBrick->id);
  case 7:
    return turnLayerToGlowingObsidian(0, Tile::goldBlock->id);
  case 8:
    return turnLayerToGlowingObsidian(1, Tile::goldBlock->id);
  case 9:
    return turnLayerToGlowingObsidian(2, Tile::goldBlock->id);
  }
}
    ...
    for (int curZ = -expandWidth; curZ <= expandWidth; ++curZ) {
      ...
      for (int curY = 0; curY < fullHeight + expandWidth; ++curY) {
        if (acceptHeight >= curY &&
            (isEdge(curX, expandWidth, curZ) || acceptHeight == curY)) {
          level->setTile(curX + x, curY + y, curZ + z, tileId);
        }
      }
    }
...
void NetherReactorTileEntity::deterioateCrockedRoofVolume(bool inverted, int x,
  ...
  for (int curX = -expandWidth; curX <= expandWidth; ++curX) {
    for (int curZ = -expandWidth; curZ <= expandWidth; ++curZ) {
      ...
      int acceptHeight = fullHeight + offset;
      for (int curY = 0; curY < fullHeight + expandWidth; ++curY) {
        if (acceptHeight >= curY && (isEdge(curX, expandWidth, curZ))) {
