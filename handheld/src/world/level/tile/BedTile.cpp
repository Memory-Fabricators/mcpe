...
bool BedTile::use(Level *level, int x, int y, int z, Player *player) {
  ...
  if (isOccupied(data)) {
    ...
    if (sleepingPlayer == NULL) {
      ...
      sleepingPlayer->displayClientMessage(
          "This bed is occupied" /*"tile.bed.occupied"*/);
      return true;
    }
  }
  ...
  if (result == BedSleepingResult::NOT_POSSIBLE_NOW) {
    player->displayClientMessage(
        "You can only sleep at night" /*tile.bed.noSleep"*/);
  } else if (result == BedSleepingResult::NOT_SAFE) {
    player->displayClientMessage("You may not rest now, there are monsters "
                                 "nearby" /*"tile.bed.notSafe"*/);
  }
  return true;
}
...
bool BedTile::isSolidRender() { return false; }
...
int BedTile::getRenderLayer() { return Tile::RENDERLAYER_ALPHATEST; }
void BedTile::setShape() { super::setShape(0, 0, 0, 1, 9.0f / 16.0f, 1); }
