...
public:
  TopSnowTile(int id, int tex) : Tile(id, tex, Material::topSnow) {
  ...
  bool mayPlace(Level *level, int x, int y, int z) {
    ...
    if (t == 0 || !Tile::tiles[t]->isSolidRender())
      return false;
  ...
  void playerDestroy(Level *level, Player *player, int x, int y, int z,
    ...
    ItemEntity *item =
        new ItemEntity(level, x + xo, y + yo, z + zo, ItemInstance(type, 1, 0));
    item->throwTime = 10;
    level->addEntity(item);
    level->setTile(x, y, z, 0);
  }

  int getResource(int data, Random *random) { return Item::snowBall->id; }
