...
ChestTile::ChestTile(int id) : super(id, Material::wood) {
  tex = 10 + 16;
  const float m = 0.025f;
  setShape(m, 0, m, 1 - m, 1 - m - m, 1 - m);
}

bool ChestTile::isSolidRender() { return false; }
...
void ChestTile::recalcLockDir(Level *level, int x, int y, int z) {
  ...
  !Tile::solid[e2]) lockDir = 5; if ((Tile::solid[e] || Tile::solid[e2]) &&
  !Tile::solid[w] && !Tile::solid[w2]) lockDir = 4; } else if (w == id || e ==
  id) { int n2 = level->getTile(w == id ? x - 1 : x + 1, y, z - 1); int s2 =
  level->getTile(w == id ? x - 1 : x + 1, y, z + 1);
  ...
  !Tile::solid[s2]) lockDir = 3; if ((Tile::solid[s] || Tile::solid[s2]) &&
  !Tile::solid[n] && !Tile::solid[n2]) lockDir = 2; } else */
  {
    ...
    if ((lockDir == Facing::NORTH && Tile::solid[n]) ||
      ...
      if (Tile::solid[n] && !Tile::solid[s])
        lockDir = Facing::SOUTH;
      if (Tile::solid[s] && !Tile::solid[n])
        lockDir = Facing::NORTH;
...
int ChestTile::getTexture(LevelSource *level, int x, int y, int z, int face) {
  ...
  */
  ...
  if (n == id || s == id) {
          ...
          if ((Tile::solid[w] || Tile::solid[w2]) && !Tile::solid[e] &&
  !Tile::solid[e2]) lockDir = 5; if ((Tile::solid[e] || Tile::solid[e2]) &&
  !Tile::solid[w] && !Tile::solid[w2]) lockDir = 4; return (face == lockDir ?
  tex + 16 : tex + 32) + offs; } else if (w == id || e == id) { if (face == 4 ||
  face == 5) return tex; int offs = 0; if (w == id) { offs = -1;
          }
          ...
          if ((Tile::solid[n] || Tile::solid[n2]) && !Tile::solid[s] &&
  !Tile::solid[s2]) lockDir = 3; if ((Tile::solid[s] || Tile::solid[s2]) &&
  !Tile::solid[n] && !Tile::solid[n2]) lockDir = 2;
      ...
      for (int i = 0; i < container->getContainerSize(); i++) {
        ...
        if (item != NULL) {
          ...
          while (item->count > 0) {
            ...
            // if (item->hasTag()) {
            //     itemEntity->item->setTag((CompoundTag*)
            //     item->getTag()->copy());
            // }
...
bool ChestTile::use(Level *level, int x, int y, int z, Player *player) {
  ...
  // @fullchest
  // if (level->getTile(x - 1, y, z) == id && (level->isSolidBlockingTile(x - 1,
  // y + 1, z))) return true; if (level->getTile(x + 1, y, z) == id &&
  // (level->isSolidBlockingTile(x + 1, y + 1, z))) return true; if
  // (level->getTile(x, y, z - 1) == id && (level->isSolidBlockingTile(x, y + 1,
  // z - 1))) return true; if (level->getTile(x, y, z + 1) == id &&
  // (level->isSolidBlockingTile(x, y + 1, z + 1))) return true;

  // if (level->getTile(x - 1, y, z) == id) container = /*new*/
  // CompoundContainer("Large chest", (ChestTileEntity) level->getTileEntity(x -
  // 1, y, z), container); if (level->getTile(x + 1, y, z) == id) container =
  // /*new*/ CompoundContainer("Large chest", container, (ChestTileEntity)
  // level->getTileEntity(x + 1, y, z)); if (level->getTile(x, y, z - 1) == id)
  // container = /*new*/ CompoundContainer("Large chest", (ChestTileEntity)
  // level->getTileEntity(x, y, z - 1), container); if (level->getTile(x, y, z +
  // 1) == id) container = /*new*/ CompoundContainer("Large chest", container,
  // (ChestTileEntity) level->getTileEntity(x, y, z + 1));
