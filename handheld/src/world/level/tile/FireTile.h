...
public:
  ...
  FireTile(int id, int tex) : Tile(id, tex, Material::fire) {
    for (int i = 0; i < 256; ++i)
  ...
  int getRenderShape() { return Tile::SHAPE_FIRE; }

  int getResourceCount(Random *random) { return 0; }

  int getTickDelay() { return 10; }

  void tick(Level *level, int x, int y, int z, Random *random) {
    return; //@fire

    bool infiniBurn =
        false; // level->getTile(x, y - 1, z) == Tile::hellRock->id;
    ...
    if (!infiniBurn && !isValidFireLocation(level, x, y, z)) {
      if (!level->isSolidBlockingTile(x, y - 1, z) || age > 3)
  ...
  bool isFlammable(int tile) { return flameOdds[tile] > 0; }
  ...
  void ignite(Level *level, int x, int y, int z) {
    ...
    bool lit = false;
    if (!lit)
      lit = tryIgnite(level, x, y + 1, z);
    if (!lit)
      lit = tryIgnite(level, x - 1, y, z);
    if (!lit)
      lit = tryIgnite(level, x + 1, y, z);
    if (!lit)
      lit = tryIgnite(level, x, y, z - 1);
    if (!lit)
      lit = tryIgnite(level, x, y, z + 1);
    if (!lit)
      lit = tryIgnite(level, x, y - 1, z);
  ...
  void animateTick(Level *level, int x, int y, int z, Random *random) {
    ...
    //     level->playSound(x + 0.5f, y + 0.5f, z + 0.5f, "fire.fire", 1 +
    //     random.nextFloat(), random.nextFloat() * 0.7f + 0.3f);
    // }
