...
public:
  ...
  StairTile(int id, Tile *base)
      : super(id, base->tex, base->material), base(base), isClipping(false),
        clipStep(0),
        baseData(
            0) // when needed in the future, set this to necessary texture data
  {
  ...
  int getRenderShape() { return Tile::SHAPE_STAIRS; }
  ...
  int getRenderLayer() { return base->getRenderLayer(); }
  ...
  int getTexture(int face) { return base->getTexture(face, baseData); }

  int getTexture(LevelSource *level, int x, int y, int z, int face) {
    return base->getTexture(face, baseData);
  }

  int getTickDelay() { return base->getTickDelay(); }
  ...
  bool mayPick() { return base->mayPick(); }

  bool mayPick(int data, bool liquid) { return base->mayPick(data, liquid); }
  ...
  void setPlacedBy(Level *level, int x, int y, int z, Mob *by) {
    ...
    if (dir == 0)
      level->setData(x, y, z, 2 | usd);
    if (dir == 1)
      level->setData(x, y, z, 1 | usd);
    if (dir == 2)
      level->setData(x, y, z, 3 | usd);
    if (dir == 3)
      level->setData(x, y, z, 0 | usd);
  }
