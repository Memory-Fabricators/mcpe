...
DoorTile::DoorTile(int id, const Material *material) : super(id, material) {
  tex = 1 + 6 * 16;
...
int DoorTile::getTexture(LevelSource *level, int x, int y, int z, int face) {
  ...
  if (isOpen) {
    ...
    if (dir == 0 && face == 5)
      ...
      texture = -texture;
    if ((compositeData & C_RIGHT_HINGE_MASK) != 0)
      texture = -texture;
  }
...
bool DoorTile::isSolidRender() { return false; }

bool DoorTile::isCubeShaped() { return false; }

int DoorTile::getRenderShape() { return Tile::SHAPE_DOOR; }

int DoorTile::getRenderLayer() { return Tile::RENDERLAYER_ALPHATEST; }
...
void DoorTile::setShape(int compositeData) {
  ...
  if (dir == 0) {
    if (open) {
      if (!hasRightHinge)
        super::setShape(0, 0, 0, 1, 1, r);
      else
        super::setShape(0, 0, 1 - r, 1, 1, 1);
    } else
      super::setShape(0, 0, 0, r, 1, 1);
  } else if (dir == 1) {
    if (open) {
      if (!hasRightHinge)
        super::setShape(1 - r, 0, 0, 1, 1, 1);
      else
        super::setShape(0, 0, 0, r, 1, 1);
    } else
      super::setShape(0, 0, 0, 1, 1, r);
