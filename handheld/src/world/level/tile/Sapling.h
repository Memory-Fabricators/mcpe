...
public:
  Sapling(int id, int tex) : super(id, tex) {
    float ss = 0.4f;
  ...
  void growTree(Level *level, int x, int y, int z, Random *random) {
    ...
    if (data == LeafTile::EVERGREEN_LEAF) {
      ...
      //             for (ox = 0; ox >= -1; ox--) {
      //                 for (oz = 0; oz >= -1; oz--) {
      //                     if (isSapling(level, x + ox, y, z + oz,
      //                     TYPE_JUNGLE) &&
      //                             isSapling(level, x + ox + 1, y, z + oz,
      //                             TYPE_JUNGLE) && isSapling(level, x + ox, y,
      //                             z + oz + 1, TYPE_JUNGLE) &&
      //                             isSapling(level, x + ox + 1, y, z + oz + 1,
      //                             TYPE_JUNGLE)) {
      //                         f = /*new*/ MegaTreeFeature(true, 10 +
      //                         random.nextInt(20), TreeTile::JUNGLE_TRUNK,
      //                         LeafTile::JUNGLE_LEAF); multiblock = true;
      //                         break;
      //                     }
      //                 }
      //                 if (f) {
      ...
      //                 TreeTile::JUNGLE_TRUNK, LeafTile::JUNGLE_LEAF, false);
      //             }
    } else {
...
protected:
  int getSpawnResourcesAuxValue(int data) { return data & TYPE_MASK; }
};
