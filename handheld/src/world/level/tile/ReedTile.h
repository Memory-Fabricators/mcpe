...
public:
  ReedTile(int id, int tex) : Tile(id, Material::plant) {
    this->tex = tex;
    float ss = 6 / 16.0f;
  ...
  bool mayPlace(Level *level, int x, int y, int z) {
    ...
    if (below == id)
      return true;
    if (below != ((Tile *)Tile::grass)->id && below != Tile::dirt->id &&
        below != Tile::sand->id)
      return false;
  ...
  int getResource(int data, Random *random) { return Item::reeds->id; }
  ...
  int getRenderShape() { return Tile::SHAPE_CROSS_TEXTURE; }
