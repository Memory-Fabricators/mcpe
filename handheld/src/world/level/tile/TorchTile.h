...
public:
  TorchTile(int id, int tex) : super(id, tex, Material::decoration) {
  ...
  int getRenderShape() { return Tile::SHAPE_TORCH; }
  ...
  void neighborChanged(Level *level, int x, int y, int z, int type) {
    if (checkCanSurvive(level, x, y, z)) {
      ...
      if (!level->isSolidBlockingTile(x, y, z + 1) && dir == 4)
        replace = true;
      if (!isConnection(level, x, y - 1, z) && dir == 5)
        replace = true;
...
private:
  bool isConnection(Level *level, int x, int y, int z) {
    ...
    if (tile == Tile::fence->id ||
        tile == Tile::glass->id /*|| tile == Tile::cobbleWall->id*/) {
      return true;
    }
    return false;
  }
