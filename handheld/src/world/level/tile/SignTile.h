...
public:
  SignTile(int id, int tileEntityClassId, bool onGround)
      : super(id, Material::wood), tileEntityClassId(tileEntityClassId),
        onGround(onGround) {
    tex = 4;
    float r = 4 / 16.0f;
    float h = 16 / 16.0f;
  ...
  void updateShape(LevelSource *level, int x, int y, int z) {
    ...
    if (face == 2)
      setShape(w0, h0, 1 - d0, w1, h1, 1);
    if (face == 3)
      setShape(w0, h0, 0, w1, h1, d0);
    if (face == 4)
      setShape(1 - d0, h0, w0, 1, h1, w1);
    if (face == 5)
      setShape(0, h0, w0, d0, h1, w1);
  }

  /*@Override*/
  int getRenderShape() { return Tile::SHAPE_INVISIBLE; }

  /*@Override*/
  bool isCubeShaped() { return false; }

  /*@Override*/
  bool isPathfindable(LevelSource *level, int x, int y, int z) { return true; }
