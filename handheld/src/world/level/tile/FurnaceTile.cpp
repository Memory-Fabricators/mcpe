...
FurnaceTile::FurnaceTile(int id, bool lit)
    : super(id, Material::stone), lit(lit) {
  tex = 13 + 16 * 2;
}
...
bool FurnaceTile::use(Level *level, int x, int y, int z, Player *player) {
  ...
  FurnaceTileEntity *furnace =
      static_cast<FurnaceTileEntity *>(level->getTileEntity(x, y, z));
  if (furnace != NULL)
...
void FurnaceTile::setPlacedBy(Level *level, int x, int y, int z, Mob *by) {
  ...
  if (dir == 1)
    level->setData(x, y, z, Facing::EAST);
  if (dir == 2)
    level->setData(x, y, z, Facing::SOUTH);
  if (dir == 3)
    level->setData(x, y, z, Facing::WEST);
}

void FurnaceTile::onRemove(Level *level, int x, int y, int z) {
  if (!noDrop && !level->isClientSide) {
    FurnaceTileEntity *container =
        (FurnaceTileEntity *)level->getTileEntity(x, y, z);
    if (container != NULL) {
