...
static float clampRot(float r) { return Mth::clamp(r, -100000.0f, 100000.0f); }
static float clampXZ(float r) { return Mth::clamp(r, 4.0f, 252.0f); }
static float clampY(float r) { return Mth::clamp(r, 4.0f, 126.0f); }
...
public:
  void loadPlayer(Player *p) const {
    ...
    if (Mth::abs(motionX) > 10.0f)
      motionX = 0;
    if (Mth::abs(motionY) > 10.0f)
      motionY = 0;
    if (Mth::abs(motionZ) > 10.0f)
      motionZ = 0;
