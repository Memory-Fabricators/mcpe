...
Dimension::Dimension()
    : foggy(false), ultraWarm(false), hasCeiling(false), biomeSource(NULL),
      id(0) {}

Dimension::~Dimension() { delete biomeSource; }
...
void Dimension::init() { biomeSource = new BiomeSource(level); }
...
bool Dimension::isValidSpawn(int x, int z) {
  ...
  if (!Tile::tiles[topTile]->isSolidRender())
    return false;
...
void Dimension::updateLightRamp() {
  ...
  for (int i = 0; i <= 15; /*Level::MAX_BRIGHTNESS;*/ i++) {
    ...
    brightnessRamp[i] =
        ((1 - v) / (v * 3 + 1)) * (1 - ambientLight) + ambientLight * 3;
  }
}
...
bool Dimension::mayRespawn() { return true; }

Dimension *Dimension::getNew(int id) {
  if (id == NORMAL)
    return new Dimension();
  if (id == NORMAL_DAYCYCLE)
