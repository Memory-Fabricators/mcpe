...
Feature::Feature(bool doUpdate /*= false*/) : doUpdate(doUpdate) {}
...
void Feature::placeBlock(Level *level, int x, int y, int z, int tile,
  ...
  if (doUpdate) {
    ...
    /*
    } else if (level->hasChunkAt(x, y, z) && level->getChunkAt(x,
    z).seenByPlayer) { if (level->setTileAndDataNoUpdate(x, y, z, tile, data)) {
