...
Level::Level(LevelStorage *levelStorage, const std::string &levelName,
             const LevelSettings &settings, int generatorVersion,
             Dimension *fixedDimension /* = NULL */)
    : levelStorage(levelStorage), isClientSide(false), isFindingSpawn(false),
      noNeighborUpdate(false), skyDarken(0), instaTick(false), random(1),
      _isNew(false), _chunkSource(NULL), _randValue(42184323),
      _addend(1013904223), _maxRecurse(0), _updateLights(true),
      _pathFinder(NULL), _spawnFriendlies(true), _spawnEnemies(true),
      _lastSavedPlayerTime(
          0), // @attn: @time: clock starts on 0 now, change if not
      raknetInstance(0), updatingTileEntities(false),
      allPlayersAreSleeping(false), _nightMode(false) {
...
Level::~Level() {
  ...
  allTileEntities.insert(pendingTileEntities.begin(),
                         pendingTileEntities.end());
  for (std::set<TileEntity *>::iterator it = allTileEntities.begin();
       it != allTileEntities.end(); ++it)
    delete *it;
}

void Level::_init(const std::string &levelName, const LevelSettings &settings,
                  int generatorVersion, Dimension *fixedDimension) {
...
ChunkSource *Level::createChunkSource() {
  ...
  return new ChunkCache(this, chunkStorage,
                        dimension->createRandomLevelSource());
}

/*public*/
bool Level::checkAndHandleWater(const AABB &box, const Material *material,
                                Entity *e) {
...
void Level::tick() {
  if (!isClientSide && levelData.getSpawnMobs()) {
    ...
    if (++_mobSpawnTick >= MobSpawnInterval) {
      ...
      MobSpawner::tick(this, _spawnEnemies && difficulty > Difficulty::PEACEFUL,
                       _spawnFriendlies &&
                           (levelData.getTime() % 400) < MobSpawnInterval);
      TIMER_POP();
    }
  }
  ...
  if (_nightMode) {
    ...
    if (curTime % TICKS_PER_DAY != MIDDLE_OF_NIGHT_TIME) {
      if (curTime % TICKS_PER_DAY < MIDDLE_OF_NIGHT_TIME &&
          (curTime + 20) % TICKS_PER_DAY > MIDDLE_OF_NIGHT_TIME) {
  ...
  //	for (EntityMap::iterator it = _pendingEntityData.begin(); it !=
  //_pendingEntityData.end(); ++it) { 		SetEntityDataPacket packet(it->first,
  //it->second->getEntityData()); 		raknetInstance->send(packet);
  //	}
...
void Level::tickTiles() {
  ...
  static const int pollChunkOffsets[] = {
      -1, -4, 0,  -4, 1,  -4, -2, -3, -1, -3, 0,  -3, 1,  -3, 2,  -3, -3,
      -2, -2, -2, -1, -2, 0,  -2, 1,  -2, 2,  -2, 3,  -2, -4, -1, -3, -1,
      -2, -1, -1, -1, 0,  -1, 1,  -1, 2,  -1, 3,  -1, 4,  -1, -4, 0,  -3,
      0,  -2, 0,  -1, 0,  0,  0,  1,  0,  2,  0,  3,  0,  4,  0,  -4, 1,
      -3, 1,  -2, 1,  -1, 1,  0,  1,  1,  1,  2,  1,  3,  1,  4,  1,  -3,
      2,  -2, 2,  -1, 2,  0,  2,  1,  2,  2,  2,  3,  2,  -2, 3,  -1, 3,
  ...
  for (size_t i = 0; i < players.size(); i++) {
    ...
    for (int i = 0; i < pollChunkOffsetsSize; i += 2) {
      ...
      if (xp >= 0 && xp < CHUNK_CACHE_WIDTH && zp >= 0 &&
          zp < CHUNK_CACHE_WIDTH)
  ...
  TIMER_PUSH("loop");
  for (ChunkPosSet::iterator it = _chunksToPoll.begin();
       it != _chunksToPoll.end(); ++it) {
    ...
    //    x += xo;
    //    z += zo;
    //    if (id == 0 && getRawBrightness(x, y, z) <= random.nextInt(8) &&
    //    getBrightness(LightLayer::Sky, x, y, z) <= 0) {
    //        Player* player = getNearestPlayer(x + 0.5, y + 0.5, z + 0.5, 8);
    //        if (player != NULL && player.distanceToSqr(x + 0.5, y + 0.5, z +
    //        0.5) > 2 * 2) {
    //            //this.playSound(x + 0.5, y + 0.5, z + 0.5,
    //            "ambient.cave.cave", 0.7f, 0.8f + random.nextFloat() * 0.2f);
    //            delayUntilNextMoodSound = random.nextInt(20 * 60 * 10) + 20 *
    //            60 * 5;
    //        }
    //    }
    //}
...
bool Level::findPath(Path *path, Entity *from, Entity *to, float maxDist,
...
bool Level::findPath(Path *path, Entity *from, int xBest, int yBest, int zBest,
...
void Level::setInitialSpawn() {
  isFindingSpawn = true;
  int xSpawn =
      CHUNK_CACHE_WIDTH * CHUNK_WIDTH / 2; // (Level.MAX_LEVEL_SIZE - 100) * 0;
  int ySpawn = 64;
  int zSpawn =
      CHUNK_CACHE_WIDTH * CHUNK_DEPTH / 2; // (Level.MAX_LEVEL_SIZE - 100) * 0;
  while (!dimension->isValidSpawn(xSpawn, zSpawn)) {
    ...
    if (xSpawn < 4)
      xSpawn += 32;
    if (xSpawn >= LEVEL_WIDTH - 4)
      xSpawn -= 32;
    if (zSpawn < 4)
      zSpawn += 32;
    if (zSpawn >= LEVEL_DEPTH - 4)
      zSpawn -= 32;
  }
...
void Level::validateSpawn() {
  ...
  while (getTopTile(xSpawn, zSpawn) == 0 ||
         getTopTile(xSpawn, zSpawn) == Tile::invisible_bedrock->id) {
...
//        saveLevelData();
//
//        if (progressListener != NULL) progressListener.progressStage("Saving
//        chunks"); chunkSource.save(force, progressListener);
//    }
//
...
int Level::getTile(int x, int y, int z) {
  // if (x < -MAX_LEVEL_SIZE || z < -MAX_LEVEL_SIZE || x >= MAX_LEVEL_SIZE || z
  // > MAX_LEVEL_SIZE) {
  //     return 0;
  // }
  if (y < 0)
...
bool Level::hasChunk(int x, int z) { return _chunkSource->hasChunk(x, z); }
...
/*private*/ void Level::neighborChanged(int x, int y, int z, int type) {
  if (noNeighborUpdate || isClientSide)
    return;
  Tile *tile = Tile::tiles[getTile(x, y, z)];
  if (tile != NULL)
...
void Level::updateLightIfOtherThan(const LightLayer &layer, int x, int y, int z,
                                   int expected) {
...
void Level::setBrightness(const LightLayer &layer, int x, int y, int z,
                          int brightness) {
  // if (x < -MAX_LEVEL_SIZE || z < -MAX_LEVEL_SIZE || x >= MAX_LEVEL_SIZE || z
  // > MAX_LEVEL_SIZE) {
  //     return;
...
HitResult Level::clip(const Vec3 &A, const Vec3 &b, bool liquid /*= false*/,
  ...
  if (A.x != A.x || A.y != A.y || A.z != A.z)
    return HitResult(); //@kludge This is actually a way to do it
  ...
  while (maxIterations-- >= 0) {
    ...
    if (solidOnly && tile != NULL &&
      ...
      if (xTile0 >= 0 && zTile0 >= 0 && xTile0 < LEVEL_WIDTH &&
          zTile0 < LEVEL_WIDTH) {
...
void Level::playSound(Entity *entity, const std::string &name, float volume,
                      float pitch) {
  for (unsigned int i = 0; i < _listeners.size(); i++) {
    _listeners[i]->playSound(name, entity->x, entity->y - entity->heightOffset,
                             entity->z, volume, pitch);
  }
}

void Level::playSound(float x, float y, float z, const std::string &name,
...
void Level::levelEvent(Player *source, int type, int x, int y, int z,
                       int data) {
  for (unsigned int i = 0; i < _listeners.size(); i++) {
...
void Level::tileEvent(int x, int y, int z, int b0, int b1) {
  int t = getTile(x, y, z);
  if (t > 0)
    Tile::tiles[t]->triggerEvent(this, x, y, z, b0, b1);
...
void Level::addParticle(const std::string &id, float x, float y, float z,
                        float xd, float yd, float zd, int data /* = 0 */) {
  for (unsigned int i = 0; i < _listeners.size(); i++)
    _listeners[i]->addParticle(id, x, y, z, xd, yd, zd, data);
}

void Level::addParticle(ParticleType::Id id, float x, float y, float z,
                        float xd, float yd, float zd, int data /* = 0 */) {
  for (unsigned int i = 0; i < _listeners.size(); i++)
    _listeners[i]->addParticle(id, x, y, z, xd, yd, zd, data);
}
...
bool Level::addEntity(Entity *e) {
  ...
  if (forced /* || hasChunk(xc, zc)*/) {
    if (e->isPlayer() &&
        std::find(players.begin(), players.end(), e) == players.end()) {
...
Biome::MobSpawnerData Level::getRandomMobSpawnAt(const MobCategory &mobCategory,
                                                 int x, int y, int z) {
  ...
  Biome::MobSpawnerData *data =
      (Biome::MobSpawnerData *)WeighedRandom::getRandomItem(&random, mobList);
  if (!data)
    return Biome::MobSpawnerData();
  return *data;
}
...
std::vector<AABB> &
Level::getCubes(const Entity *source,
                const AABB &box_) { //@attn: check the AABB* new/delete stuff
  boxes.clear();
  const AABB *box = &box_;
  int x0 = Mth::floor(box->x0);
  int x1 = Mth::floor(box->x1 + 1);
  ...
  if (!pendingTileEntities.empty()) {
    for (unsigned int i = 0; i < pendingTileEntities.size(); ++i) {
      ...
      if (!e->isRemoved()) {
        ...
        if (e && lc)
          lc->setTileEntity(xx & 15, yy, zz & 15, e);
...
void Level::setZombieAi(std::vector<Zombie *> &zombies) {
  ...
  for (unsigned int i = 0; i < players.size(); ++i) {
    ...
    std::nth_element(zombies.begin(),
                     zombies.begin() + NumSmartZombiesPerPlayer, zombies.end(),
                     sorter);
...
void Level::tick(Entity *e) { tick(e, true); }
...
void Level::tick(Entity *e, bool actual) {
  ...
  // SANITY!!
  if (e->x != e->x)
    e->x = e->xOld; // @note: checking for NaN, not sure about Infinite
  if (e->y != e->y)
    e->y = e->yOld;
  if (e->z != e->z)
    e->z = e->zOld;
  ...
  for (float xx = 0; xx <= 1; xx += xs)
    for (float yy = 0; yy <= 1; yy += ys)
      for (float zz = 0; zz <= 1; zz += zs) {
        ...
        if (!clip(Vec3(x, y, z), center).isHit())
          hits++;
        count++;
      }
...
void Level::extinguishFire(int x, int y, int z, int face) {
  ...
  if (getTile(x, y, z) == ((Tile *)Tile::fire)->id) {
    // playSound(x + 0.5f, y + 0.5f, z + 0.5f, "random.fizz", 0.5f, 2.6f +
...
void Level::removeTileEntity(int x, int y, int z) {
  ...
  if (te && updatingTileEntities) {
    ...
    LevelChunk *lc = getChunk(x >> 4, z >> 4);
    if (lc)
      lc->removeTileEntity(x & 15, y, z & 15);
...
int Level::getLightsToUpdate() { return _lightUpdates.size(); }
...
void Level::setUpdateLights(bool doUpdate) { _updateLights = doUpdate; }

void Level::updateLight(const LightLayer &layer, int x0, int y0, int z0, int x1,
                        int y1, int z1) {
...
void Level::updateLight(const LightLayer &layer, int x0, int y0, int z0, int x1,
  ...
  if ((dimension->hasCeiling && &layer == &LightLayer::Sky) || !_updateLights)
    return;
  ...
  if (join) {
    ...
    for (int i = 0; i < toCheck; i++) {
      ...
      if (last.layer == &layer &&
          last.expandToContain(x0, y0, z0, x1, y1, z1)) {
        maxLoop--;
        return;
      }
    }
...
//    List<Entity> getEntitiesOfClass(Class<? extends Entity> baseClass, AABB
//    bb) {
//        int xc0 = Mth.floor((bb.x0 - 2) / 16);
//        int xc1 = Mth.floor((bb.x1 + 2) / 16);
//        int zc0 = Mth.floor((bb.z0 - 2) / 16);
...
const EntityList &Level::getAllEntities() { return entities; }
...
//	_entitiesToRemove.insert(_entitiesToRemove.end(), list.begin(),
//list.end());
// }
...
bool Level::mayPlace(int tileId, int x, int y, int z, bool ignoreEntities,
  ...
  if (ignoreEntities)
    aabb = NULL;
  if (aabb != NULL && !isUnobstructed(*aabb))
    return false;
  if (targetTile == Tile::water || targetTile == Tile::calmWater ||
      targetTile == Tile::lava || targetTile == Tile::calmLava ||
      targetTile == (Tile *)(Tile::fire) || targetTile == Tile::topSnow)
    targetTile = NULL;
...
int Level::getSeaLevel() { return SEA_LEVEL; }
...
bool Level::hasDirectSignal(int x, int y, int z) {
  if (getDirectSignal(x, y - 1, z, 0))
    return true;
  if (getDirectSignal(x, y + 1, z, 1))
    return true;
  if (getDirectSignal(x, y, z - 1, 2))
    return true;
  if (getDirectSignal(x, y, z + 1, 3))
    return true;
  if (getDirectSignal(x - 1, y, z, 4))
    return true;
  if (getDirectSignal(x + 1, y, z, 5))
    return true;
  return false;
}
...
bool Level::hasNeighborSignal(int x, int y, int z) {
  if (getSignal(x, y - 1, z, 0))
    return true;
  if (getSignal(x, y + 1, z, 1))
    return true;
  if (getSignal(x, y, z - 1, 2))
    return true;
  if (getSignal(x, y, z + 1, 3))
    return true;
  if (getSignal(x - 1, y, z, 4))
    return true;
  if (getSignal(x + 1, y, z, 5))
    return true;
  return false;
}
...
//    }
//
void Level::setTime(long time) { this->levelData.setTime(time); }

long Level::getSeed() { return levelData.getSeed(); }

long Level::getTime() { return levelData.getTime(); }

Pos Level::getSharedSpawnPos() {
  return Pos(levelData.getXSpawn(), levelData.getYSpawn(),
             levelData.getZSpawn());
}
...
bool Level::mayInteract(Player *player, int xt, int yt, int zt) { return true; }
...
ChunkSource *Level::getChunkSource() { return _chunkSource; }
...
LevelStorage *Level::getLevelStorage() { return levelStorage; }
...
int Level::countInstanceOfBaseType(int baseTypeId) {
  if (baseTypeId < MobTypes::BaseEnemy ||
      baseTypeId > MobTypes::BaseWaterCreature) {
    LOGE("Bad typeId sent to Level::countInstanceOf. Note that only Base types "
         "(MobTypes::Base*) are supported for now.\n");
    return -1;
  }
...
int Level::isNightMode() { return _nightMode; }

void Level::setNightMode(bool isNightMode) { _nightMode = isNightMode; }

bool Level::inRange(int x, int y, int z) {
  return x >= 0 && x < LEVEL_WIDTH && y >= 0 && y < LEVEL_HEIGHT && z >= 0 &&
         z < LEVEL_DEPTH;
}
...
AdventureSettings::AdventureSettings()
    : doTickTime(true), noPvP(false), noPvM(false), noMvP(false),
      immutableWorld(false), showNameTags(false) {}
