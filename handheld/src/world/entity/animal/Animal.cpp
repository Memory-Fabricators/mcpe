...
Animal::Animal(Level *level) : super(level), inLove(0) {
...
float Animal::getWalkTargetValue(int x, int y, int z) {
  if (level->getTile(x, y - 1, z) == ((Tile *)Tile::grass)->id)
    return 10;
...
bool Animal::removeWhenFarAway() { return false; }

int Animal::getCreatureBaseType() const { return MobTypes::BaseCreature; }
