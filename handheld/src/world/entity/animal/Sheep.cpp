...
Sheep::Sheep(Level *level) : super(level), eatAnimationTick(0) {
...
float Sheep::getHeadEatAngleScale(float a) {
  if (eatAnimationTick > 4 && eatAnimationTick <= (EAT_ANIMATION_TICKS - 4)) {
    float scale =
        ((float)(eatAnimationTick - 4) - a) / (float)(EAT_ANIMATION_TICKS - 8);
...
bool Sheep::interact(Player *player) {
  ...
  if (item && item->id == ((Item *)Item::shears)->id && !isSheared() &&
      !isBaby()) {
    if (!level->isClientSide) {
      setSheared(true);
      int count = 1 + random.nextInt(3);
      for (int i = 0; i < count; i++) {
        ItemEntity *ie = spawnAtLocation(
            new ItemInstance(Tile::cloth->id, 1, getColor()), 1);
...
int Sheep::getEntityTypeId() const { return MobTypes::Sheep; }
...
int Sheep::getDeathLoot() { return Tile::cloth->id; }
...
void Sheep::updateAi() {
  ...
  if (!isPathFinding() && eatAnimationTick <= 0 &&
      ((isBaby() && random.nextInt(50) == 0) || random.nextInt(1000) == 0)) {
    ...
    if (/*(level->getTile(xx, yy, zz) == Tile::tallgrass->id &&
        ...
        level->getTile(xx, yy - 1, zz) == ((Tile *)Tile::grass)->id) {
      eatAnimationTick = EAT_ANIMATION_TICKS;
    ...
    Tile::tallgrass->id + TallGrass::TALL_GRASS * 256); level->setTile(xx, yy,
    zz, 0); ate = true; } else */
...
bool Sheep::shouldHoldGround() { return eatAnimationTick > 0; }

const char *Sheep::getAmbientSound() { return "mob.sheep"; }

std::string Sheep::getHurtSound() { return "mob.sheep"; }

std::string Sheep::getDeathSound() { return "mob.sheep"; }
