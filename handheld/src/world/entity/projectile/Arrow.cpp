...
Arrow::Arrow(Level *level) : super(level), playerArrow(false), ownerId(0) {
  _init();
}

Arrow::Arrow(Level *level, float x, float y, float z)
    : super(level), playerArrow(false), ownerId(0) {
  _init();
...
Arrow::Arrow(Level *level, Mob *mob, float power)
    : super(level), ownerId(mob->entityId), playerArrow(mob->isPlayer()) {
  _init();
...
void Arrow::tick() {
  ...
  if (inGround) {
    ...
    if (tile != lastTile || data != lastData) {
      ...
      if (++life == 60 * SharedConstants::TicksPerSecond)
        remove();
      return;
    }
  } else {
  ...
  EntityList &objects =
      level->getEntities(this, this->bb.expand(xd, yd, zd).grow(1, 1, 1));
  float nearest = 0;
  for (unsigned int i = 0; i < objects.size(); i++) {
    Entity *e = objects[i];
    if (!e->isPickable() || (e->entityId == ownerId && flightTime < 5))
      continue;
  ...
  if (res.isHit()) {
    if (res.type == ENTITY) {
      ...
      if (critArrow)
        dmg += sharedRandom.nextInt(dmg / 2 + 2);
...
int Arrow::getAuxData() { return ownerId; }
