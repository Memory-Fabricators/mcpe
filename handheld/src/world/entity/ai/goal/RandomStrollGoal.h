...
public:
  RandomStrollGoal(PathfinderMob *mob, float speed) : mob(mob), speed(speed) {
    setRequiredControlFlags(Control::MoveControlFlag);
  }

  /*@Override*/
  bool canUse() {
    if (mob->getNoActionTime() >= SharedConstants::TicksPerSecond * 5)
      return false;
    if (mob->random.nextInt(120) != 0)
      return false;

    Vec3 pos;
    if (!RandomPos::getPos(pos, mob, 10, 7))
      return false;
    wantedX = pos.x;
    wantedY = pos.y;
    wantedZ = pos.z;
  ...
  /*@Override*/
  bool canContinueToUse() { return !mob->getNavigation()->isDone(); }
