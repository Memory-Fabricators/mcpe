...
public:
  ...
  bool canUse() {
    if (randomInterval > 0 && mob->random.nextInt(randomInterval) != 0)
      return false;
    Mob *potentialTarget = NULL;
    if (targetType == 1)
      potentialTarget =
          mob->level->getNearestPlayer(mob, within); //@todo: targetType
    // else potentialTarget = (Mob*)
    // mob->level->getClosestEntityOfClass(targetType, mob->bb.grow(within, 4,
    // within), mob);
  ...
  void start() { mob->setTarget(target); }
