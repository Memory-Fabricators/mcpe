...
public:
  ...
  TargetGoal(Monster *mob, float within, bool mustSee)
      : mob(mob), within(within), mustSee(mustSee) {}
  ...
  bool canContinueToUse() {
    ...
    if (mob->distanceToSqr(target) > within * within)
      return false;
    if (mustSee && !mob->sensing->canSee(target))
      return false;
    return true;
  }

  void stop() { mob->setTarget(NULL); }
...
protected:
  bool canAttack(Mob *target, bool allowInvulnerable) {
    ...
    if (!target->isAlive())
      return false;
    if (target->bb.y1 <= mob->bb.y0 || target->bb.y0 >= mob->bb.y1)
      return false;
    // if (target instanceof Creeper || target instanceof Ghast) return false;

    if (target->isPlayer()) {
      if (!allowInvulnerable && ((Player *)target)->abilities.invulnerable)
        return false;
    }
    if (mustSee && !mob->sensing->canSee(target))
      return false;
