...
public:
  PathNavigation(Mob *mob, Level *level, float maxDist)
      : mob(mob), level(level), maxDist(maxDist), _tick(0), avoidSun(false),
        avoidWater(false), canOpenDoors(false), lastStuckCheckPos(0, 0, 0),
        speed(0), lastStuckCheck(0), path(NULL), deletePath(false) {}
  ~PathNavigation() { deletePathIfNeeded(); }
...
private:
  ...
  void updatePath() {
    ...
    if (_tick - lastStuckCheck > 100) {
      if (mobPos.distanceToSqr(lastStuckCheckPos) < 1.5 * 1.5)
        stop();
      lastStuckCheck = _tick;
      lastStuckCheckPos.x = mobPos.x;
      lastStuckCheckPos.y = mobPos.y;
  ...
  Vec3 getTempMobPos() { return Vec3(mob->x, (float)getSurfaceY(), mob->z); }

  int getSurfaceY() {
    if (!mob->isInWater())
      return (int)(mob->bb.y0 + 0.5);
    ...
    while (tileId == Tile::water->id || tileId == Tile::calmWater->id) {
      ...
      if (++steps > 16)
        return (int)(mob->bb.y0);
    }
    return surface;
  }

  bool canUpdatePath() { return mob->onGround || isInLiquid(); }
      ...
      for (int yy = startY; yy < startY + sy; yy++) {
        for (int zz = startZ; zz < startZ + sz; zz++) {
          ...
          if (Tile::tiles[tile]->material->blocksMotion())
            return false;
          // if (!Tile::tiles[tile]->isPathfindable(level, xx, yy, zz)) return
          // false; //@todo
        }
      }
    }
