...
private:
  static bool generateRandomPos(Vec3 &outPos, PathfinderMob *mob, int xzDist,
    ...
    //             float restDist = mob->getRestrictCenter().dist((int)
    //             Math.floor(mob->x), (int) Math.floor(mob->y), (int)
    //             Math.floor(mob->z)) + 4; restrict = restDist <
    //             mob->getRestrictRadius() + xzDist;
    //         } else restrict = false;
    ...
    for (int i = 0; i < 10; i++) {
      ...
      if (dir != NULL && xt * dir->x + zt * dir->z < 0)
        continue;
