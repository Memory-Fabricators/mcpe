...
Zombie::Zombie(Level *level)
    : super(level), fireCheckTick(0), _useNewAi(false) {
  ...
  targetSelector->addGoal(
      2, new NearestAttackableTargetGoal(this, 1, 16, 0, true));
...
int Zombie::getEntityTypeId() const { return MobTypes::Zombie; }

void Zombie::setUseNewAi(bool use) { _useNewAi = use; }
...
const char *Zombie::getAmbientSound() { return "mob.zombie"; }

std::string Zombie::getHurtSound() { return "mob.zombiehurt"; }

std::string Zombie::getDeathSound() { return "mob.zombiedeath"; }
...
bool Zombie::useNewAi() { return _useNewAi; }
...
int Zombie::getAttackDamage(Entity *target) {
  ...
  if (weapon != NULL)
    damage += weapon->getAttackDamage(this);
  return damage;
}
