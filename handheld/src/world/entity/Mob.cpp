...
Mob::Mob(Level *level)
    : super(level), invulnerableDuration(20),
      // hasHair(false),
      textureName("mob/char.png"), allowAlpha(true), modelName(""),
      bobStrength(1), deathScore(0), renderOffset(0),
      // interpolateOnly(false),
      lookingAtId(0), jumping(false), defaultLookAngle(0.0f), runSpeed(0.7f),
      walkingSpeed(0.1f), flyingSpeed(0.02f), run(0), oRun(0), animStep(0),
      animStepO(0), lx(0), ly(0), lz(0), lxr(0), lyr(0), lSteps(0),
      walkAnimSpeed(0), walkAnimSpeedO(0), walkAnimPos(0), attackAnim(0),
      oAttackAnim(0),

      autoSendPosRot(true), sentX(0), sentY(0), sentZ(0), sentRotX(0),
      sentRotY(0), sentXd(0), sentYd(0), sentZd(0), arrowCount(0),
      removeArrowTime(0), speed(0), moveControl(NULL), jumpControl(NULL),
      navigation(NULL), goalSelector(NULL), targetSelector(NULL), sensing(NULL),
...
float Mob::getWalkingSpeedModifier() {
  ...
  //	speed *= 1.0f + .2f * (getEffect(MobEffect.movementSpeed).getAmplifier()
  //+ 1);
  // }
  // if (hasEffect(MobEffect.movementSlowdown)) {
  //	speed *= 1.0f - .15f *
  //(getEffect(MobEffect.movementSlowdown).getAmplifier() + 1);
  // }
  return speed;
}
...
std::string Mob::getTexture() { return textureName; }
...
void Mob::baseTick() {
  ...
  if (((ambientSoundTime++ & 15) == 0) &&
      random.nextInt(2000) < ambientSoundTime) {
...
void Mob::spawnAnim() {
  ...
  float dd = 10;
  level.addParticle(PARTICLETYPE(explode), x + random.nextFloat() * bbWidth * 2
  - bbWidth - xa * dd, y + random.nextFloat() * bbHeight - ya * dd, z +
  random.nextFloat() * bbWidth * 2 - bbWidth - za
  * dd, xa, ya, za);
  }
  */
}
...
void Mob::setSize(float w, float h) { super::setSize(w, h); }
...
const char *Mob::getAmbientSound() { return NULL; }

std::string Mob::getHurtSound() { return "random.hurt"; }

std::string Mob::getDeathSound() { return "random.hurt"; }
...
void Mob::aiStep() {
  ...
  if (player == NULL || !player->isSleeping()) {
    EntityList &entities =
        level->getEntities(this, this->bb.grow(0.2f, 0, 0.2f));
  ...
  if (lookingAtId != 0) {
    ...
    if (lookingAt != NULL) {
      ...
      if (lookTime-- <= 0 || lookingAt->removed ||
          lookingAt->distanceToSqr(this) > lookDistance * lookDistance) {
        lookingAtId = 0;
      } else {
...
void Mob::newServerAiStep() {
  ...
  if (moveControl)
    moveControl->tick(); //@todo; we shouldn't be here if not
  if (jumpControl)
    jumpControl->tick();
...
int Mob::getMaxHeadXRot() { return 10; }
...
bool Mob::isLookingAtAnEntity() { return lookingAtId != 0; }
...
bool Mob::interpolateOnly() { return level->isClientSide; }
...
SynchedEntityData *Mob::getEntityData() { return &entityData; }

const SynchedEntityData *Mob::getEntityData() const { return &entityData; }
...
void Mob::checkDespawn() { checkDespawn(level->getNearestPlayer(this, -1)); }
...
void Mob::checkDespawn(Mob *nearestBlocking) {
  if (nearestBlocking != NULL) {
    ...
    if (noActionTime > 30 * SharedConstants::TicksPerSecond &&
        random.nextInt(800) == 0 && removeIfFar && sd > 32 * 32)
      remove();
    else
      noActionTime = 0;
  }
...
bool Mob::getSharedFlag(int flag) {
  return entityData.getFlag<SharedFlagsInformation::SharedFlagsInformationType>(
      SharedFlagsInformation::DATA_SHARED_FLAGS_ID, flag);
}
void Mob::setSharedFlag(int flag, bool value) {
  if (value) {
    entityData.setFlag<SharedFlagsInformation::SharedFlagsInformationType>(
        SharedFlagsInformation::DATA_SHARED_FLAGS_ID, flag);
  } else {
