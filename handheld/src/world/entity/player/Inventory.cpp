...
ItemInstance *Inventory::getSelected() { return getLinked(selected); }
...
int Inventory::getSelectionSize() { return MAX_SELECTION_SIZE; }
...
void Inventory::setupDefault() {
  ...
  if (_isCreative) {
    ...
    addItem(
        new ItemInstance(Tile::quartzBlock, 1, QuartzBlockTile::TYPE_LINES));
    addItem(
        new ItemInstance(Tile::quartzBlock, 1, QuartzBlockTile::TYPE_CHISELED));
...
bool Inventory::canDestroy(Tile *tile) {
  if (tile->material->isAlwaysDestroyable())
    return true;
...
bool Inventory::stillValid(Player *player) {
  if (this->player->removed)
    return false;
  if (player->distanceToSqr(this->player) > 8 * 8)
    return false;
  return true;
}
