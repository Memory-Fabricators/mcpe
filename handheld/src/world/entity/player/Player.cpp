...
Player::Player(Level *level, bool isCreative)
    : super(level), userType(0), playerHasRespawnPosition(false),
      hasFakeInventory(false), containerMenu(NULL), useItemDuration(0),
      playerIsSleeping(false), sleepCounter(0), bedOffsetX(0), bedOffsetY(0),
      bedOffsetZ(0), respawnPosition(0, -1, 0), allPlayersSleeping(false) {
  canRemove = false;
...
int Player::startSleepInBed(int x, int y, int z) {
  ...
  entityData.setFlag<SharedFlagsInformation::SharedFlagsInformationType>(
      DATA_PLAYER_FLAGS_ID, PLAYER_SLEEP_FLAG);
...
void Player::stopSleepInBed(bool forcefulWakeUp, bool updateLevelList,
  ...
  // Quick fix to make the spawn position always saved, not sure if we always
...
int Player::getSleepTimer() { return allPlayersSleeping ? sleepCounter : 0; }
...
void Player::tick() {
  bool shouldSleep =
      entityData.getFlag<SharedFlagsInformation::SharedFlagsInformationType>(
          DATA_PLAYER_FLAGS_ID, PLAYER_SLEEP_FLAG);
...
// Use items
//
bool Player::isUsingItem() { return !useItem.isNull(); }

ItemInstance *Player::getUseItem() { return &useItem; }
...
/*protected*/
bool Player::isImmobile() { return health <= 0 || isSleeping(); }
...
/*protected*/
void Player::updateAi() { updateAttackAnim(); }
...
void Player::aiStep() {
  if (level->difficulty == Difficulty::PEACEFUL && health < MAX_HEALTH) {
    if (tickCount % (12 * SharedConstants::TicksPerSecond) == 0)
      heal(1);
  }
  // inventory.tick();
  oBob = bob;
...
/*private*/
void Player::touch(Entity *entity) { entity->playerTouch(this); }
...
float Player::getWalkingSpeedModifier() { return 1.0f; }

void Player::awardKillScore(Entity *victim, int score) { this->score += score; }
...
void Player::drop(ItemInstance *item, bool randomly) {
  ...
  if (randomly) {
    ...
    pow = 0.3f;
    thrownItem->xd =
        -Mth::sin(yRot / 180 * Mth::PI) * Mth::cos(xRot / 180 * Mth::PI) * pow;
    thrownItem->zd =
        Mth::cos(yRot / 180 * Mth::PI) * Mth::cos(xRot / 180 * Mth::PI) * pow;
...
void Player::readAdditionalSaveData(CompoundTag *entityTag) {
  ...
  if (entityTag->contains("Sleeping") && entityTag->contains("SleepTimer") &&
      entityTag->contains("BedPositionX") &&
      entityTag->contains("BedPositionY") &&
      entityTag->contains("BedPositionZ")) {
...
//     if (entityTag.contains("SpawnX") && entityTag.contains("SpawnY") &&
//     entityTag.contains("SpawnZ")) {
//         return /*new*/ Pos(entityTag.getInt("SpawnX"),
...
bool Player::isHurt() { return health > 0 && health < getMaxHealth(); }
...
// level->raknetInstance->send(owner, new AnimatePacket(AnimatePacket::Swing,
// this));
//}
//}
...
// }
ItemInstance *Player::getCarriedItem() { return inventory->getSelected(); }
...
//@Override
bool Player::isInWall() { return super::isInWall(); }

bool Player::hasResource(int id) { return inventory->hasResource(id); }
...
void Player::setRespawnPosition(
    const Pos &respawnPosition) { // @attn WARNING CHECK THIS THROUGH @fix
                                  // @todo: rewrite
...
/*static*/
bool Player::isPlayer(Entity *e) { return e && e->isPlayer(); }

Player *Player::asPlayer(Entity *e) { return isPlayer(e) ? (Player *)e : NULL; }
...
void Player::tileEntityDestroyed(int tileEntityId) {
  ...
  // ((FurnaceMenu*)containerMenu)->furnaceTileEntityId);
...
bool Player::canUseCarriedItemWhileMoving() {
  ...
  return item && (item->id == Item::bow->id || item->getItem()->isFood());
}
...
ItemInstance *Player::getSelectedItem() { return inventory->getSelected(); }

bool Player::hasRespawnPosition() { return playerHasRespawnPosition; }
