...
void move_surface(App *app, AppContext *state, uint32_t x, uint32_t y,
  ...
  static const EGLint context_attributes[] = {EGL_CONTEXT_CLIENT_VERSION, 1,
                                              EGL_NONE};
  EGLConfig config;
  ...
  while (SDL_PollEvent(&event)) {
    ...
    if (SDL_KEYDOWN == event.type) {
      ...
      /*
                              if (SDLK_ESCAPE == key) {
                                      if (SDL_GRAB_ON ==
         SDL_WM_GrabInput(SDL_GRAB_QUERY)) { setGrabbed(false);
                                      }
                              }
      */
      // printf("KeyDown: %d => %d\n", key, transformed);
    ...
    if (SDL_MOUSEBUTTONUP == event.type) {
      bool left = SDL_BUTTON_LEFT == event.button.button;
      char button =
          left ? 1 : 2; // MouseAction::ACTION_LEFT : MouseAction::ACTION_RIGHT;
    ...
    if (SDL_ACTIVEEVENT == event.type) {
      if (SDL_APPACTIVE & event.active.state)
        _app_window_normal = event.active.gain;
      // printf("state: %d (%d), %d, %d, %d\n", event.active.state,
      // event.active.gain, SDL_APPACTIVE, SDL_APPMOUSEFOCUS,
      // SDL_APPINPUTFOCUS);
    }
    /*
