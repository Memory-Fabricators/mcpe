...
const Options::Option Options::Option::MUSIC(0, "options.music", true, false),
...
void Options::load() {
  ...
  //     FileReader(optionsFile)); std::string line = ""; while ((line =
  //     br.readLine()) != NULL) {
  //         std::string[] cmds = line.split(":");
  //         if (cmds[0].equals("music")) music = readFloat(cmds[1]);
  //         if (cmds[0].equals("sound")) sound = readFloat(cmds[1]);
  //         if (cmds[0].equals("mouseSensitivity")) sensitivity =
  //         readFloat(cmds[1]); if (cmds[0].equals("invertYMouse"))
  //         invertYMouse = cmds[1].equals("true"); if
  //         (cmds[0].equals("viewDistance")) viewDistance =
  //         Integer.parseInt(cmds[1]); if (cmds[0].equals("guiScale")) guiScale
  //         = Integer.parseInt(cmds[1]); if (cmds[0].equals("bobView")) bobView
  //         = cmds[1].equals("true"); if (cmds[0].equals("anaglyph3d"))
  //         anaglyph3d = cmds[1].equals("true"); if
  //         (cmds[0].equals("limitFramerate")) limitFramerate =
  //         cmds[1].equals("true"); if (cmds[0].equals("difficulty"))
  //         difficulty = Integer.parseInt(cmds[1]); if
  //         (cmds[0].equals("fancyGraphics")) fancyGraphics =
  //         cmds[1].equals("true"); if (cmds[0].equals("ao")) ambientOcclusion
  //         = cmds[1].equals("true"); if (cmds[0].equals("skin")) skin =
  //         cmds[1]; if (cmds[0].equals("lastServer") && cmds.length >= 2)
  //         lastMpIp = cmds[1];
...
bool Options::readBool(const std::string &string, bool &value) {
  ...
  if (string == "false" || string == "0" || string == "NO") {
    value = false;
    return true;
  }
  return false;
