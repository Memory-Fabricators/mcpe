...
Gui::Gui(Minecraft *minecraft)
    : minecraft(minecraft), tickCount(0), progress(0), overlayMessageTime(0),
      animateOverlayMessageColor(false), tbr(1), _inventoryNeedsUpdate(true),
      _flashSlotId(-1), _flashSlotStartTime(-1), _slotFont(NULL), _numSlots(4),
      _currentDropTicks(-1), _currentDropSlot(-1), MAX_MESSAGE_WIDTH(240),
...
// font->drawShadow("This is a demo, not the finished product", 2, 10 + 2,
// 0xffffffff);
...
bool Gui::isInside(int x, int y) { return getSlotIdAt(x, y) != -1; }

int Gui::getNumSlots() { return _numSlots; }
...
RectangleArea Gui::getRectangleArea(int extendSide) {
  ...
  const float pHalfWidth =
      (1.0f + (getNumSlots() * 10 + Spacing)) * Gui::GuiScale;
...
void Gui::onGraphicsReset() { inventoryUpdated(); }

void Gui::texturesLoaded(Textures *textures) {
  //_slotFont = new Font(&minecraft->options, "gui/gui_blocks.png", textures, 0,
  // 504, 10, 1, '0');
}
...
void Gui::renderHearts() {
  ...
  for (int i = 0; i < Player::MAX_HEALTH / 2; i++) {
    ...
    if (armor > 0) {
      int xo = xx + 80 + i * 8 + 4;
      if (ip2 < armor)
        blit(xo, yo, 16 + 2 * 9, 9 * 1, 9, 9);
      else if (ip2 == armor)
        blit(xo, yo, 16 + 4 * 9, 9 * 1, 9, 9);
      else if (ip2 > armor)
        blit(xo, yo, 16 + 0 * 9, 9 * 1, 9, 9);
    }
    ...
    if (blink) {
      if (ip2 < oh)
        blit(xo, yo, 16 + 6 * 9, 9 * 0, 9, 9);
      else if (ip2 == oh)
        blit(xo, yo, 16 + 7 * 9, 9 * 0, 9, 9);
    }
    if (ip2 < h)
      blit(xo, yo, 16 + 4 * 9, 9 * 0, 9, 9);
    else if (ip2 == h)
      blit(xo, yo, 16 + 5 * 9, 9 * 0, 9, 9);
  }
}
...
void Gui::renderBubbles() {
  if (minecraft->player->isUnderLiquid(Material::water)) {
    ...
    for (int i = 0; i < count + extra; i++) {
      int xo = i * 8 + 2;
      if (i < count)
        blit(xo, yo, 16, 9 * 2, 9, 9);
      else
        blit(xo, yo, 16 + 9, 9 * 2, 9, 9);
    }
  }
}
...
void Gui::renderToolBar(float a, int ySlot, const int screenWidth) {
  ...
  // Render damaged items (@todo: investigate if it's faster by drawing in same
  // batch)
  ...
  for (int i = 0; i < getNumSlots() - 1; i++) {
    ItemRenderer::renderGuiItemDecorations(
        minecraft->player->inventory->getItem(i), x, (float)ySlot);
    x += 20;
  }
  t.endOverrideAndDraw();
  glEnable(GL_DEPTH_TEST);
