...
Button::Button(int id, const std::string &msg)
    : GuiElement(true, true, 0, 0, 200, 24), id(id), msg(msg), selected(false),
      _currentlyDown(false) {}

Button::Button(int id, int x, int y, const std::string &msg)
    : GuiElement(true, true, x, y, 200, 24), id(id), msg(msg), selected(false),
      _currentlyDown(false) {}

Button::Button(int id, int x, int y, int w, int h, const std::string &msg)
    : GuiElement(true, true, x, y, w, h), id(id), msg(msg), selected(false),
      _currentlyDown(false) {}
...
bool Button::hovered(Minecraft *minecraft, int xm, int ym) {
  return minecraft->useTouchscreen() ? (_currentlyDown && isInside(xm, ym))
                                     : false;
}
...
// BlankButton
//
BlankButton::BlankButton(int id) : super(id, "") { visible = false; }
...
TButton::TButton(int id, const std::string &msg) : super(id, msg) {
  width = 66;
  height = 26;
}
...
THeader::THeader(int id, int x, int y, int w, int h, const std::string &msg)
    : super(id, x, y, w, h, msg), xText(-99999) {
  active = false;
}
