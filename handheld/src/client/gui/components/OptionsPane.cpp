...
void OptionsPane::createToggle(unsigned int group, std::string label,
...
void OptionsPane::createProgressSlider(Minecraft *minecraft, unsigned int group,
                                       std::string label,
                                       const Options::Option *option,
                                       float progressMin /*=1.0f*/,
                                       float progressMax /*=1.0f */) {
  // 	if(group > children.size()) return;
  // 	Slider* element = new Slider(minecraft, option, progressMin,
  // progressMax); 	element->width = 100; 	element->height = 20; 	OptionsItem* item
  // = new OptionsItem(label, element);
  // 	((OptionsGroup*)children[group])->addChild(item);
  // 	setupPositions();
}

void OptionsPane::createStepSlider(Minecraft *minecraft, unsigned int group,
                                   std::string label,
                                   const Options::Option *option,
