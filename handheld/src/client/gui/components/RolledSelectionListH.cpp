...
RolledSelectionListH::RolledSelectionListH(Minecraft *minecraft, int width,
    ...
    : minecraft(minecraft), width(width), height(height), x0((float)x0),
      ...
      renderSelection(true), doRenderHeader(false), headerWidth(0),
      dragState(DRAG_OUTSIDE), xDrag(0.0f), xo(0.0f), xoo(0.0f), xInertia(0.0f),
      _componentSelected(false), _renderTopBorder(true),
      _renderBottomBorder(true), _lastxoo(0), _xinertia(0) {
  ...
  if (Mouse::isButtonDown(MouseAction::ACTION_LEFT)) {
    ...
    if (ym >= y0 && ym <= y1) {
      if (dragState == NO_DRAG) {
        ...
        // float localX = (float)(xm*Gui::InvGuiScale - x0 - xo + lastSelection
    ...
    if (dragState >= 0) {
      if (dragState >= 0) {
        xInertia = _xinertia < 0 ? Mth::Max(-20.0f, _xinertia)
                                 : Mth::Min(20.0f, _xinertia);
      }
      // LOGI("Inertia: %f. Time: %d, delta-x: %d, (xm, sel: %d, %d)\n",
      // xInertia, getTimeMs() - lastSelectionTime, std::abs(selectionX - xm),
      // xm, selectionX);
      ...
      if (std::abs(xInertia) <= 10 && getTimeMs() - lastSelectionTime < 300) {
        ...
        // selectionX, xm);
        if (slot >= 0 && slot == lastSelection &&
            std::abs(selectionX - xm) < 10)
...
void RolledSelectionListH::renderHoleBackground(
