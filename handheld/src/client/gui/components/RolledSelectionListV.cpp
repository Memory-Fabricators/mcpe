...
RolledSelectionListV::RolledSelectionListV(Minecraft *minecraft_, int width_,
    ...
    : minecraft(minecraft_), width(width_), height(height_), x0((float)x0_),
      ...
      renderSelection(true), doRenderHeader(false), headerHeight(0),
      dragState(DRAG_OUTSIDE), yDrag(0.0f), yo(0.0f), yoo(0.0f), yInertia(0.0f),
      _componentSelected(false), _renderDirtBackground(true),
      _renderTopBorder(true), _renderBottomBorder(true), _lastyoo(0),
      _yinertia(0), _stickPixels(0), _lastxm(0), _lastym(0) {
  ...
  if (Mouse::isButtonDown(MouseAction::ACTION_LEFT)) {
    ...
    if (ym >= y0 && ym <= y1) {
      if (dragState == NO_DRAG) {
        ...
        lastSelection =
            convertSelection(getItemAtPosition(width / 2, ym), xm, ym);
    ...
    if (dragState >= 0) {
      if (dragState >= 0) {
        yInertia = _yinertia < 0 ? Mth::Max(-10.0f, _yinertia)
                                 : Mth::Min(10.0f, _yinertia);
      }
      ...
      if (std::abs(yInertia) <=
          10 /*&& getTimeMs() - lastSelectionTime < 300 */) {
        ...
        // selectionX, xm);
        if (xm >= x0 && xm <= x1 && slot >= 0 && slot == lastSelection &&
            std::abs(selectionY - ym) < 10)
...
void RolledSelectionListV::renderHoleBackground(
