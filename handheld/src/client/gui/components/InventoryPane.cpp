...
InventoryPane::~InventoryPane() { delete _clickArea; }
...
void InventoryPane::renderBatch(std::vector<GridItem> &items, float alpha) {
  ...
  if (!mc->isCreativeMode()) {
    ...
    for (unsigned int i = 0; i < items.size(); ++i) {
      ...
      float tx =
          Gui::floorAlignToScreenPixel(kText * (item.xf + BorderPixels + 3));
      float ty =
          Gui::floorAlignToScreenPixel(kText * (item.yf + BorderPixels + 3));
  ...
  // fillGradient(bbox.x - 1, bbox.y, bbox.x + bbox.w + 1, bbox.y + 20,
