...
Screen::Screen()
    : passEvents(false), clickedButton(NULL), tabButtonIndex(0), width(1),
      height(1), minecraft(NULL), font(NULL) {}
...
bool Screen::handleBackEvent(bool isDown) { return false; }
...
void Screen::keyboardTextEvent() { keyboardNewChar(Keyboard::getChar()); }
...
bool Screen::closeOnPlayerHurt() { return false; }
...
void Screen::keyPressed(int eventKey) {
  ...
  if (eventKey == o.keyMenuNext.key)
    if (++tabButtonIndex == tabButtonCount)
      tabButtonIndex = 0;
  if (eventKey == o.keyMenuPrevious.key)
    if (--tabButtonIndex == -1)
      tabButtonIndex = tabButtonCount - 1;
  ...
  if (buttonNum == MouseAction::ACTION_LEFT) {
    for (unsigned int i = 0; i < buttons.size(); ++i) {
      ...
      if (button->clicked(minecraft, x, y)) {
        ...
        /*
        #if !defined(ANDROID) && !defined(__APPLE__) //if
        (!minecraft->isTouchscreen()) {
                                                minecraft->soundEngine->playUI("random.click",
        1, 1); buttonClicked(button); #endif }
        */
      }
    }
  }
...
void Screen::mouseReleased(int x, int y, int buttonNum) {
  // LOGI("b_id: %d, (%p), text: %s\n", buttonNum, clickedButton,
  // clickedButton?clickedButton->msg.c_str():"<null>");
...
bool Screen::renderGameBehind() { return true; }

bool Screen::hasClippingArea(IntRectangle &out) { return false; }
