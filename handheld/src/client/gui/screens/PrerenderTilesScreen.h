...
public:
  void init() {
    ...
    for (unsigned int i = 0; i < recipes.size(); ++i) {
      ...
      std::vector<ItemInstance> required =
          recipes[i]->getItemPack().getItemInstances();
  ...
  void render(int xm, int ym, float a) {
    ...
    LOGI("--------------------\n");
    /*int j = 0;
    for (int i = Inventory::MAX_SELECTION_SIZE; i <
    inventory->getContainerSize(); ++i) {
            ...
            if (item->id < 256 &&
    TileRenderer::canRender(Tile::tiles[item->id]->getRenderShape())) { LOGI("0,
    %d, %d, %d, 0\n", j, item->id, item->getAuxValue());
                    ItemRenderer::renderGuiItemCorrect(minecraft->font,
    minecraft->textures, item, x, y); } else if (item->getIcon() >= 0) {
                    LOGI("1, %d, %d, %d, %d\n", j, item->id,
    item->getAuxValue(), item->getIcon());
            }
            ++j;
    }*/
    int j = 0;
  ...
  void addItem(ItemInstance *item) {
    ...
    for (std::vector<ItemInstance>::iterator i = mItems.begin();
      ...
      if (jitem->id != item->id)
        continue;
      if (jitem->isStackedByData() &&
          jitem->getAuxValue() != item->getAuxValue())
        continue;
