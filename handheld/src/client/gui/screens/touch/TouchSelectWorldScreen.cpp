...
float TouchWorldSelectionList::getPos(float alpha) {
  if (mode != 1)
    return RolledSelectionListH::getPos(alpha);
...
SelectWorldScreen::SelectWorldScreen()
    : bDelete(1, ""), bCreate(2, "Create new"), bBack(3, "Back"),
      bHeader(0, "Select world"), bWorldView(4, ""), worldsList(NULL),
      _hasStartedLevel(false), _state(_STATE_DEFAULT) {
...
SelectWorldScreen::~SelectWorldScreen() { delete worldsList; }
  ...
  if (button->id == bCreate.id) {
    if (_state == _STATE_DEFAULT && !_hasStartedLevel) {
      minecraft->platform()->createUserInput(
          DialogDefinitions::DIALOG_CREATE_NEW_WORLD);
  ...
  if (_state == _STATE_CREATEWORLD) {
    ...
    if (status > -1) {
      if (status == 1) {
        ...
        // Read the level name.
        // 1) Trim name 2) Remove all bad chars 3) Append '-' chars 'til the
        // name is unique
        ...
        for (int i = 0; i < sizeof(ILLEGAL_FILE_CHARACTERS) / sizeof(char); ++i)
          levelId = Util::stringReplace(
              levelId, std::string(1, ILLEGAL_FILE_CHARACTERS[i]), "");
