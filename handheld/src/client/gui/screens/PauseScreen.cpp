...
PauseScreen::PauseScreen(bool wasBackPaused)
    : saveStep(0), visibleTime(0), bContinue(0), bQuit(0),
      bQuitAndSaveLocally(0), bServerVisibility(0),
      //	bThirdPerson(0),
      wasBackPaused(wasBackPaused), bSound(&Options::Option::SOUND, 1, 0),
...
void PauseScreen::init() {
  ...
  for (unsigned int i = 0; i < buttons.size(); ++i) {
    if (buttons[i] == &bSound)
      continue;
    if (buttons[i] == &bThirdPerson)
      continue;
    if (buttons[i] == &bHideGui)
      continue;
...
void PauseScreen::setupPositions() {
  ...
  bQuitAndSaveLocally->x = bServerVisibility->x =
      (width - bQuitAndSaveLocally->width) / 2;
...
void PauseScreen::buttonClicked(Button *button) {
  ...
  if (button->id == bServerVisibility->id) {
    if (minecraft->raknetInstance && minecraft->netCallback &&
        minecraft->raknetInstance->isServer()) {
