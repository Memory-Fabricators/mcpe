...
IngameBlockSelectionScreen::IngameBlockSelectionScreen()
    : selectedItem(0), _area(0, 0, 0, 0), _pendingQuit(false), InventoryRows(1),
      InventoryCols(1), InventorySize(1), bArmor(1, "Armor") {}
...
void IngameBlockSelectionScreen::renderSlot(int slot, int x, int y, float a) {
  ...
  if (minecraft->gameMode->isCreativeType())
    return;
  if (!isAllowed(slot - Inventory::MAX_SELECTION_SIZE))
    return;
...
#ifdef RPI
  if (eventKey == o.keyMenuCancel.key || eventKey == Keyboard::KEY_ESCAPE)
...
void IngameBlockSelectionScreen::mouseClicked(int x, int y, int buttonNum) {
  if (buttonNum == MouseAction::ACTION_LEFT) {
    ...
    if (isAllowed(slot)) {
      ...
      _pendingQuit =
          !_area.isInside((float)x, (float)y) && !bArmor.isInside(x, y);
    }
  }
  if (!_pendingQuit)
...
void IngameBlockSelectionScreen::selectSlotAndClose() {
  ...
  // inventory->moveToSelectedSlot(selectedItem + Inventory::MAX_SELECTION_SIZE,
  // true);
  inventory->moveToSelectionSlot(
      0, selectedItem + Inventory::MAX_SELECTION_SIZE, true);
