...
PlayerRenderer::PlayerRenderer(HumanoidModel *humanoidModel, float shadow)
    : super(humanoidModel, shadow), armorParts1(new HumanoidModel(1.0f)),
...
void PlayerRenderer::renderName(Mob *mob, float x, float y, float z) {
  ...
  if (mob != entityRenderDispatcher->cameraEntity &&
      mob->level->adventureSettings.showNameTags) {
...
int PlayerRenderer::prepareArmor(Mob *mob, int layer, float a) {
  ...
  int fnIndex =
      (armorItem->modelIndex + armorItem->modelIndex) + (layer == 2 ? 1 : 0);
...
void PlayerRenderer::onGraphicsReset() {
  ...
  if (armorParts1)
    armorParts1->onGraphicsReset();
  if (armorParts2)
    armorParts2->onGraphicsReset();
}
