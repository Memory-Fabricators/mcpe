...
TileRenderer::TileRenderer(LevelSource *level /* = NULL */)
    : level(level), fixedTexture(-1), xFlipTexture(false), noCulling(false),
      blsmooth(1), applyAmbienceOcclusion(false) {}
...
bool TileRenderer::tesselateBlockInWorld(Tile *tt, int x, int y, int z, float r,
  ...
  if (noCulling || tt->shouldRenderFace(level, x, y + 1, z, Facing::UP)) {
    float br = tt->getBrightness(level, x, y + 1, z);
    if (tt->yy1 != 1 && !tt->material->isLiquid())
      br = centerBrightness;
...
bool TileRenderer::tesselateCrossInWorld(Tile *tt, int x, int y, int z) {
  ...
  tesselateCrossTexture(tt, level->getData(x, y, z), (float)x, (float)y,
  (float)z); return true;*/
}
...
bool TileRenderer::tesselateBedInWorld(Tile *tt, int x, int y, int z) {
  ...
  if (isHead) {
    skipEdge =
        Direction::DIRECTION_FACING[Direction::DIRECTION_OPPOSITE[direction]];
  }
...
void TileRenderer::renderTile(Tile *tile, int data) {
  ...
  if (shape == Tile::SHAPE_BLOCK) {
    ...
    for (int i = 0; i < 2; i++) {
      if (i == 0)
        tile->setShape(0, 0, 0, 1, 1, 0.5f);
      if (i == 1)
        tile->setShape(0, 0, 0.5f, 1, 0.5f, 1);
    ...
    for (int i = 0; i < 4; i++) {
      float w = 2 / 16.0f;
      if (i == 0)
        tile->setShape(0.5f - w, 0, 0, 0.5f + w, 1, w * 2);
      if (i == 1)
        tile->setShape(0.5f - w, 0, 1 - w * 2, 0.5f + w, 1, 1);
      w = 1 / 16.0f;
      if (i == 2)
    ...
    for (int i = 0; i < 3; i++) {
      float w = 1 / 16.0f;
      if (i == 0)
        tile->setShape(0.5f - w, .3f, 0, 0.5f + w, 1, w * 2);
      if (i == 1)
        tile->setShape(0.5f - w, .3f, 1 - w * 2, 0.5f + w, 1, 1);
      if (i == 2)
        tile->setShape(0.5f - w, .5f, w * 2, 0.5f + w, 1 - w, 1 - w * 2);
...
bool TileRenderer::canRender(int renderShape) {
  ...
  if (renderShape == Tile::SHAPE_CACTUS)
    return true;
  if (renderShape == Tile::SHAPE_STAIRS)
    return true;
  if (renderShape == Tile::SHAPE_FENCE)
    return true;
  if (renderShape == Tile::SHAPE_FENCE_GATE)
    return true;
...
void TileRenderer::renderGuiTile(Tile *tile, int data) {
  ...
  if (shape == Tile::SHAPE_BLOCK) {
    ...
    for (int i = 0; i < 3; i++) {
      ...
      w = 1 / 16.0f;
      if (i == 2)
        tile->setShape(0.5f - w, .5f, 0, 0.5f + w, 1 - w, 1);
