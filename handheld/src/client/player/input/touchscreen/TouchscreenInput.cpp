...
TouchscreenInput_TestFps::TouchscreenInput_TestFps(Minecraft *mc,
    ...
    : _minecraft(mc), _options(options), _northJump(false), _forward(false),
      ...
      aUpLeft(0), aUpRight(0), _allowHeightChange(false) {
  releaseAllKeys();
  onConfigChanged(createConfig(mc));

  Tesselator &t = Tesselator::instance;
  const int alpha = 128;
  t.color(0xc0c0c0, alpha);
  cPressed = t.getColor();
  t.color(0xffffff, alpha);
  cReleased = t.getColor();
  t.color(0xffffff, alpha / 4);
  cDiscreet = t.getColor();
  t.color(0xc0c0c0, 80);
  cPressedPause = t.getColor();
  t.color(0xffffff, 80);
  cReleasedPause = t.getColor();
}

TouchscreenInput_TestFps::~TouchscreenInput_TestFps() { clear(); }

void TouchscreenInput_TestFps::clear() {
  _model.clear();

  delete aUpLeft;
  aUpLeft = NULL; // @todo: SAFEDEL
  delete aUpRight;
  aUpRight = NULL;
}
...
void TouchscreenInput_TestFps::onConfigChanged(const Config &c) {
  ...
  */
  ...
  const float BaseX = _options->isLeftHanded ? -8 + w - 3 * Bw : 8 + 0;
  // Setup the bounding rectangle
  _boundingRectangle =
      RectangleArea(BaseX, BaseY, BaseX + 3 * Bw, BaseY + 3 * Bh);
...
void TouchscreenInput_TestFps::setKey(int key, bool state) {
  ...
  int id = -1;
  if (key == _options->keyUp.key)
    id = KEY_UP;
  if (key == _options->keyDown.key)
    id = KEY_DOWN;
  if (key == _options->keyLeft.key)
    id = KEY_LEFT;
  if (key == _options->keyRight.key)
    id = KEY_RIGHT;
  if (key == _options->keyJump.key)
    id = KEY_JUMP;
  if (key == _options->keySneak.key)
    id = KEY_SNEAK;
  if (key == _options->keyCraft.key)
    id = KEY_CRAFT;
  if (id >= 0) {
    _keys[id] = state;
  }
...
void TouchscreenInput_TestFps::rebuild() {
  ...
  // aDown, aJump, _bufferId); t.setAccessMode(Tesselator::ACCESS_DYNAMIC);
  t.begin();
  ...
  // render left button
  if (northDiagonals || isChangingFlightHeight)
    t.colorABGR(cDiscreet);
  else if (isButtonDown(AREA_DPAD_W))
  ...
  if (northDiagonals || isChangingFlightHeight)
    t.colorABGR(cDiscreet);
  else if (isButtonDown(AREA_DPAD_E))
    t.colorABGR(cPressed);
  else
    t.colorABGR(cReleased);
  ...
  if (northDiagonals)
    t.colorABGR(cDiscreet);
  else if (isButtonDown(AREA_DPAD_S))
    t.colorABGR(cPressed);
  else
    t.colorABGR(cReleased);
  if (isChangingFlightHeight) {
  ...
  if (_renderFlightImage && northDiagonals)
    t.colorABGR(cDiscreet);
  else if (isButtonDown(AREA_DPAD_C))
    t.colorABGR(cPressed);
  else
    t.colorABGR(cReleased);
  ...
  if (!_minecraft->screen) {
    if (isButtonDown(AREA_PAUSE))
      t.colorABGR(cPressedPause);
    else
      t.colorABGR(cReleasedPause);
