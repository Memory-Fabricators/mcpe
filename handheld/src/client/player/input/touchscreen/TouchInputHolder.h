...
template <class T> class ModifyNotify {
public:
  ModifyNotify() : _changed(false) {}
  ...
  virtual bool equals(const T &newVal) { return _old != newVal; }
...
public:
  ...
  UnifiedTurnBuild(int turnMode, int width, int height, float maxMovementDelta,
      ...
      : mode(turnMode), _holder(holder), _options(&minecraft->options), cxO(0),
        ...
        screenArea(-1, -1, 0, 0), allowPicking(false), state(State_None),
        moveArea(-1, -1, 0, 0), joyTouchArea(-1, -1, 0, 0),
        inventoryArea(-1, -1, 0, 0), pauseArea(-1, -1, 0, 0), _buildMovement(0),
        _sentFirstRemove(false), _canDestroy(false), _forceCanUse(false) {
  ...
  virtual void onConfigChanged(const Config &c) {
    if (false && _options->isJoyTouchArea) {
      ...
      const float widen =
          (moveArea._x1 - moveArea._x0) * 0.05f + border; // ~5% wider
      moveArea._x0 -= widen;
      moveArea._x1 += widen;
      const float heighten =
          (moveArea._y1 - moveArea._y0) * 0.05f + border; // ~5% taller
      ...
      // LOGI("move: %f, %f, %f, %f\n", moveArea._x0, moveArea._y0,
      // moveArea._x1, moveArea._y1);
...
#endif /*__APPLE__*/
      // LOGI("Movearea: %f %f %f% f\n", moveArea._x0, moveArea._x1,
      // moveArea._y0, moveArea._y1);
  ...
  TurnDelta getTurnDelta() {
    ...
    if (isActive && !wasActive) {
      ...
      _totalMoveDelta = 0;
      bool isInMovement =
          _lastPlayer ? getSpeedSquared(_lastPlayer) > 0.01f /*.25f*/ : false;
      // state = isInMovement? State_Turn : State_Deciding;
      state = State_Deciding;
      _canDestroy = !isInMovement;
      _forceCanUse = false;
      if (!_canDestroy &&
          (_lastPlayer && _lastPlayer->canUseCarriedItemWhileMoving())) {
        _forceCanUse = true;
        _canDestroy = true;
      }
    ...
    if (MODE_DELTA == mode && (wasActive || isActive)) {
      ...
      if (isActive) {
        ...
        if (state == State_Deciding &&
            _totalMoveDelta >= MaxBuildMovement /* && !_forceCanUse*/)
          state = State_Turn;
  ...
  bool isInsideArea(float x, float y) { return _area.isInside(x, y); }
  ...
  virtual bool tickBuild(Player *player, BuildActionIntention *bai) {
    ...
    if (state == State_Destroy) {
      if (!_sentFirstRemove) {
        *bai = BuildActionIntention(
            (_forceCanUse ? 0 : BuildActionIntention::BAI_FIRSTREMOVE) |
            BuildActionIntention::BAI_INTERACT);
        _sentFirstRemove = true;
      } else {
        *bai = BuildActionIntention(
            (_forceCanUse ? 0 : BuildActionIntention::BAI_REMOVE) |
            BuildActionIntention::BAI_INTERACT);
      }
      return true;
    }
    ...
    while (Multitouch::next()) {
      ...
      if (m.action == MouseAction::ACTION_MOVE)
        continue;
      ...
      // MouseAction::DATA_DOWN), z && (m.data == MouseAction::DATA_UP), m.x,
      // m.y);

      if (_totalMoveDelta <= MaxBuildMovement &&
          (m.data == MouseAction::DATA_UP && !handled)) {
...
public:
...
#endif
    _turnBuild.inventoryArea =
        _mc->gui.getRectangleArea(_mc->options.isLeftHanded ? 1 : -1);
  ...
  virtual void render(float alpha) { _turnBuild.render(alpha); }
