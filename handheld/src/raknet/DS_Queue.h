...
template <class queue_type> inline bool Queue<queue_type>::IsEmpty(void) const {
  return head == tail;
}
...
void Queue<queue_type>::Push(const queue_type &input, const char *file,
  ...
  if (tail == head) {
    ...
    new_array =
        RakNet::OP_NEW_ARRAY<queue_type>(allocation_size * 2, file, line);
#ifdef _DEBUG
    RakAssert(new_array);
#endif
    if (new_array == 0)
...
template <class queue_type> bool Queue<queue_type>::Find(queue_type q) {
  if (allocation_size == 0)
    return false;
