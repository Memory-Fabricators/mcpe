...
public:
  ...
  /// Set according to your available bandwidth and the estimated average
  /// bandwidth per forwarded address. A single connection requires 2 entries,
  /// as connections are bi-directional.
  /// \param[in] maxEntries The maximum number of simultaneous forwarding
  /// entries. Defaults to 64 (32 connections)
  void SetMaxForwardEntries(unsigned short maxEntries);

  /// \return The \a maxEntries parameter passed to SetMaxForwardEntries(), or
  /// the default if it was never called
  int GetMaxForwardEntries(void) const;

  /// \note Each call to StartForwarding uses up two forwarding entries, since
  ...
  /// \pre Call Startup()
  /// \note RakNet's protocol will ensure a message is sent at least every 15
  /// seconds, so if routing RakNet messages, it is a reasonable value for
  /// timeoutOnNoDataMS, plus an some extra seconds for latency
  /// \param[in] source The source IP and port
  /// \param[in] destination Where to forward to (and vice-versa)
  /// \param[in] timeoutOnNoDataMS If no messages are forwarded for this many
  /// MS, then automatically remove this entry. Currently hardcoded to
  /// UDP_FORWARDER_MAXIMUM_TIMEOUT (else the call fails)
  /// \param[in] forceHostAddress Force binding on a particular address. 0 to
  /// use any.
