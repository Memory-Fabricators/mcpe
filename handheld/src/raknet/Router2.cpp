...
1. Sender calls ConnectInternal(). A ConnnectRequest structure is allocated and
stored in the connectionRequests list, containing a list of every system we are
connected to. ID_ROUTER_2_QUERY_FORWARDING is sent to all connected systems.

2. Upon the router getting ID_ROUTER_2_QUERY_FORWARDING,
ID_ROUTER_2_REPLY_FORWARDING is sent to the sender indicating if that router is
connected to the endpoint, along with the ping from the router to the endpoint.

3. Upon the sender getting ID_ROUTER_2_REPLY_FORWARDING, the connection request
structure is looked up in Router2::UpdateForwarding. The ping is stored in that
structure. Once all systems have replied, the system continues to the next
state. If every system in step 1 has been exhausted, and routing has occured at
least once, then ID_CONNECTION_LOST is returned. If every system in step 1 has
been exhausted and routing has never occured, then
ID_ROUTER_2_FORWARDING_NO_PATH is returned. Otherwise, the router with the
lowest ping is chosen, and RequestForwarding() is called with that system, which
sends ID_ROUTER_2_REQUEST_FORWARDING to the router.

4. When the router gets ID_ROUTER_2_REQUEST_FORWARDING, a MiniPunchRequest
structure is allocated and stored in the miniPunchesInProgress list. The
function SendOOBMessages() sends ID_ROUTER_2_REPLY_TO_SENDER_PORT from the
routing sockets to both the sender and endpoint. It also sends
...
ID_ROUTER_2_REPLY_TO_SPECIFIED_PORT depending on what type of router they have.
If ID_ROUTER_2_REPLY_TO_SENDER_PORT arrives, then this will reply back to the
routing socket directly. If ID_ROUTER_2_REPLY_TO_SPECIFIED_PORT arrives, then
the reply port is modified to be the port specified by the router system. In
both cases, ID_ROUTER_2_MINI_PUNCH_REPLY is sent. As the router has already
setup the forwarding, ID_ROUTER_2_MINI_PUNCH_REPLY will actually arrive to the
endpoint from the sender, and from the sender to the endpoint.

6. When ID_ROUTER_2_MINI_PUNCH_REPLY arrives,
ID_ROUTER_2_MINI_PUNCH_REPLY_BOUNCE will be sent to the router. This is to tell
the router that the forwarding has succeeded.

7. When ID_ROUTER_2_MINI_PUNCH_REPLY_BOUNCE arrives on the router, the router
will find the two systems in the miniPunchesInProgress list, which was added in
step 4 (See OnMiniPunchReplyBounce()). gotReplyFromSource or
gotReplyFromEndpoint will be set to true, depending on the sender. When both
gotReplyFromSource and gotReplyFromEndpoint have replied, then
ID_ROUTER_2_REROUTE is sent to the endpoint, and
ID_ROUTER_2_FORWARDING_ESTABLISHED is sent to the sender.

8. When the endpoint gets ID_ROUTER_2_REROUTE, the system address is updated for
the guid of the sender using RakPeer::ChangeSystemAddress(). This happens in
OnRerouted().

9. When the sender gets ID_ROUTER_2_FORWARDING_ESTABLISHED, then in
OnForwardingSuccess() the endpoint is removed from the connectionRequest list
and moved to the forwardedConnectionList.

10. In OnClosedConnection(), for the sender, if the closed connection is the
endpoint, then the endpoint is removed from the forwardedConnectionList (this is
a graceful disconnect). If the connection was instead lost to the router, then
ConnectInternal() gets called, which goes back to step 1. If instead this was a
connection requset in progress, then UpdateForwarding() gets called, which goes
back to step 3.

11. When the user connects the endpoint and sender, then the sender will get
ID_CONNECTION_REQUEST_ACCEPTED. The sender will look up the endpoint in the
forwardedConnectionList, and send ID_ROUTER_2_INCREASE_TIMEOUT to the endpoint.
This message will call SetTimeoutTime() on the endpoint, so that if the router
...
bool Router2::ConnectInternal(RakNetGUID endpointGuid,
  ...
  if (largestPing < 0) {
    ...
    if (debugInterface)
      debugInterface->ShowFailure(FormatStringTS(
          buff, "Router2: ConnectInternal(%I64d) failed at %s:%i\n",
    ...
    if (guids[i] != endpointGuid) {
      ...
      if (debugInterface) {
        ...
        debugInterface->ShowDiagnostic(FormatStringTS(
            buff, "Router2::ConnectInternal: at %s:%i, pack_id = %d", __FILE__,
            __LINE__, pack_id));
      }
      ...
      if (debugInterface) {
        char buff[512];
        debugInterface->ShowDiagnostic(FormatStringTS(
            buff, "Router2::ConnectInternal: at %s:%i [else ..].: %I64d==%I64d",
  ...
  if (debugInterface) {
    char buff[512];
    debugInterface->ShowDiagnostic(FormatStringTS(
        buff, "Broadcasting ID_ROUTER_2_QUERY_FORWARDING to %I64d at %s:%i\n",
        endpointGuid.g, __FILE__, __LINE__));
  }
...
PluginReceiveResult Router2::OnReceive(Packet *packet) {
  ...
  if (packet->data[0] == ID_ROUTER_2_INTERNAL) {
    ...
    case ID_ROUTER_2_REQUEST_FORWARDING: {
      ...
      if (debugInterface) {
        ...
        debugInterface->ShowDiagnostic(FormatStringTS(
            buff, "Got ID_ROUTER_2_REQUEST_FORWARDING on ip %s from %I64d, ",
            buff2, packet->guid.g));
      }
    ...
    case ID_ROUTER_2_INCREASE_TIMEOUT: {
      /// The routed system wants more time to stay alive on no communication,
      /// in case the router drops or crashes
  ...
  } else if (packet->data[0] == ID_OUT_OF_BAND_INTERNAL &&
             packet->length >= 2) {
    switch (packet->data[1]) {
    case ID_ROUTER_2_REPLY_TO_SENDER_PORT: {
      RakNet::BitStream bsOut;
      bsOut.Write(packet->guid);
      SendOOBFromRakNetPort(ID_ROUTER_2_MINI_PUNCH_REPLY, &bsOut,
                            packet->systemAddress);
      ...
      if (debugInterface) {
        ...
        debugInterface->ShowDiagnostic(FormatStringTS(
            buff,
            "Got ID_ROUTER_2_REPLY_TO_SENDER_PORT %i on address %s, replying "
            "with ID_ROUTER_2_MINI_PUNCH_REPLY at %s:%i\n",
    ...
    if (forwardingIndex < forwardedConnectionList.Size()) {
      ...
      // Have the endpoint take longer to drop us, in case the intermediary
      // system drops
      RakNet::BitStream bsOut;
      bsOut.Write((MessageID)ID_ROUTER_2_INTERNAL);
      bsOut.Write((unsigned char)ID_ROUTER_2_INCREASE_TIMEOUT);
      rakPeerInterface->Send(&bsOut, HIGH_PRIORITY, RELIABLE, 0, packet->guid,
                             false);
    ...
    if (connectionRequest->requestState ==
      ...
      if (anyRemoved) {
        if (connectionRequestIndex != (unsigned int)-1) {
          // connectionRequestsMutex should be locked before calling this
          // function
...
void Router2::OnClosedConnection(
  ...
  while (forwardedConnectionIndex < forwardedConnectionList.Size()) {
    if (forwardedConnectionList[forwardedConnectionIndex].endpointGuid ==
      ...
      if (debugInterface) {
        ...
        debugInterface->ShowDiagnostic(
            FormatStringTS(buff,
                           "Closed connection to the %I64d, removing "
                           "forwarding from list at %s:%i\n",
    ...
    } else if (forwardedConnectionList[forwardedConnectionIndex]
                       .intermediaryGuid == rakNetGUID &&
               forwardedConnectionList[forwardedConnectionIndex]
                   .weInitiatedForwarding) {
      // Lost connection to intermediary. Restart process to connect to
      // endpoint. If failed, push ID_CONNECTION_LOST. Also remove connection
      // request if it already is in the list, to restart it
      connectionRequestsMutex.Lock();
      ...
      // This should not be removed - the connection is still forwarded, but
      // perhaps through another system
      //			forwardedConnectionList.RemoveAtIndexFast(forwardedConnectionIndex);
    } else
      forwardedConnectionIndex++;
  }
    ...
    if (connectionRequestGuidIndex != (unsigned int)-1) {
          ...
          false) // If returns false, no connection request systems left
      {
        ...
        RemoveConnectionRequest(connectionRequestIndex);
             ...
             // cr->requestState==R2RS_REQUEST_STATE_QUERY_FORWARDING then we
             // are still waiting for other systems to respond.
      {
        if (debugInterface) {
          char buff[512];
          debugInterface->ShowDiagnostic(FormatStringTS(
              buff,
              "Aborted connection attempt to %I64d, restarting forwarding to "
              "%I64d at %s:%i\n",
              rakNetGUID.g, cr->endpointGuid.g, __FILE__, __LINE__));
        }
        //                 if(volatile bool is_my_fix_a_truth = true) { // A
        //                 system in the list of potential systems to try
        //                 routing to dropped. There is no need to restart the
        //                 whole process.
        //                     connectionRequestsMutex.Lock();
  ...
  while (forwardedConnectionIndex < forwardedConnectionList.Size()) {
    if (forwardedConnectionList[forwardedConnectionIndex].intermediaryAddress ==
      ...
      if (debugInterface) {
        ...
        debugInterface->ShowDiagnostic(FormatStringTS(
            buff,
            "Failed connection attempt to forwarded system (%I64d : %s) at "
            "%s:%i\n",
...
bool Router2::UpdateForwarding(ConnnectRequest *connectionRequest) {
  ...
  //   RAKNET_DEBUG_PRINTF(__FUNCTION__":
  //   connectionRequest->connectionRequestSystems.Size = %d",
  //   connectionRequest->connectionRequestSystems.Size());
  ...
  if (connectionRequest->connectionRequestSystems.Size() == 0) {
    ...
    if (connectionRequest->returnConnectionLostOnFailure) {
      ReturnToUser(ID_CONNECTION_LOST, connectionRequest->endpointGuid,
                   ...
                   true); // This is a connection which was previously
                          // established. Rerouting is not possible.
      ...
      //             RAKNET_DEBUG_PRINTF(__FUNCTION__": call
      //             rakPeerInterface->CloseConnection(%I64d)" ,
      //             connectionRequest->endpointGuid.g);
    } else
      ReturnToUser(ID_ROUTER_2_FORWARDING_NO_PATH,
                   connectionRequest->endpointGuid, UNASSIGNED_SYSTEM_ADDRESS,
                   false);
  ...
  if (connectionRequest->requestState == R2RS_REQUEST_STATE_QUERY_FORWARDING) {
    ...
    for (unsigned int i = 0;
         i < connectionRequest->connectionRequestSystems.Size(); i++) {
...
void Router2::RemoveConnectionRequest(unsigned int connectionRequestIndex) {
  RakNet::OP_DELETE(connectionRequests[connectionRequestIndex],
                    _FILE_AND_LINE_);
  connectionRequests.RemoveAtIndexFast(connectionRequestIndex);
}
...
void Router2::RequestForwarding(ConnnectRequest *connectionRequest) {
  RakAssert(connectionRequest->requestState ==
            R2RS_REQUEST_STATE_QUERY_FORWARDING);
  connectionRequest->requestState = REQUEST_STATE_REQUEST_FORWARDING;
...
int Router2::ReturnFailureOnCannotForward(RakNetGUID sourceGuid,
  ...
  // We cannot forward connections which are themselves forwarded. Return fail
  // in that case
  forwardedConnectionListMutex.Lock();
  for (unsigned int i = 0; i < forwardedConnectionList.Size(); i++) {
    if ((forwardedConnectionList[i].endpointGuid == endpointGuid) ||
        (forwardedConnectionList[i].endpointGuid ==
         sourceGuid)) // sourceGuid is here so you do not respond to routing
                      // requests from systems you are already routing through.
    {
      forwardedConnectionListMutex.Unlock();
...
void Router2::OnQueryForwarding(Packet *packet) {
  ...
  if (pingToEndpoint == -1) {
    ...
    if (debugInterface)
      debugInterface->ShowFailure(
          FormatStringTS(buff, "Router2 failed (%I64d) at %s:%i\n",
                         packet->guid.g, __FILE__, __LINE__));
    return;
  }
  ...
  if (debugInterface) {
    char buff[512];
    debugInterface->ShowDiagnostic(FormatStringTS(
        buff, "Sending ID_ROUTER_2_REPLY_FORWARDING to the %I64d at %s:%i\n",
        packet->guid.g, __FILE__, __LINE__));
  }
}
void Router2::OnQueryForwardingReply(Packet *packet) {
  ...
  if (debugInterface) {
    ...
    debugInterface->ShowDiagnostic(FormatStringTS(
        buff,
        "Got ID_ROUTER_2_REPLY_FORWARDING on address %s(%I64d -> %I64d) "
        "canForward=%i at %s:%i\n",
...
void Router2::SendOOBFromRakNetPort(OutOfBandIdentifiers oob,
                                    BitStream *extraData, SystemAddress sa) {
...
void Router2::SendOOBFromSpecifiedSocket(OutOfBandIdentifiers oob,
                                         SystemAddress sa, SOCKET socket) {
  RakNet::BitStream bs;
  rakPeerInterface->WriteOutOfBandHeader(&bs);
  bs.Write((unsigned char)oob);
  SocketLayer::SendTo_PC(socket, (const char *)bs.GetData(),
                         bs.GetNumberOfBytesUsed(), sa, __FILE__, __LINE__);
}
void Router2::SendOOBMessages(Router2::MiniPunchRequest *mpr) {
  // Mini NAT punch
  // Send from srcToDestPort to packet->systemAddress (source). If the message
  // arrives, the remote system should reply.
  SendOOBFromSpecifiedSocket(ID_ROUTER_2_REPLY_TO_SENDER_PORT,
                             mpr->sourceAddress, mpr->forwardingSocket);

  // Send from destToSourcePort to endpointSystemAddress (destination). If the
  // message arrives, the remote system should reply.
  SendOOBFromSpecifiedSocket(ID_ROUTER_2_REPLY_TO_SENDER_PORT,
                             mpr->endpointAddress, mpr->forwardingSocket);
  ...
  SendOOBFromRakNetPort(ID_ROUTER_2_REPLY_TO_SPECIFIED_PORT, &extraData,
                        mpr->sourceAddress);
...
void Router2::OnRequestForwarding(Packet *packet) {
  ...
  if (pingToEndpoint == -1) {
    ...
    if (debugInterface)
      debugInterface->ShowFailure(
          FormatStringTS(buff,
                         "Router2 failed (packet->guid =%I64d, endpointGuid = "
                         "%I64d) at %s:%i\n",
  ...
  if (result == UDPFORWARDER_FORWARDING_ALREADY_EXISTS) {
    if (debugInterface) {
      ...
      debugInterface->ShowDiagnostic(FormatStringTS(
          ...
          "result=UDPFORWARDER_FORWARDING_ALREADY_EXISTS "
          "(packet->guid =%I64d, endpointGuid = %I64d) at %s:%i\n",
          packet->guid.g, endpointGuid.g, __FILE__, __LINE__));
    }

    SendForwardingSuccess(ID_ROUTER_2_FORWARDING_ESTABLISHED, packet->guid,
                          endpointGuid, forwardingPort);
    ...
    if (debugInterface)
      debugInterface->ShowFailure(FormatStringTS(
          buff,
          "Router2 failed at %s:%i with UDPFORWARDER_NO_SOCKETS, "
    ...
    if (debugInterface)
      debugInterface->ShowFailure(FormatStringTS(
          buff,
          "Router2 failed at %s:%i with UDPFORWARDER_BIND_FAILED, "
          "packet->systemAddress=%s, endpointSystemAddress=%s, "
          "forwardingPort=%i, forwardingSocket=%i\n",
          __FILE__, __LINE__, buff2, buff3, forwardingPort, forwardingSocket));
    SendFailureOnCannotForward(packet->guid, endpointGuid);
  ...
  if (debugInterface) {
    ...
    debugInterface->ShowDiagnostic(
        FormatStringTS(buff,
                       ...
                       "guid=%I64d (miniPunchesInProgress.Size() = %d)",
  ...
  while (i < miniPunchesInProgress.Size()) {
    if (miniPunchesInProgress[i].sourceGuid == packet->guid ||
      ...
      if (debugInterface) {
        ...
        debugInterface->ShowDiagnostic(FormatStringTS(
            buff,
            "Processing ID_ROUTER_2_MINI_PUNCH_REPLY_BOUNCE, "
            "gotReplyFromSource=%i gotReplyFromEndpoint=%i at %s:%i\n",
      ...
      if (miniPunchesInProgress[i].gotReplyFromEndpoint == true &&
          miniPunchesInProgress[i].gotReplyFromSource == true) {
        SendForwardingSuccess(ID_ROUTER_2_REROUTED,
                              miniPunchesInProgress[i].endpointGuid,
                              miniPunchesInProgress[i].sourceGuid,
                              miniPunchesInProgress[i].forwardingPort);
        SendForwardingSuccess(ID_ROUTER_2_FORWARDING_ESTABLISHED,
                              miniPunchesInProgress[i].sourceGuid,
                              miniPunchesInProgress[i].endpointGuid,
                              miniPunchesInProgress[i].forwardingPort);
        miniPunchesInProgress.RemoveAtIndexFast(i);
...
void Router2::OnMiniPunchReply(Packet *packet) {
  ...
  if (debugInterface) {
    ...
    debugInterface->ShowDiagnostic(FormatStringTS(
        buff, "Sending ID_ROUTER_2_MINI_PUNCH_REPLY_BOUNCE (%s) at %s:%i\n",
...
void Router2::OnRerouted(Packet *packet) {
  ...
  if (forwardingIndex < forwardedConnectionList.Size()) {
    ...
    // add to forwarding list. This is only here to avoid reporting direct
    // connections in Router2::ReturnFailureOnCannotForward
...
bool Router2::OnForwardingSuccess(Packet *packet) {
  ...
  if (forwardingIndex < forwardedConnectionList.Size()) {
    ...
    if (debugInterface) {
      ...
      debugInterface->ShowDiagnostic(
          FormatStringTS(buff,
                         "Got ID_ROUTER_2_FORWARDING_ESTABLISHED, returning "
                         "ID_ROUTER_2_REROUTED, Calling "
                         "RakPeer::ChangeSystemAddress at %s:%i\n",
                         _FILE_AND_LINE_));
    }
    ...
    if (debugInterface) {
      ...
      debugInterface->ShowDiagnostic(
          FormatStringTS(buff,
                         "Got and returning to user "
                         "ID_ROUTER_2_FORWARDING_ESTABLISHED at %s:%i\n",
                         _FILE_AND_LINE_));
    }
  }
  return true; // Return packet to user
