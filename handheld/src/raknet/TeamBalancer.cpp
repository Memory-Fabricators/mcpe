...
void TeamBalancer::SetLockTeams(bool lock) {
  ...
  // If lock is false, and teams were set to be forced as even, then run through
  // the even team algorithm
  if (lockTeams == false) {
    // Process even swaps
    TeamId i, j;
...
void TeamBalancer::RequestSpecificTeam(NetworkID memberId, TeamId desiredTeam) {
  ...
  // Also flag that we have requested a team, and record desiredTeam in case the
  // host changes and it needs to be resent.
  BitStream bsOut;
...
PluginReceiveResult TeamBalancer::OnReceive(Packet *packet) {
  ...
  // - This user does not already have a team. Assign a team as if the user
  // called RequestAnyTeam(), with a preference for the requested team. Return
  // ID_TEAM_BALANCER_TEAM_ASSIGNED once the team has been assigned. If teams
  // are not locked
  // - If even team balancing is on, only assign this user if this would not
  // cause teams to be unbalanced. If teams WOULD be unbalanced, then flag this
  // user as wanting to join this team. Return
  // ID_TEAM_BALANCER_REQUESTED_TEAM_CHANGE_PENDING
  // - If the destination team is full, flag this user as wanting to join this
  // team. Return ID_TEAM_BALANCER_REQUESTED_TEAM_CHANGE_PENDING
...
void TeamBalancer::RemoveByGuid(RakNetGUID rakNetGUID) {
  // If we are the host, and the closed connection has a team, and teams are not
  // locked:
  if (WeAreHost()) {
      ...
      if (teamMembers[droppedMemberIndex].memberGuid == rakNetGUID) {
        ...
        if (lockTeams == false) {
          if (forceTeamsToBeEven) {
            ...
            // - teams were NOT forced to be even, and the team the dropped
            // player on was full, then move users wanting to join that team (if
            // any)
...
void TeamBalancer::OnStatusUpdateToNewHost(Packet *packet) {
  ...
  for (uint8_t i = 0; i < requestedTeamChangeListSize; i++) {
    ...
    if (memberIndex == (unsigned int)-1) {
      // Add this system (by GUID) to the list of members if he is not already
      // there Also update his requested team flag. Do not process balancing on
      // requested teams, since we don't necessarily have all data from all
      // systems yet and hopefully the state during the host migration was
      // stable.
      if (tm.currentTeam == UNASSIGNED_TEAM_ID) {
...
void TeamBalancer::OnRequestSpecificTeam(Packet *packet) {
  ...
  if (memberIndex == (unsigned int)-1) {
    ...
    if (TeamsWouldBeEvenOnSwitch(tm.requestedTeam, oldTeamThisUserWasOn) ==
      ...
      // If someone wants to join this user's old team, and we want to join
...
void TeamBalancer::EvenTeams(void) {
  // Ensure all teams are even. If not, pick players at random from
  // overpopulated teams, and move to underpopulated teams.
  int minMembersOnASingleTeam;
  int maxMembersOnASingleTeam;
  GetMinMaxTeamMembers(minMembersOnASingleTeam, maxMembersOnASingleTeam);

  // First select among players that have requested to switch teams, if any,
  // before choosing players that did not want to switch teams. Players that are
  // moved should be notified of ID_TEAM_BALANCER_TEAM_ASSIGNED
...
TeamId TeamBalancer::GetNextDefaultTeam(void) {
  // Accounting for team balancing and team limits, get the team a player should
  // be placed on
...
bool TeamBalancer::TeamWouldBeOverpopulatedOnAddition(
  ...
  // Accounting for team balancing and team limits, would this team be
  // overpopulated if a member was added to it?
