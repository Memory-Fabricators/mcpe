/// \file
/// \brief Message filter plugin. Assigns systems to FilterSets.  Each FilterSet
/// limits what messages are allowed.  This is a security related plugin.
///
...
/// MessageIdentifier (ID_*) values shoudln't go higher than this.  Change it if
/// you do.
...
/// \brief Assigns systems to FilterSets.  Each FilterSet limits what kinds of
/// messages are allowed.
/// \details The MessageFilter plugin is used for security where you limit what
/// systems can send what kind of messages.<BR> You implicitly define
/// FilterSets, and add allowed message IDs to these FilterSets.<BR> You then
/// add systems to these filters, such that those systems are limited to sending
/// what the filters allows.<BR> You can automatically assign systems to a
/// filter.<BR> You can automatically kick and possibly ban users that stay in a
/// filter too long, or send the wrong message.<BR> Each system is a member of
/// either zero or one filters.<BR> Add this plugin before any plugin you wish
/// to filter (most likely just add this plugin before any other).
...
public:
  ...
  /// Defaults to -1
  /// \param[in] filterSetID Which filter to add new systems to.  <0 for do not
  /// add.
  void SetAutoAddNewConnectionsToFilter(int filterSetID);

  /// Allow a range of message IDs
  /// Always allowed by default: ID_CONNECTION_REQUEST_ACCEPTED through
  /// ID_DOWNLOAD_PROGRESS Usually you specify a range to make it easier to add
  /// new enumerations without having to constantly refer back to this function.
  /// \param[in] allow True to allow this message ID, false to disallow. By
  /// default, all messageIDs except the noted types are disallowed.  This
  /// includes messages from other plugins!
  /// \param[in] messageIDStart The first ID_* message to allow in the range.
  /// Inclusive.
  /// \param[in] messageIDEnd The last ID_* message to allow in the range.
  /// Inclusive.
  ...
  /// What action to take on a disallowed message.  You can kick or not.  You
  /// can add them to the ban list for some time By default no action is taken.
  /// The message is simply ignored. param[in] 0 for permanent ban, >0 for ban
  /// time in milliseconds.
  /// \param[in] kickOnDisallowed kick the system that sent a disallowed
  /// message.
  /// \param[in] banOnDisallowed ban the system that sent a disallowed message.
  /// See \a banTimeMS for the ban duration
  /// \param[in] banTimeMS Passed to the milliseconds parameter of
  /// RakPeer::AddToBanList.
  /// \param[in] filterSetID A user defined ID to represent a filter set.  If no
  /// filter with this ID exists, one will be created with default settings.
  ...
  /// is no limit to how long a connection can stay in a particular filter set.
  /// \param[in] allowedTimeMS How many milliseconds to allow a connection to
  /// stay in this filter set.
  /// \param[in] banOnExceed True or false to ban the system, or not, when \a
  /// allowedTimeMS is exceeded
  /// \param[in] banTimeMS Passed to the milliseconds parameter of
  /// RakPeer::AddToBanList.
  /// \param[in] filterSetID A user defined ID to represent a filter set.  If no
  /// filter with this ID exists, one will be created with default settings.
  ...
  /// SetAutoAddNewConnectionsToFilter() This function is used to change the
  /// filter set a system is using, to add it to a new filter set, or to remove
  /// it from all existin filter sets.
  /// \param[in] addressOrGUID The system we are referring to
  /// \param[in] filterSetID A user defined ID to represent a filter set.  If no
  /// filter with this ID exists, one will be created with default settings.  If
  ...
  /// Using anything other than -1 for \a filterSetID is slow, so you should
  /// store the returned value.
  /// \param[in] filterSetID The filter set to limit to.  Use -1 for none (just
  /// returns the total number of filter systems in that case).
  ...
  /// Delete a FilterSet.  All systems formerly subscribed to this filter are
  /// now unrestricted.
