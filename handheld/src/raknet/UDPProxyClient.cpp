...
// bool operator<( const
// DataStructures::MLKeyRef<UDPProxyClient::ServerWithPing> &inputKey, const
// UDPProxyClient::ServerWithPing &cls ) {return inputKey.Get().serverAddress <
// cls.serverAddress;} bool operator>( const
// DataStructures::MLKeyRef<UDPProxyClient::ServerWithPing> &inputKey, const
// UDPProxyClient::ServerWithPing &cls ) {return inputKey.Get().serverAddress >
// cls.serverAddress;} bool operator==( const
// DataStructures::MLKeyRef<UDPProxyClient::ServerWithPing> &inputKey, const
...
bool UDPProxyClient::RequestForwarding(
  ...
  // Pretty much a bug not to set the result handler, as otherwise you won't
  // know if the operation succeeed or not
...
void UDPProxyClient::Update(void) {
  ...
  while (idx1 < pingServerGroups.Size()) {
    ...
    if (psg->serversToPing.Size() > 0 &&
        RakNet::GetTimeMS() >
            psg->startPingTime + DEFAULT_UNRESPONSIVE_PING_TIME) {
      // If they didn't reply within DEFAULT_UNRESPONSIVE_PING_TIME, just give
      // up on them
    ...
    case ID_UDP_PROXY_FORWARDING_NOTIFICATION: {
      ...
      case ID_UDP_PROXY_FORWARDING_SUCCEEDED: {
        ...
        if (packet->data[1] == ID_UDP_PROXY_FORWARDING_SUCCEEDED) {
          ...
          // router adds an entry to the routing table. Otherwise the router
          // would block the incoming datagrams from source It doesn't matter if
          // the message actually arrives as long as it goes through the router
      ...
      case ID_UDP_PROXY_IN_PROGRESS:
        if (resultHandler)
          resultHandler->OnForwardingInProgress(packet->systemAddress,
...
bool UDPProxyClient::PingServerGroup::AreAllServersPinged(void) const {
  ...
  for (serversToPingIndex = 0; serversToPingIndex < serversToPing.Size();
    ...
    if (serversToPing[serversToPingIndex].ping ==
        DEFAULT_UNRESPONSIVE_PING_TIME)
      return false;
  }
  return true;
}
