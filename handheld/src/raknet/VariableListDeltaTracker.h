...
namespace RakNet {
/// Class to write a series of variables, copy the contents to memory, and
/// return if the newly written value is different than what was last written
...
public:
  ...
  /// Records the passed value of the variable to memory, and returns true if
  /// the value is different from the write before that (or if it is the first
  /// write)
  /// \pre Call StartWrite() before doing the first of a series of calls to
  /// WriteVar or other functions that call WriteVar
  /// \note Variables must be of the same type, written in the same order, each
  /// time
  ...
  /// Calls WriteVarToBitstream(). Additionally, adds the boolean result of
  ...
  /// Variables flagged dirty will cause WriteVar() to return true, even if the
  /// variable had not otherwise changed This updates all the variables in the
  /// list, where in each index \a varsWritten is true, so will the variable at
  /// the corresponding index be flagged dirty
