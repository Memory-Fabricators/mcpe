...
std::string I18n::getDescriptionString(const ItemInstance &item) {
  // Convert to lower. Normally std::transform would be used, but tolower might
  // be implemented with a macro in certain C-implementations -> messing stuff
  // up
  ...
  if (s[0] == 't')
    s = Util::stringReplace(s, "tile.", "desc.");
  if (s[0] == 'i')
    s = Util::stringReplace(s, "item.", "desc.");
  if (I18n::get(s, trans))
    return trans;
