...
static void setupExternalPath(JNIEnv *env, MAIN_CLASS *app) {
  ...
  jmethodID method = env->GetStaticMethodID(
      clazz, "getExternalStorageDirectory", "()Ljava/io/File;");
...
// Keyboard events
//
JNIEXPORT void JNICALL Java_com_mojang_minecraftpe_MainActivity_nativeOnKeyDown(
    JNIEnv *env, jclass cls, jint keyCode) {
  LOGI("@nativeOnKeyDown: %d\n", keyCode);
  Keyboard::feed(keyCode, true);
}
JNIEXPORT void JNICALL Java_com_mojang_minecraftpe_MainActivity_nativeOnKeyUp(
    JNIEnv *env, jclass cls, jint keyCode) {
...
// Mouse events
//
JNIEXPORT void JNICALL Java_com_mojang_minecraftpe_MainActivity_nativeMouseDown(
    JNIEnv *env, jclass cls, jint pointerId, jint buttonId, jfloat x,
    jfloat y) {
  // LOGI("@nativeMouseDown: %f %f\n", x, y);
  mouseDown(1, x, y);
  pointerDown(pointerId, x, y);
}
JNIEXPORT void JNICALL Java_com_mojang_minecraftpe_MainActivity_nativeMouseUp(
    JNIEnv *env, jclass cls, jint pointerId, jint buttonId, jfloat x,
    jfloat y) {
...
JNIEXPORT void JNICALL Java_com_mojang_minecraftpe_MainActivity_nativeMouseMove(
