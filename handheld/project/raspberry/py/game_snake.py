...
class Level:
    ...
    def setup(self, numApples):
        ...
        margin = Vec3(1, 0, 1)
        api.world.setBlocks(
            self.pos - margin + Vec3.down(3), self.pos + size + margin, BlockType.ROCK
        )
...
class SnakeGame:
    ...
    def onTile(self, snake, pos):
        x, y = pos.x, pos.z
        if self.level.isWall(x, y):
            snake.kill()
        if self.level.isFood(x, y):
            snake.grow(2)
