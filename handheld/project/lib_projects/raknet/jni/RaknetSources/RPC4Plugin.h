/// \file
/// \brief Remote procedure call, supporting C functions only. No external
...
/// \details This should not be used at the same time as RPC3. This is a less
/// functional version of RPC3, and is here for users that do not want the Boost
/// dependency of RPC3.
...
/// \brief Error codes returned by a remote system as to why an RPC function
/// call cannot execute
/// \details Error code follows packet ID ID_RPC_REMOTE_ERROR, that is
/// packet->data[1]<BR> Name of the function will be appended starting at
/// packet->data[2]
/// \ingroup RPC_PLUGIN_GROUP
enum RPCErrorCodes {
  /// Named function was not registered with RegisterFunction(). Check your
  /// spelling.
  RPC_ERROR_FUNCTION_NOT_REGISTERED,
};

/// \brief Instantiate this class globally if you want to register a function
/// with RPC4 at the global space
...
/// \brief The RPC4 plugin is just an association between a C function pointer
/// and a string.
/// \details It is for users that want to use RPC, but do not want to use boost.
/// You do not have the automatic serialization or other features of RPC3, and
/// C++ member calls are not supported.
...
public:
  ...
  /// \details The hash of the function name will be stored as an association
  /// with the function pointer When a call is made to call this function from
  /// the \a Call() or CallLoopback() function, the function pointer will be
  /// invoked with the passed bitStream to Call() and the actual Packet that
  /// RakNet got.
  /// \sa RegisterPacketCallback()
  /// \param[in] uniqueID Identifier to be associated with \a functionPointer.
  /// If this identifier is already in use, the call will return false.
  ...
  /// occurs, all slots with the same identifier are called.
  /// \param[in] sharedIdentifier A string to identify the slot. Recommended to
  /// be the same as the name of the function.
  /// \param[in] functionPtr Pointer to the function. For C, just pass the name
  /// of the function. For C++, use ARPC_REGISTER_CPP_FUNCTION
  /// \param[in] callPriority Slots are called by order of the highest
  ...
  /// returned, execute CallLoopback() on a function previously registered with
  /// RegisterFunction() For example, you could call "OnClosedConnection"
  /// whenever you get ID_DISCONNECTION_NOTIFICATION or ID_CONNECTION_LOST
  ...
  /// \brief Unregister a function pointer previously registered with
  /// RegisterFunction()
  /// \param[in] Identifier originally passed to RegisterFunction()
  /// \return True if the hash of uniqueID was in use, and hence removed. false
  /// otherwise.
  ...
  /// \param[in] uniqueID Identifier passed as uniqueID to
  /// RegisterLocalCallback()
  /// \param[in] messageId Identifier passed as messageId to
  /// RegisterLocalCallback()
  /// \return True if the combination of uniqueID and messageId was in use, and
  /// hence removed
  bool UnregisterLocalCallback(const char *uniqueID, MessageID messageId);

  /// Remove the association created with RegisterSlot()
  /// \param[in] sharedIdentifier Identifier passed as sharedIdentifier to
  /// RegisterSlot()
  bool UnregisterSlot(const char *sharedIdentifier);

  /// \deprecated Use RegisterSlot() and Signal() with your own RakNetGUID as
  /// the send target Send to the attached instance of RakPeer. See
  /// RakPeerInterface::SendLoopback()
  /// \param[in] Identifier originally passed to RegisterFunction() on the local
  /// system
  /// \param[in] bitStream bitStream encoded data to send to the function
  /// callback
  ...
  /// \param[in] uniqueID Identifier originally passed to RegisterFunction() on
  /// the remote system(s)
  /// \param[in] bitStream bitStream encoded data to send to the function
  /// callback
  ...
  /// system
  /// \note This function does not return until the remote system responds,
  /// disconnects, or was never connected to begin with
  /// \param[in] Identifier originally passed to RegisterBlockingFunction() on
  /// the remote system(s)
  /// \param[in] bitStream bitStream encoded data to send to the function
  /// callback
  /// \param[in] priority See RakPeerInterface::Send()
  ...
  /// \param[in] orderingChannel See RakPeerInterface::Send()
  /// \param[in] systemIdentifier See RakPeerInterface::Send()
  /// \param[out] returnData Written to by the function registered with
  ...
  /// with RegisterSlot()
  /// \param[in] sharedIdentifier parameter of the same name passed to
  /// RegisterSlot() on the remote system
  /// \param[in] bitStream bitStream encoded data to send to the function
  /// callback
  /// \param[in] priority See RakPeerInterface::Send()
  /// \param[in] reliability See RakPeerInterface::Send()
