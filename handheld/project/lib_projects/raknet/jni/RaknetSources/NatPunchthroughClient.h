...
// Trendnet TEW-632BRP sometimes starts at port 1024 and increments
// sequentially. Zonnet zsr1134we. Replies go out on the net, but are always
// absorbed by the remote router?? Dlink ebr2310 to Trendnet ok Trendnet
// TEW-652BRP to Trendnet 632BRP OK Trendnet TEW-632BRP to Trendnet 632BRP OK
...
struct RAK_DLL_EXPORT PunchthroughConfiguration {
  ...
  /// After giving up on one internal port, how long to wait before trying the
  /// next port
  ...
  /// After giving up on one external  port, how long to wait before trying the
  /// next port
  ...
  /// Should be high enough to try all internal IP addresses on the majority of
  /// computers
  ...
  /// message on a higher numbered port before responding to a lower numbered
  /// port from the other system
  bool retryOnFailure;
};
...
/// \details Maintain connection to NatPunchthroughServer to process incoming
/// connection attempts through NatPunchthroughClient<BR> Client will send
/// datagrams to port to estimate next port<BR> Will simultaneously connect with
/// another client once ports are estimated.
...
public:
  ...
  /// \param[in] destination The system to punch. Must already be connected to
  /// \a facilitator
  ...
  /// You will get ID_NAT_TARGET_NOT_CONNECTED, ID_NAT_TARGET_UNRESPONSIVE,
  /// ID_NAT_CONNECTION_TO_TARGET_LOST, ID_NAT_ALREADY_IN_PROGRESS, or
  /// ID_NAT_PUNCHTHROUGH_FAILED on failures of various types However, if you
  ...
  /// Same as calling OpenNAT for a list of systems, but reply is delayed until
  /// all systems pass. This is useful for peer to peer games where you want to
  /// connect to every system in the remote session, not just one particular
  /// system
  /// \note For cloud computing, all systems in the group must be connected to
  /// the same facilitator since we're only specifying one You will get
  /// ID_NAT_GROUP_PUNCH_SUCCEEDED on success You will get
  /// ID_NAT_TARGET_NOT_CONNECTED, ID_NAT_ALREADY_IN_PROGRESS, or
  ...
  /// \param[in] i Pointer to an interface. The pointer is stored, so don't
  /// delete it while in progress. Pass 0 to clear.
...
protected:
  ...
  struct SendPing {
    ...
    int attemptCount;
    int retryCount;
    int punchingFixedPortAttempts; // only used for
                                   // TestMode::PUNCHING_FIXED_PORT
    uint16_t sessionId;
    // Give priority to internal IP addresses because if we are on a LAN, we
    // don't want to try to connect through the internet
    enum TestMode {
      ...
      // If you continue punching on a different port for the same IP it bans
      // you and the communication becomes unidirectioal
      PUNCHING_FIXED_PORT,
  ...
  // The first time we fail a NAT attempt, we add it to failedAttemptList and
  // try again, since sometimes trying again later fixes the problem The second
  // time we fail, we return ID_NAT_PUNCHTHROUGH_FAILED
  struct AddrAndGuid {
    SystemAddress addr;
    RakNetGUID guid;
  };
