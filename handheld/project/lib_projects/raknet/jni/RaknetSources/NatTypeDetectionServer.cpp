...
void NatTypeDetectionServer::Update(void) {
  ...
  // Only socket that receives messages is s3p4, to see if the external address
  // is different than that of the connection to rakPeerInterface
  char data[MAXIMUM_MTU_SIZE];
  int len;
  SystemAddress senderAddr;
  len = NatTypeRecvFrom(data, s3p4, senderAddr);
  // Client is asking us if this is port restricted. Only client requests of
  // this type come in on s3p4
  ...
  while (i < (int)natDetectionAttempts.Size()) {
    if (time > natDetectionAttempts[i].nextStateTime) {
      ...
      case STATE_TESTING_NONE_2:
        ...
        // S4P5 sends to C2. If arrived, no NAT. Done. (Else S4P5 potentially
      ...
      case STATE_TESTING_FULL_CONE_2:
        ...
        // S2P3 sends to C1 (Different address, different port, to previously
        // used port on client). If received, Full-cone nat. Done.  (Else S2P3
      ...
      case STATE_TESTING_ADDRESS_RESTRICTED_2:
        ...
        // S1P2 sends to C1 (Same address, different port, to previously used
        // port on client). If received, address-restricted cone nat. Done.
        saOut = natDetectionAttempts[i].systemAddress;
        saOut.SetPort(natDetectionAttempts[i].systemAddress.GetPort());
      ...
      case STATE_TESTING_PORT_RESTRICTED_2:
        // C1 sends to S3P4. If address of C1 as seen by S3P4 is the same as the
        // address of C1 as seen by S1P1, then port-restricted cone nat. Done
      ...
      default:
        printf("Warning, exceeded final check "
               "STATE_TESTING_PORT_RESTRICTED_2.\nExpected that client would "
               "have sent NAT_TYPE_PORT_RESTRICTED on s3p4.\nDefaulting to "
               "Symmetric\n");
        bs.Write((unsigned char)ID_NAT_TYPE_DETECTION_RESULT);
        bs.Write((unsigned char)NAT_TYPE_SYMMETRIC);
        rakPeerInterface->Send(&bs, HIGH_PRIORITY, RELIABLE, 0,
