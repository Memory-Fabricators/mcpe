...
/// This can be used to determine what permissions are should be allowed to the
/// other system
///
...
enum DynDnsResultCode {
  ...
  // ----- Permanent failure -----
  RC_ABUSE, // Your host has been blocked, too many failures disable your
            // account
  RC_TCP_FAILED_TO_START, // TCP port already in use
  RC_TCP_DID_NOT_CONNECT, // DynDNS down?
  RC_UNKNOWN_RESULT, // DynDNS returned a result code that was not documented as
                     // of 12/4/2010 on
                     // http://www.dyndns.com/developers/specs/flow.pdf
  RC_PARSING_FAILURE, // Can't read the result returned, format change?
  RC_CONNECTION_LOST_WITHOUT_RESPONSE, // Lost the connection to DynDNS while
                                       // communicating
  RC_BAD_AGENT,                        // ???
  RC_BAD_SYS,                          // ???
  RC_DNS_ERROR,                        // ???
...
public:
  ...
  void UpdateHostIP(const char *dnsHost, const char *newIPAddress,
...
protected:
  void Stop(void);
  void SetCompleted(RakNet::DynDnsResultCode _result,
