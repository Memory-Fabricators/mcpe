...
/// \brief Code that uses the TCPInterface class to communicate with the
/// Rackspace API servers
/// \pre Compile RakNet with OPEN_SSL_CLIENT_SUPPORT set to 1
/// \pre Packets returned from TCPInterface::OnReceive() must be passed to
/// Rackspace::OnReceive()
/// \pre Packets returned from TCPInterface::HasLostConnection() must be passed
/// to Rackspace::OnClosedConnection()
class RAK_DLL_EXPORT Rackspace {
public:
  Rackspace();
  ~Rackspace();

  /// \brief Authenticate with Rackspace servers, required before executing any
  /// commands.
  /// \details All requests to authenticate and operate against Cloud Servers
  /// are performed using SSL over HTTP (HTTPS) on TCP port 443. Times out after
  /// 24 hours - if you get RET_Authenticate_Unauthorized in the
  /// RackspaceEventCallback callback, call again
  /// \sa RackspaceEventCallback::OnAuthenticationResult()
  /// \param[in] _tcpInterface An instance of TCPInterface, build with
  /// OPEN_SSL_CLIENT_SUPPORT 1 and already started
  /// \param[in] _authenticationURL See
  ...
  /// \param[in] _rackspaceCloudUsername Username you registered with Rackspace
  /// on their website
  /// \param[in] _apiAccessKey Obtain your API access key from the Rackspace
  /// Cloud Control Panel in the Your Account API Access section.
  ...
  /// \details Create a server with a given image (harddrive contents) and
  /// flavor (hardware configuration) Get the available images with ListImages()
  /// Get the available flavors with ListFlavors()
  /// It is possible to configure the server in more detail. See the XML schema
  /// at http://docs.rackspacecloud.com/servers/api/v1.0 You can execute such a
  /// custom command by calling AddOperation() manually. See the implementation
  /// of CreateServer for how to do so. The server takes a while to build. Call
  /// GetServerDetails() to get the current build status. Server id to pass to
  /// GetServerDetails() is returned in the field <server ... id="1234">
  /// \sa
  /// http://docs.rackspacecloud.com/servers/api/v1.0/cs-devguide-20110112.pdf
  /// \sa RackspaceEventCallback::OnCreateServerResult()
  /// \param[in] name Name of the server. Only alphanumeric characters, periods,
  /// and hyphens are valid. Server Name cannot start or end with a period or
  /// hyphen.
  ...
  /// \param[in] serverId Which server to get details on. You can call
  /// ListServers() to get the list of active servers.
  ...
  /// \param[in] serverId Which server to get details on. You can call
  /// ListServers() to get the list of active servers.
  /// \param[in] newName The new server name. Leave blank to leave unchanged.
  /// Only alphanumeric characters, periods, and hyphens are valid. Server Name
  ...
  /// \param[in] serverId Which server to operate on. You can call ListServers()
  ...
  /// \brief Changes the hardware configuration of a server. This does not take
  /// effect until you call ConfirmResizedServer()
  /// \sa
  /// http://docs.rackspacecloud.com/servers/api/v1.0/cs-devguide-20110112.pdf
  ...
  /// \brief List all images (software configs, including operating systems),
  /// which includes images you create yourself
  /// \sa
  /// http://docs.rackspacecloud.com/servers/api/v1.0/cs-devguide-20110112.pdf
  ...
  /// \brief Images a running server. This essentially copies the harddrive, and
  /// lets you start a server with the same harddrive contents later
  /// \sa
  /// http://docs.rackspacecloud.com/servers/api/v1.0/cs-devguide-20110112.pdf
  ...
  /// \brief Adds a callback to the list of callbacks to be called when any of
  /// the above functions finish executing The callbacks are called in the order
  /// they are added
  void AddEventCallback(RackspaceEventCallback *callback);
  /// \brief Removes a callback from the list of callbacks to be called when any
  /// of the above functions finish executing The callbacks are called in the
  ...
  /// Call this when TCPInterface returns something other than
  /// UNASSIGNED_SYSTEM_ADDRESS from HasLostConnection()
