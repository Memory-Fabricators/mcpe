...
/// connected to by a telnet client.
///
...
RAK_THREAD_DECLARATION(RakNet::ConnectionAttemptLoop) {
  ...
  SOCKET sockfd =
      tcpInterface->SocketConnect(str1, systemAddress.GetPort(), socketFamily);
  if (sockfd == (SOCKET)-1) {
...
#pragma warning(disable                                                        \
      ...
      for (i = 0; i < (unsigned int)sts->remoteClientsLength; i++) {
        sts->remoteClients[i].isActiveMutex.Lock();
        if (sts->remoteClients[i].isActive &&
            sts->remoteClients[i].socket != INVALID_SOCKET) {
          ...
          if (sts->remoteClients[i].socket >
              largestDescriptor) // @see largestDescriptorDef
              ...
              if (len > 0) {
                ...
                incomingMessage->data[len] =
                    0; // Null terminate this so we can print it out as regular
                       // strings.  This is different from RakNet which does not
                       // do this.
                       //								printf("RECV:
                       //%s\n",incomingMessage->data);
