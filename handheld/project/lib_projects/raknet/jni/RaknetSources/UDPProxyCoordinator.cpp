...
static const int DEFAULT_UNRESPONSIVE_PING_TIME =
    DEFAULT_CLIENT_UNRESPONSIVE_PING_TIME + 1000;

using namespace RakNet;

// bool operator<( const DataStructures::MLKeyRef<unsigned short> &inputKey,
// const UDPProxyCoordinator::ServerWithPing &cls ) {return inputKey.Get() <
// cls.ping;} bool operator>( const DataStructures::MLKeyRef<unsigned short>
// &inputKey, const UDPProxyCoordinator::ServerWithPing &cls ) {return
// inputKey.Get() > cls.ping;} bool operator==( const
// DataStructures::MLKeyRef<unsigned short> &inputKey, const
...
void UDPProxyCoordinator::OnPingServersReplyFromClientToCoordinator(
  ...
  // Both systems have to give us pings to progress here. Otherwise will timeout
  // in Update()
