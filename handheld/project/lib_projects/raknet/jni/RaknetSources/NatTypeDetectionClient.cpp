...
NatTypeDetectionClient::NatTypeDetectionClient() { c2 = INVALID_SOCKET; }
...
void NatTypeDetectionClient::OnCompletion(NATTypeDetectionResult result) {
  ...
  // Symmetric and port restricted are determined by server, so no need to
  // notify server we are done
  if (result != NAT_TYPE_PORT_RESTRICTED && result != NAT_TYPE_SYMMETRIC) {
    // Otherwise tell the server we got this message, so it stops sending tests
    // to us
    RakNet::BitStream bs;
    bs.Write((unsigned char)ID_NAT_TYPE_DETECTION_REQUEST);
    bs.Write(false); // Done
