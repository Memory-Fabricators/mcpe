...
int UDPForwarder::SrcAndDestForwardEntryComp(
    const UDPForwarder::SrcAndDest &inputKey,
    UDPForwarder::ForwardEntry *const &cls) {
...
/*
bool operator<( const DataStructures::MLKeyRef<UDPForwarder::SrcAndDest>
&inputKey, const UDPForwarder::ForwardEntry *cls )
{
        return inputKey.Get().source < cls->srcAndDest.source ||
                (inputKey.Get().source == cls->srcAndDest.source &&
inputKey.Get().dest < cls->srcAndDest.dest);
}
bool operator>( const DataStructures::MLKeyRef<UDPForwarder::SrcAndDest>
&inputKey, const UDPForwarder::ForwardEntry *cls )
{
...
UDPForwarder::ForwardEntry::ForwardEntry() {
  ...
  updatedSourcePort = false;
  updatedDestPort = false;
}
UDPForwarder::ForwardEntry::~ForwardEntry() {
...
void UDPForwarder::UpdateThreaded_Old(void) {
  ...
  while (i < forwardList.Size()) {
    if (curTime >
        ...
        curTime > forwardList[i]->timeLastDatagramForwarded +
                      forwardList[i]->timeoutOnNoDataMS) {
...
#pragma warning(disable                                                        \
  ...
  if (selectResult > 0) {
    ...
    for (i = 0; i < forwardList.Size(); i++) {
      ...
      // I do this because I'm updating the forwardList, and don't want to lose
      // FD_ISSET as the list is no longer in order
    ...
    while (entriesToRead.IsEmpty() == false) {
      ...
      receivedDataLen = recvfrom__(forwardEntry->socket, data, MAXIMUM_MTU_SIZE,
                                   flag, (sockaddr *)&sa, (socklen_t *)&len2);
      ...
      if (forwardEntry->srcAndDest.source.address.addr4.sin_addr.s_addr ==
              sa.sin_addr.s_addr &&
          forwardEntry->updatedSourcePort == false &&
          forwardEntry->srcAndDest.dest.GetPort() != portnum) {
        forwardEntry->updatedSourcePort = true;

        if (forwardEntry->srcAndDest.source.GetPort() != portnum) {
          unsigned int index;
          SrcAndDest srcAndDest(forwardEntry->srcAndDest.dest,
                                forwardEntry->srcAndDest.source);
          bool objectExists;
          ...
          forwardList.Insert(forwardEntry->srcAndDest, forwardEntry, true,
                             _FILE_AND_LINE_);
        }
      }
      ...
      if (forwardEntry->srcAndDest.source.address.addr4.sin_addr.s_addr ==
              sa.sin_addr.s_addr &&
          forwardEntry->srcAndDest.source.GetPort() == portnum) {
        // Forward to dest
        len = 0;
        sockaddr_in saOut;
        saOut.sin_port =
            forwardEntry->srcAndDest.dest.GetPortNetworkOrder(); // User port
        saOut.sin_addr.s_addr =
            forwardEntry->srcAndDest.dest.address.addr4.sin_addr.s_addr;
        saOut.sin_family = AF_INET;
        do {
          len = sendto__(forwardEntry->socket, data, receivedDataLen, 0,
                         (const sockaddr *)&saOut, sizeof(saOut));
        } while (len == 0);
      ...
      if (forwardEntry->srcAndDest.dest.address.addr4.sin_addr.s_addr ==
              sa.sin_addr.s_addr &&
          forwardEntry->updatedDestPort == false &&
          forwardEntry->srcAndDest.source.GetPort() != portnum) {
        forwardEntry->updatedDestPort = true;

        if (forwardEntry->srcAndDest.dest.GetPort() != portnum) {
          unsigned int index;
          SrcAndDest srcAndDest(forwardEntry->srcAndDest.source,
                                forwardEntry->srcAndDest.dest);
          bool objectExists;
          index = forwardList.GetIndexFromKey(srcAndDest, &objectExists);
          if (objectExists) {
            forwardList.RemoveAtIndex(index);
      ...
      if (forwardEntry->srcAndDest.dest.address.addr4.sin_addr.s_addr ==
        ...
        sockaddr_in saOut;
        saOut.sin_port =
            forwardEntry->srcAndDest.source.GetPortNetworkOrder(); // User port
        saOut.sin_addr.s_addr =
            forwardEntry->srcAndDest.source.address.addr4.sin_addr.s_addr;
        saOut.sin_family = AF_INET;
        do {
          len = sendto__(forwardEntry->socket, data, receivedDataLen, 0,
                         (const sockaddr *)&saOut, sizeof(saOut));
  ...
  if (selectResult > 0) {
    ...
    while (entriesToRead.IsEmpty() == false) {
      ...
      if (their_addr.ss_family == AF_INET) {
        ...
        //	receivedAddr.address.addr6.sin6_port=ntohs(
        //sockAddrIn6->sin6_port );
      }

      if (forwardEntry->srcAndDest.source.EqualsExcludingPort(receivedAddr) &&
          forwardEntry->updatedSourcePort == false &&
          forwardEntry->srcAndDest.dest.GetPort() != receivedAddr.GetPort()) {
        forwardEntry->updatedSourcePort = true;

        if (forwardEntry->srcAndDest.source.GetPort() !=
            receivedAddr.GetPort()) {
          unsigned int index;
          SrcAndDest srcAndDest(forwardEntry->srcAndDest.dest,
                                forwardEntry->srcAndDest.source);
          bool objectExists;
          ...
          forwardList.Push(forwardEntry, forwardEntry->srcAndDest,
                           _FILE_AND_LINE_);
        }
      }

      if (forwardEntry->srcAndDest.source.EqualsExcludingPort(receivedAddr) &&
          forwardEntry->srcAndDest.source.GetPort() == receivedAddr.GetPort()) {
        // Forward to dest
        len = 0;
        ...
        if (forwardEntry->srcAndDest.dest.address.addr4.sin_family == AF_INET) {
          do {
            len = sendto__(
                forwardEntry->socket, data, receivedDataLen, 0,
                (const sockaddr *)&forwardEntry->srcAndDest.dest.address.addr4,
                sizeof(sockaddr_in));
          } while (len == 0);
        } else {
          do {
            len = sendto__(
                forwardEntry->socket, data, receivedDataLen, 0,
                (const sockaddr *)&forwardEntry->srcAndDest.dest.address.addr6,
                sizeof(sockaddr_in));
          } while (len == 0);
        }
      ...
      if (forwardEntry->srcAndDest.dest.EqualsExcludingPort(receivedAddr) &&
          forwardEntry->updatedDestPort == false &&
          forwardEntry->srcAndDest.source.GetPort() != receivedAddr.GetPort()) {
        forwardEntry->updatedDestPort = true;
      ...
      if (forwardEntry->srcAndDest.dest.EqualsExcludingPort(receivedAddr) &&
        ...
        // Forward to source
        len = 0;
        if (forwardEntry->srcAndDest.source.address.addr4.sin_family ==
            AF_INET) {
          do {
            len = sendto__(forwardEntry->socket, data, receivedDataLen, 0,
                           (const sockaddr *)&forwardEntry->srcAndDest.source
                               .address.addr4,
                           sizeof(sockaddr_in));
          } while (len == 0);
        } else {
...
UDPForwarderResult UDPForwarder::AddForwardingEntry(
    SrcAndDest srcAndDest, RakNet::TimeMS timeoutOnNoDataMS,
    unsigned short *port, const char *forceHostAddress, short socketFamily) {
  (void)socketFamily;
  ...
  if (objectExists == false) {
    ...
    // Immediate hard close. Don't linger the readSocket, or recreating the
    // readSocket quickly on Vista fails.
    sock_opt = 0;
...
UDPForwarderResult UDPForwarder::StartForwarding(
    SystemAddress source, SystemAddress destination,
...
#else
  return StartForwardingThreaded(
      source, destination, timeoutOnNoDataMS, forceHostAddress, socketFamily,
...
UDPForwarderResult UDPForwarder::StartForwardingThreaded(
  ...
  UDPForwarderResult result =
      AddForwardingEntry(srcAndDest, timeoutOnNoDataMS, forwardingPort,
                         forceHostAddress, socketFamily);
...
void UDPForwarder::StopForwardingThreaded(SystemAddress source,
                                          SystemAddress destination) {
...
RAK_THREAD_DECLARATION(UpdateUDPForwarder) {
  ...
  while (udpForwarder->isRunning) {
    ...
    while (udpForwarder->threadOperationIncomingQueue.Size()) {
      ...
      if (threadOperation.operation ==
          UDPForwarder::ThreadOperation::TO_START_FORWARDING) {
        threadOperation.result = udpForwarder->StartForwardingThreaded(
            threadOperation.source, threadOperation.destination,
            threadOperation.timeoutOnNoDataMS, threadOperation.forceHostAddress,
            threadOperation.socketFamily, &threadOperation.forwardingPort,
            &threadOperation.forwardingSocket);
        ...
        udpForwarder->StopForwardingThreaded(threadOperation.source,
                                             threadOperation.destination);
      }
