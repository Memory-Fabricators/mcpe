/// \file
/// \brief Contains enumerations used by the ReplicaManager system.  This file
/// is a lightweight header, so you can include it without worrying about
/// linking in lots of other crap
///
...
enum ReplicaReturnResult {
  ...
  /// This means cancel the processing - don't send any network messages and
  /// don't change the current state.
  REPLICA_CANCEL_PROCESS,
  /// Same as REPLICA_PROCESSING_DONE, where a message is sent, but does not
  /// clear the send bit. Useful for multi-part sends with different reliability
  /// levels. Only currently used by Replica::Serialize
  REPLICA_PROCESS_AGAIN,
  /// Only returned from the Replica::SendConstruction interface, means act as
  /// if the other system had this object but don't actually Send a construction
  /// packet.  This way you will still send scope and serialize packets to that
  /// system
