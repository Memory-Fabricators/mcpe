...
struct RAK_DLL_EXPORT SocketDescriptor {
  ...
  /// addresses such as 94.198.81.195, you may have an address such as
  /// fe80::7c:31f7:fec4:27de%14. Encoding takes 16 bytes instead of 4, so IPV6
  /// is less efficient for bandwidth. On the positive side, NAT Punchthrough is
  /// not needed and should not be used with IPV6 because there are enough
  /// addresses that routers do not need to create address mappings.
