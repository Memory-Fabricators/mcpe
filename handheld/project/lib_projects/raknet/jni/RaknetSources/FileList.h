...
struct FileListNode {
  ...
  /// Length of \a data. May be greater than fileLength if prepended with a file
  /// hash
  ...
  /// If true, data and dataLengthBytes should be empty. This is just storing
  /// the filename
  bool isAReference;
};

/// Callback interface set with FileList::SetCallback() in case you want
/// progress notifications when FileList::AddFilesFromDirectory() is called
...
public:
  ...
  /// \brief This function is called when we are sending a file to a remote
  /// system.
...
public:
  ...
  /// \param[in] applicationDirectory The first part of the path. This is not
  /// stored as part of the filename.  Use \ as the path delineator.
  /// \param[in] subDirectory The rest of the path to the file. This is stored
  /// as a prefix to the filename
  /// \param[in] writeHash The first 4 bytes is a hash of the file, with the
  /// remainder the actual file data (should \a writeData be true)
  ...
  /// \brief Given the existing set of files, search applicationDirectory for
  /// the same files.
  ...
  /// the hash if written if \a alwaysWriteHash is true alwaysWriteHash and
  /// neverWriteHash are optimizations to avoid reading the file contents to
  /// generate the hash if not necessary because the file is missing or has
  /// different lengths anyway.
  /// \param[in] applicationDirectory The first part of the path. This is not
  /// stored as part of the filename. Use \ as the path delineator.
  /// \param[out] missingOrChangedFiles Output list written to
  /// \param[in] alwaysWriteHash If true, and neverWriteHash is false, will hash
  /// the file content of the file on disk, and write that as the file data with
  /// a length of SHA1_LENGTH bytes. If false, if the file length is different,
  /// will only write the filename.
  /// \param[in] neverWriteHash If true, will never write the hash, even if
  ...
  /// \details Specify dirSubset to only consider files that start with this
  /// path specify remoteSubdir to assume that all filenames in input start with
  /// this path, so strip it off when comparing filenames.
  /// \param[in] input Full list of files
  /// \param[out] output Files that we need to match input
  /// \param[in] dirSubset If the filename does not start with this path, just
  /// skip this file.
  ...
  /// \param[in] applicationDirectory Prepend this path to each filename.
  /// Trailing slash will be added if necessary. Use \ as the path delineator.
  /// \param[in] writeFileData True to read and store the file data. The first
  /// SHA1_LENGTH bytes will contain the hash if \a writeFileHash is true
  /// \param[in] writeFileHash True to read and store the hash of the file data.
  /// The first SHA1_LENGTH bytes will contain the hash if \a writeFileHash is
  /// true
  ...
  /// required for IncrementalReadInterface to process them incrementally
  void FlagFilesAsReferences(void);

  /// \brief Write all files to disk, prefixing the paths with
  ...
  /// \param[in] filename Name of a file, optionally prefixed with a partial or
  /// complete path. Use \ as the path delineator.
  /// \param[in] fullPathToFile Full path to the file on disk
  /// \param[in] data Contents to write
  /// \param[in] dataLength length of the data, which may be greater than
  /// fileLength should you prefix extra data, such as the hash
  /// \param[in] fileLength Length of the file
  /// \param[in] context User defined byte to store with each file. Use for
  /// whatever you want.
  /// \param[in] isAReference Means that this is just a reference to a file
  /// elsewhere - does not actually have any data
  ...
  /// itself
  /// \param[in] filename filename to store internally, anything you want, but
  ...
  /// \ as the path delineator.
  void DeleteFiles(const char *applicationDirectory);

  /// \brief Adds a callback to get progress reports about what the file list
  /// instances do.
  /// \param[in] cb A pointer to an externally defined instance of
  /// FileListProgress. This pointer is held internally, so should remain valid
  /// as long as this class is valid.
  void AddCallback(FileListProgress *cb);

  /// \brief Removes a callback
  /// \param[in] cb A pointer to an externally defined instance of
  /// FileListProgress that was previously added with AddCallback()
